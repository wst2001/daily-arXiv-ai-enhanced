...
Motivation: 现有3D化身建模面临保真度与泛化性的关键权衡：多视角工作室数据能实现高保真建模但泛化性差；大规模野外数据训练的模型泛化性好但质量低。需要同时实现高质量和泛化性。

Method: 采用预训练/后训练范式：首先在100万野外视频上进行预训练，学习外观和几何的广泛先验；然后在高质量策划数据上进行后训练，增强表达能力和保真度。模型采用编解码器架构实现前馈推理。

Result: LCA能够泛化到各种发型、服装和人口统计特征，同时提供精细的面部表情和手指级关节控制，保持强身份一致性。观察到对重新光照、宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA首次展示了大规模预训练/后训练范式在3D化身建模中的有效性，实现了高质量、可泛化的全身3D化身建模，为世界规模人群提供了高效的前馈推理解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在Design-to-Code生成中存在"整体瓶颈"，无法同时兼顾高层次结构层次和细粒度视觉细节，导致布局失真或使用通用占位符。

Method: DOne框架包含三个核心组件：(1)学习的布局分割模块分解复杂设计，避免启发式裁剪限制；(2)专门化的混合元素检索器处理UI组件的极端宽高比和密度；(3)模式引导的生成范式连接布局和代码。

Result: 在HiFi2Code基准测试中，DOne在高层视觉相似性（如GPT Score提升超过10%）和细粒度元素对齐方面均优于现有方法。人工评估显示生产力提升3倍且视觉保真度更高。

Conclusion: 通过解耦结构理解和元素渲染，DOne有效解决了Design-to-Code生成中的"整体瓶颈"问题，在复杂布局场景下实现了显著性能提升。
...
Method: 利用模型注意力模式估计与查询相关的视觉区域和检索到的文本元素，然后基于这些突出显示的证据生成答案。通过轻量级的提示级标记突出显示所选线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的视觉问答基准测试中，相比零样本多模态大语言模型取得了持续改进。在仅视觉中心和幻觉导向的基准测试中，仅视觉证据突出显示就能提升模型性能，且无需额外训练或架构修改。

Conclusion: Look Twice 框架通过推理时重新关注相关多模态证据，有效提升了预训练多模态大语言模型在知识密集型查询中的表现，证明了注意力引导机制在改善模型证据利用方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在医学影像领域缺乏鲁棒性，存在跨数据集干扰和对数据制度的敏感性。在临床工作流中，数据和任务按顺序到达，传统的持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，结合频谱路由的专家混合架构和理论驱动的缩放规则。从预训练权重的非重叠奇异值分解片段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。

Result: 在23个医学数据集上（包括视觉问答、报告生成、放射学分类和幻觉缓解）表现优异：在零样本分类任务上接近全微调性能，但仅使用1/339的可训练参数；将顺序遗忘降低至约5%，而强基线模型性能下降20-50%。

Conclusion: MedQwen通过参数高效的专家混合架构和理论驱动的缩放规则，有效解决了医学视觉语言模型中的跨数据集干扰和持续学习灾难性遗忘问题，为临床工作流提供了可靠且高效的解决方案。
...
TL;DR: 基于视觉Transformer的模型在扩展数据集上检测美式足球危险擒抱动作，通过不平衡感知训练将危险召回率提升8%以上


<details>
  <summary>Details</summary>
Motivation: 早期识别接触性运动中的危险动作能够及时干预并提高运动员安全性，需要开发可靠的方法来检测美式足球训练视频中的危险擒抱动作

Method: 使用视觉Transformer模型结合不平衡感知训练，在733个单运动员-假人擒抱视频片段的数据集上进行检测，每个片段都标注了标准化擒抱技术评估(SATT-3)的撞击区域组件

Result: 在交叉验证中获得危险召回率0.67和危险F1分数0.59，相比之前较小数据集上的基线(危险召回率0.58，危险F1 0.56)，危险召回率提升了超过8个百分点
...
TL;DR: 使用合成蹦床姿态数据集(STP)微调ViTPose模型，显著提升极端姿态下的姿态估计精度


<details>
  <summary>Details</summary>
Motivation: 现有姿态估计模型在蹦床体操等极端姿态和非常规视角下表现不佳，需要专门的数据集来提升性能

Method: 从运动捕捉数据生成合成蹦床姿态数据集(STP)，通过参数化人体模型拟合噪声数据，生成多视角真实图像，用此数据微调ViTPose模型

Result: 在2D姿态估计上达到SOTA，显著缩小了常规姿态与极端姿态间的性能差距；3D三角测量中MPJPE降低12.5mm，相比预训练ViTPose模型提升19.6%

Conclusion: 合成数据微调能有效提升姿态估计模型在极端姿态下的性能，为特定领域姿态估计提供了有效解决方案
...
TL;DR: 提出Attention Regularization方法，通过bootstrap重采样技术量化注意力分数的不确定性，去除噪声产生的虚假注意力，提高注意力图的稀疏性和可解释性。
...
Method: 提出基于bootstrap的注意力正则化方法：通过重采样输入特征生成注意力分数的基线分布，利用该分布估计注意力分数的显著性和后验概率，去除噪声产生的虚假注意力。

Result: 在自然图像和医学图像上，该方法能有效去除噪声引起的虚假注意力，显著提高注意力图的收缩性和稀疏性。通过模拟和真实数据集进行了定量评估。

Conclusion: bootstrap可作为实用的正则化工具，用于提高Vision Transformer注意力分数作为解释的可信度。该方法为注意力机制提供了统计显著性检验框架。
...
TL;DR: 研究发现CNN模型作为视觉系统模型的评估指标（Brain-Score）与其纹理感知能力之间没有相关性，表明纹理感知机制与基于物体识别训练的CNN模型所建模的机制不同。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究CNN作为视觉系统模型时，其纹理表示（基于Gram矩阵的特征相关性）是否与人类纹理感知内容对齐，特别是那些被认为是更好视觉系统模型的CNN是否具有更接近人类的纹理表示。

Method: 方法包括：1）计算不同CNN模型中非线性特征的线性相关性（Gram矩阵）作为纹理表示；2）比较这些纹理表示与人类纹理感知内容的对齐程度；3）使用Brain-Score指标评估CNN模型作为哺乳动物视觉系统模型的质量；4）分析模型质量指标与纹理感知对齐度之间的相关性。

Result: 令人惊讶的结果是：CNN模型作为视觉系统模型的传统评估指标（Brain-Score）与其纹理感知对齐度之间没有关联。更好的视觉系统模型并不一定具有更接近人类的纹理表示。

Conclusion: 结论是纹理感知涉及与基于物体识别训练的CNN模型通常建模的机制不同的机制，可能依赖于上下文信息的整合。这表明当前CNN模型在纹理感知方面存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在局限性：缺乏对工具在组织表面交互位置的预测能力，缺少显式条件输入来强制执行工具-动作特定的安全交互区域

Method: 结合时间视觉编码器（捕捉多视角下的工具运动和组织动态）、语言条件（实现跨不同仪器-动作对的泛化）和DiT风格解码器进行密集可操作性预测

Result: 在15,638个视频片段（覆盖103个胆囊切除术）的基准测试中，相比视觉语言模型基线（Molmo-VLM的60.2 px ASSD），取得了显著改进（20.6 px ASSD），任务特定架构优于大规模基础模型

Conclusion: AffordTissue通过预测工具-动作特定的组织可操作性区域，为安全手术自动化提供显式空间推理，可能解锁对适当组织区域的显式策略指导，并在工具偏离预测安全区域时实现早期安全停止
...
TL;DR: GRAZE是一个无需训练的管道，用于在美式足球训练视频中定位首次接触点（FPOC），通过Grounding DINO发现候选交互、运动感知时间推理和SAM2像素级验证来实现。
...
Method: GRAZE采用无训练管道：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时间推理进行精炼；3) 使用SAM2作为显式像素级接触验证器，不依赖检测置信度。候选发现与接触确认分离使方法对杂乱场景和冲击附近不稳定接地具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的开放词汇3D场景理解方法存在两个关键局限：1) 非结构化、重叠的高斯分布导致空间模糊性，需要概率性特征注册；2) 基于物体级掩码的特征池化导致多级语义模糊性，稀释细粒度细节。
...
Conclusion: 稀疏体素光栅化提供了一种有效的结构化几何表示，能够解决3D高斯泼溅方法中的空间和语义模糊性问题，通过确定性特征注册和利用基础模型的密集对齐特性，显著提升了开放词汇3D场景理解的性能，特别是在细粒度任务上。
...
<details>
  <summary>Details</summary>
Motivation: 从第一人称视频理解和预测物体运动对具身感知和交互至关重要，但现有生成模型缺乏显式物理推理，难以生成物理一致的6DoF轨迹，主要挑战包括遮挡、快速运动和物理合理性

Method: 提出EgoFlow流匹配框架，采用混合Mamba-Transformer-Perceiver架构联合建模时序动态、场景几何和语义意图，通过梯度引导推理过程强制执行可微物理约束（如碰撞避免和运动平滑性）

Result: 在HD-EPIC、EgoExo4D和HOT3D数据集上优于基于扩散和Transformer的基线方法，在准确性、泛化性和物理真实性方面表现更好，碰撞率降低达79%，对未见场景有强泛化能力

Conclusion: 流匹配生成模型为可扩展且物理基础的第一人称运动理解提供了有前景的解决方案，结合混合架构和梯度引导物理约束能生成连贯可控的运动，无需后处理过滤或额外监督
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性的基本矛盾：前馈重建效率高且输入对齐好，但缺乏全局先验导致结构不完整；扩散生成能提供丰富几何细节，但多视图一致性差。需要整合两种范式优势。

Method: 提出UniRecGen统一框架，通过共享规范空间对齐两种模型。采用解耦协同学习，保持训练稳定性并在推理时实现无缝协作。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在稀疏观测下创建完整且一致的3D模型方面，实现了卓越的保真度和鲁棒性，优于现有方法。

Conclusion: 通过整合前馈重建和扩散生成两种范式，UniRecGen成功解决了稀疏视图3D建模中重建保真度与生成合理性的矛盾，为创建完整且一致的3D模型提供了有效解决方案。
...
TL;DR: 提出TAG热成像框架解决材料非均匀性导致的鬼影效应，实现高保真夜视成像


<details>
  <summary>Details</summary>
Motivation: 热成像中的鬼影效应导致纹理细节丢失，现有HADAR技术仅适用于均匀材料场景，无法处理现实世界中普遍存在的材料非均匀性问题

Method: 提出TAG（热反鬼影）通用计算热成像框架，利用高光谱光子流进行非参数纹理恢复，解决材料非均匀性问题

Result: TAG在多种场景下普遍优于HADAR，首次实现鬼影人脸的表情恢复，展示热3D拓扑对齐和情绪检测能力
...
Result: 在两个无人机数据集上验证：UAVid达到71.86% mIoU，UDD6达到80.92% mIoU，在保持计算效率的同时实现竞争性性能。

Conclusion: PBSeg为无人机图像语义分割提供了一种高效解决方案，通过原型交叉注意力和多尺度特征提取，在计算效率和分割质量之间取得良好平衡。
...
Method: 1. 完整规范空间构建模块：增强静态和动态区域的初始化；2. 基于分离的动态场景建模模块：解耦静态和动态组件以实现针对性运动监督；3. 自校正流匹配机制：包括全流匹配（对齐3D场景流与时间变化的2D观测）和相机流匹配（强制静态对象的多视角一致性）。
...
TL;DR: VideoZeroBench：一个用于长视频问答的分层基准测试，通过验证时空证据来评估视频多模态大语言模型的细粒度视觉理解和推理能力，揭示当前模型在基于证据的推理方面存在显著差距。
...
Method: 提出VideoZeroBench，一个用于挑战性长视频问答的分层基准测试。包含500个手动标注的问题，覆盖13个领域，每个问题都配有作为证据的时间区间和空间边界框。引入五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro正确回答的问题也不到17%。当施加定位约束时，性能急剧下降：在要求正确答案和精确时空定位的情况下（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。这揭示了表面答案正确性与真实基于证据的推理之间存在显著差距。

Conclusion: 基于证据的视频理解仍然是长视频QA的瓶颈。VideoZeroBench通过分层评估协议暴露了当前模型在细粒度视觉理解和时空推理方面的严重不足，为未来基于证据的视频推理研究提供了重要见解和评估框架。
...
Method: 采用交替单模态学习和共享分类器解耦多模态梯度，设计基于不确定性的跨模态梯度手术选择性对齐跨模态梯度，使共享参数对所有模态都有利
...
TL;DR: 提出卫星图像无关训练框架SFT，通过无人机多视角序列重建3D场景，生成伪正射影像，实现无卫星数据训练的跨视图地理定位


<details>
  <summary>Details</summary>
Motivation: 现有无人机地理定位方法依赖卫星图像进行训练（配对监督或无监督对齐），但在卫星数据不可用或受限时难以部署。需要开发无需卫星图像的训练框架

Method: 三阶段框架：1) 使用3D高斯泼溅从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像，并用轻量级几何引导修复完善纹理；3) 从生成的正射影像提取DINOv3特征，仅用无人机数据学习Fisher向量聚合模型，测试时复用该模型编码卫星图块进行跨视图检索

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星图像无关的泛化基线方法，缩小了与使用卫星图像训练方法的性能差距

Conclusion: 提出的卫星图像无关训练框架通过无人机端3D重建和伪正射影像生成，实现了无需卫星数据的地理定位模型训练，为卫星数据受限场景提供了实用解决方案
...
Method: SHOE将每个HOI预测分解为动词和对象组件，使用多个大型语言模型（LLMs）的平均值估计它们的语义相似度，然后将它们组合成相似度分数，实现超越精确字符串匹配的对齐评估

Result: SHOE分数比现有指标（包括基于LLM和嵌入的基线）更接近人类判断，与平均人类评分的协议达到85.73%，在HICO-DET等标准基准上实现了灵活可扩展的评估

Conclusion: 需要基于语义的HOI评估来更好地反映人类对交互的理解，SHOE框架为开放词汇HOI检测提供了更合理的评估方法，将公开发布以促进未来研究
...
TL;DR: ROSETTA是一种鲁棒的开集测试时适应方法，通过角损失和特征范数损失来改善OOD检测，同时保持ID分类性能，解决了熵最小化和最大化之间的内在冲突。
...
Method: 提出ROSETTA方法：1) 分析熵最大化在OSTTA中的局限性；2) 引入角损失来调节特征范数大小；3) 使用特征范数损失来抑制csOOD logits，从而改善OOD检测。
...
Conclusion: ROSETTA通过角损失和特征范数损失有效解决了开集测试时适应中熵最小化和最大化之间的冲突，在保持ID分类性能的同时显著提升了OOD检测能力，适用于多种实际场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS流水线假设集中访问所有观测数据，这在分布式机器人设置中不适用，因为智能体独立运行且集中数据聚合可能受限。直接将集中训练扩展到多智能体系统会引入通信开销和几何不一致问题。
...
Conclusion: Tex3D是首个在VLA仿真环境中端到端优化3D对抗纹理的框架，成功实现了物理可行的对抗攻击。研究暴露了VLA系统对3D对抗攻击的严重脆弱性，强调了鲁棒性感知训练的必要性
...
TL;DR: 使用稀疏自编码器从基础模型特征中提取单语义神经元，构建自动化形态特征标注流水线，创建大规模昆虫形态特征数据集


<details>
  <summary>Details</summary>
Motivation: 形态特征是生物与环境相互作用的重要指标，但传统人工标注方法效率低下，限制了大规模生态研究。主要瓶颈在于缺乏连接生物图像与特征标注的高质量数据集。

Method: 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元；构建特征标注流水线，定位显著区域并使用视觉语言提示生成可解释的特征描述；创建Bioscan-Traits数据集（19K昆虫图像的80K特征标注）。

Result: 人类评估确认生成的形态描述具有生物学合理性；通过全面消融研究评估设计敏感性，系统变化关键设计选择并测量其对特征描述质量的影响；构建了可扩展的自动化标注系统。

Conclusion: 通过模块化流水线而非昂贵的人工标注，为大规模形态分析提供了可扩展方法，为基础模型注入生物学意义监督，弥合生态相关性与机器学习实用性之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景生成方法主要关注静态几何重建，忽略了云、水、烟雾等场景尺度的环境动态建模。这些动态建模的挑战在于运动需要在扩展场景中保持全局一致性，同时支持低延迟用户交互。

Method: 采用渐进式构建全局一致运动场的方法：1) 引入几何感知对齐模块解决视图间的方向和尺度模糊问题；2) 使用基于哈希的紧凑运动场表示，支持高效查询和稳定动态传播；3) 支持渲染时的双向运动传播，无需依赖昂贵的视频细化。

Result: 在单张RTX 5090 GPU上，每个新场景扩展步骤生成需要9秒，运动对齐和运动场更新需要3秒，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架成功解决了从单张图像生成具有全局一致环境动态的4D世界的挑战，通过渐进式运动场构建和紧凑表示实现了交互式生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但在现实场景中，由于3D标注成本高昂，通常只有部分目标被标注。这种稀疏标注设置导致现有方法性能下降，需要解决稀疏监督下的鲁棒检测问题。
...
Conclusion: 通过结合几何保持增强和原型引导的伪标签生成，提出的框架在稀疏监督下单目3D目标检测中表现出色，为解决现实场景中标注成本高昂的问题提供了有效方案。
...
TL;DR: 提出基于莫尔效应的物理认证签名，利用真实相机自然产生但生成模型无法忠实复现的物理特性来区分真实视频与AI生成视频


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分，需要开发能够可靠区分两者的认证方法

Method: 利用莫尔效应，推导莫尔运动不变量，证明条纹相位与光栅图像位移通过光学几何线性耦合，不受观看距离和光栅结构影响。验证器从视频中提取信号并测试相关性

Result: 在真实拍摄视频和多个最先进AI生成视频上验证，发现真实视频与AI生成视频产生显著不同的相关签名，表明该方法能有效区分两者

Conclusion: 确定性光学现象可作为物理基础的可验证签名，用于对抗AI生成视频，提供了一种基于物理的鲁棒认证方法
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在处理高度动态运动或需要精细运动控制时表现不佳，主要原因是常用训练数据集中缺乏此类示例

Method: 提出DynaVid框架，利用计算机图形学管道渲染的光流作为合成运动数据，采用两阶段生成：运动生成器先合成运动，然后运动引导的视频生成器基于该运动生成视频帧

Result: 在剧烈人体运动生成和极端相机运动控制两个挑战性场景中，DynaVid显著提升了动态运动生成的真实性和运动控制的可控性

Conclusion: 通过合成运动数据（光流）与解耦的两阶段生成框架，DynaVid能够从合成数据学习动态运动模式，同时保持真实世界视频的视觉真实性，有效解决了动态运动生成中的真实性和可控性问题
...
Result: 在多个标准基准数据集上的广泛实验表明，该方法在完全无需样本和域标识的设置下显著减少了灾难性遗忘，其准确率优于现有最先进方法

Conclusion: 提出的框架有效解决了具身多媒体系统中动态环境分布漂移问题，通过解耦表示和权重融合实现了鲁棒的连续环境适应，为开放物理空间中的具身感知系统提供了实用的增量学习解决方案
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，通过转移低频光谱分量来增强rPPG模型对跨域外观变化的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记术(rPPG)在跨域部署时性能显著下降，现有深度学习方法容易过拟合到光照、相机特性等外观相关因素，需要提高模型对跨域外观变化的鲁棒性

Method: 提出频率域适应(FDA)策略，通过转移低频光谱分量来编码域相关外观特征；进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性指导原始表示与FDA转移表示之间的对齐

Result: 广泛的跨数据集实验表明，FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力

Conclusion: FDA和HOT为rPPG领域提供了一种原则性的域适应策略，能够学习对外观变化的不变性，同时保留心脏诱导信号，解决了rPPG实际部署中的域偏移问题
...
Method: 1) 利用时间对齐的实例掩码和多模态大语言模型的句子嵌入，通过两个MLP解码器监督高斯点的可学习语义特征；2) 将2D光流与4D高斯连接并微调其运动，提高时间稳定性；3) 引入几何感知的SDF约束和表面连续性正则化项，增强动态前景建模的时间一致性。

Result: 实验表明Director能够实现时间一致的4D重建，同时支持实例分割和开放词汇查询，在动态场景理解和语义推理方面表现出色。

Conclusion: 嵌入实例一致性语义自然补充了4D建模，实现了更准确的场景分解，同时支持鲁棒的动态场景理解，为体积视频的语义感知表示提供了有效解决方案。
...
Method: 提出双视角理解方法：1）外部：通过蒸馏方案将视觉基础模型中对象与其引发效果的关系迁移到视频扩散模型；2）内部：提出帧间上下文交叉注意力机制，将每个去噪块锚定在目标区域周围信息丰富的未掩码上下文中。
...
TL;DR: 提出一种通过群体检测实现端到端共享注意力估计的方法，同时完成群体检测和共享注意力估计


<details>
  <summary>Details</summary>
Motivation: 现有方法要么在估计共享注意力时不检测实际关注该点的群体，要么假设图像中只有一个共享注意力点，这些限制影响了实际应用和性能
...
Conclusion: 提出的端到端方法能够同时进行群体检测和共享注意力估计，解决了现有方法的局限性，提高了实际应用性能
...
<details>
  <summary>Details</summary>
Motivation: 现有基于流的生成模型在文本引导图像编辑中存在源图像保真度不足的问题：高阶求解器需要额外推理开销，截断反演限制可编辑性，特征注入方法缺乏架构可移植性。
...
Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的实验表明，SteerFlow相比现有方法能获得更好的编辑质量，且能自然扩展到多轮编辑范式而不积累漂移。
...
TL;DR: 该论文提出DPMO和RPS框架，从点标注生成密集人群实例分割，并通过掩码监督提升人群计数性能


<details>
  <summary>Details</summary>
Motivation: 人群实例分割在监控和交通等领域有广泛应用，但现有数据集通常只有点标注而缺乏准确区域标注。直接应用SAM等基础模型在密集人群场景效果不佳，需要专门方法从点标注生成掩码并实现准确分割。
...
Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上实现了最先进的人群实例分割性能。掩码监督的损失函数显著提升了不同模型的计数准确率。

Conclusion: 提出的DPMO和RPS框架有效解决了密集人群实例分割问题，证明了掩码标注在提升人群计数精度中的重要作用，为从点标注生成高质量掩码提供了有效方法。
...
TL;DR: 提出几何感知的无人机跨视角地理定位框架，通过3D场景重建和虚拟鸟瞰图渲染统一粗粒度位置识别和细粒度姿态估计，在GNSS拒止环境下实现米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 解决无人机在GNSS拒止环境下的跨视角地理定位挑战，特别是倾斜无人机图像与正交卫星地图之间的严重几何差异问题。现有方法通常采用解耦的检索-姿态估计流程，将透视畸变视为外观噪声而非显式几何变换，限制了定位精度。

Method: 提出几何感知的无人机地理定位框架：1) 使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景；2) 渲染虚拟鸟瞰图表示，将无人机透视正交校正以对齐卫星图像；3) 引入卫星注意力块，隔离每个卫星候选与重建无人机场景的交互，防止候选间干扰；4) 发布重新校准的University-1652数据集，包含精确坐标标注和空间重叠分析。

Result: 在重新校准的University-1652基准和SUES-200数据集上的广泛实验表明，该方法显著优于现有最先进基线，实现了鲁棒的米级定位精度，并在复杂城市环境中表现出更好的泛化能力。

Conclusion: 通过显式建模3D场景几何，将粗粒度位置识别和细粒度姿态估计统一到单个推理流程中，有效解决了无人机跨视角地理定位中的几何差异问题，为GNSS拒止环境下的无人机定位提供了有效解决方案。
...
TL;DR: 该研究构建了US-365K超声图像-文本数据集，提出Ultrasound-CLIP框架，通过语义感知对比学习提升超声图像理解能力，在分类和检索任务上达到SOTA性能。
...
Method: 1. 构建US-365K大规模超声图像-文本数据集（36.5万配对样本，52个解剖类别）；2. 建立超声诊断分类学（UDT），包含超声分层解剖分类学和超声诊断属性框架；3. 提出Ultrasound-CLIP框架，引入语义软标签和语义损失进行样本细粒度区分；4. 构建基于UDAF文本表示的异构图模态，实现病变-属性关系的结构化推理。

Result: 在患者级数据划分的实验表明，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务上展现出强大的泛化能力。

Conclusion: 该研究通过构建大规模超声数据集和专门设计的预训练框架，成功解决了现有视觉语言模型在超声医学影像应用中的局限性，为超声图像的智能分析提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要用作生成渲染器，通常通过感知、几何或简单语义信号进行条件控制。自监督学习获得的高维特征作为视觉模型的通用接口，但尚未被广泛用作预训练视频扩散模型的通用条件信号。这些特征包含风格、光照和语义的纠缠信息，限制了生成能力。

Method: 引入轻量级架构和训练策略，将外观特征与其他需要保留的特征解耦。通过补偿低空间分辨率与高特征维度，提高从显式空间表示进行生成渲染的可控性。

Result: 实现了视频域迁移和从3D生成视频等任务。解耦方法使得外观变化（如风格化和重光照）的控制更加鲁棒，同时保持了其他特征的完整性。

Conclusion: 自监督学习特征可作为预训练视频扩散模型的通用条件信号，通过适当的架构设计和特征解耦策略，能够实现鲁棒的生成控制，扩展视频模型在域迁移和3D生成等任务中的应用能力。
...
TL;DR: 该论文提出了RebusBench基准测试，用于评估大型视觉语言模型在解决需要多步骤推理的谜题（如字谜）方面的能力，发现当前模型性能严重不足，无法有效整合视觉感知和语言知识。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步骤推理的任务中存在认知鸿沟。当视觉输入仅作为线索而非答案时，模型难以进行抽象推理，无法将视觉属性与语言先验知识（如成语）整合起来解决谜题。

Method: 研究者构建了RebusBench基准测试，包含1,164个字谜谜题，专门测试模型整合感知和知识的能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，分析了它们在精确匹配和语义准确性方面的表现，并考察了模型缩放和上下文学习的影响。

Result: 评估结果显示模型性能严重不足：精确匹配低于10%，语义准确性低于20%。模型缩放和上下文学习均未带来显著改进。这表明虽然模型具备必要的视觉和语言组件，但缺乏将它们连接起来的认知推理能力。

Conclusion: 大型视觉语言模型在需要神经符号推理的任务中存在根本性缺陷。虽然拥有视觉感知和语言知识组件，但缺乏将它们整合以解决抽象推理问题的认知能力。RebusBench为评估这种能力提供了重要基准。
...
Method: 使用大型语言模型处理语言指令、多视角图像和动作，然后通过三个轻量级生成器分别生成深度、未来视频和动作。模型学习几何感知的世界表示，并用其指导统一的未来预测和规划框架
...
TL;DR: FSKD：一种LiDAR到RGB-红外知识蒸馏框架，通过多模态教师融合RGBI图像与LiDAR数据，训练仅使用RGBI图像的SegFormer学生模型来预测森林结构指标，在零样本CHM预测上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR成本高且不频繁，但却是森林结构监测的参考标准。需要开发仅使用更易获取的RGBI图像就能准确预测森林结构指标的方法，以实现可扩展的高分辨率森林监测。

Method: 提出FSKD知识蒸馏框架：多模态教师模型通过交叉注意力融合RGBI图像与LiDAR衍生的平面指标和垂直剖面，训练仅使用RGBI图像的SegFormer学生模型来复现这些输出。在德国萨克森州384平方公里森林数据上训练，使用20厘米地面采样距离数据。

Result: 在8个地理上不同的测试区域上，学生模型在零样本CHM预测上达到SOTA性能：MedAE 4.17米，R²=0.51，IoU 0.87。相比HRCHM/DAC基线，MAE改善29-46%（5.81米 vs 8.14-10.84米），相关系数更强（0.713 vs 0.166-0.652）。多模态融合比仅RGBI训练提升10-26%性能。

Conclusion: FSKD框架能够仅使用RGBI图像准确预测CHM、PAI和FHD等多个森林结构指标，打破了严格的同时采集限制（支持冬季LiDAR与夏季RGBI的时序不匹配），为德国数字孪生和国家数字正射影像计划等应用提供了可扩展的20厘米分辨率操作监测方案。
...
TL;DR: 提出一种基于轮式里程计的一致性感知单目深度估计框架，通过融合光流、三角测量和贝叶斯尺度估计，实现跨帧稳定的深度预测


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间缺乏时间一致性，导致深度估计抖动，且在深度范围突变时容易失败。这限制了其在自动驾驶和移动机器人感知系统中的实际应用。

Method: 1) 利用移动机器人的轮式里程计信息；2) 通过连续帧间的光流进行三角测量，估计相机位姿和稀疏深度；3) 使用稀疏深度更新度量尺度的递归贝叶斯估计；4) 将估计的尺度应用于预训练深度估计基础模型预测的相对深度，进行重新缩放。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，展示了鲁棒且准确的深度估计性能，特别是在保持时间一致性方面表现优异。

Conclusion: 提出的框架通过融合里程计信息和贝叶斯尺度估计，有效解决了单目深度估计中的时间一致性问题，为自动驾驶和移动机器人提供了更稳定的感知能力。
...
TL;DR: STRIVE是一个用于视频问答的时空强化学习框架，通过构建输入视频的多个时空变体并进行跨文本生成和视觉变体的联合归一化，解决了组策略优化中奖励方差低的问题，从而稳定了优势估计并提升了视频推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于组的策略优化方法在大型多模态模型中表现出潜力，但当回答具有相似正确性时，往往面临奖励方差低的问题，导致优势估计弱或不稳定。这限制了强化学习在视频问答任务中的效果。

Method: STRIVE框架包含三个核心组件：1）构建输入视频的多个时空变体；2）在文本生成和视觉变体之间进行联合归一化；3）引入重要性感知采样机制，优先选择与输入问题最相关的帧，同时保持时间覆盖。这种方法将组比较从语言多样性扩展到结构化视觉扰动。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上，STRIVE在多个大型多模态模型中相比强化的强化学习基线都表现出一致的改进。

Conclusion: 结构化时空探索作为一种原则性机制，能够稳定多模态强化学习并提升视频推理性能。STRIVE通过丰富奖励信号和促进更稳定的策略更新，展示了在互补视觉视角上进行稳健推理的价值，而不是过度拟合单一时空配置。
...
TL;DR: SafeRoPE：针对基于transformer的扩散模型（如MMDiT）的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间中，提出基于RoPE扰动的细粒度安全控制方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于U-Net的扩散模型安全缓解方法（如微调或注意力调制）计算开销大，且不适用于基于transformer的扩散模型（如MMDiT）。研究发现不安全语义集中在注意力头的低维子空间中，且扰动RoPE可以有效修改生成图像中的特定概念。

Method: 1. 分析MMDiT注意力机制，发现不安全语义集中在安全关键头的可解释低维子空间中；2. 构建头级不安全子空间，通过投影计算潜在风险分数（LRS）；3. 引入头级RoPE扰动，对查询和键向量嵌入进行风险特定的头级旋转，抑制不安全语义同时保持生成质量。

Result: 实验表明SafeRoPE在平衡有害内容缓解和效用保持方面达到SOTA性能，有效抑制不安全输出同时保持生成保真度，代码已开源。

Conclusion: SafeRoPE为基于transformer的扩散模型提供了轻量级、细粒度的安全生成框架，通过头级RoPE扰动实现精确的不安全语义抑制，无需昂贵微调即可保持生成质量。
...
TL;DR: 本文提出了一种通过桥接训练阶段实现语言预训练模型到视觉任务的跨模态适应方法，发现随机标签桥接训练无需人工标注即可有效对齐LLM参数与视觉任务，且部分桥接训练效果更佳。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为语言预训练模型由于参数空间差异不适合视觉任务，但本文挑战这一假设，旨在探索语言预训练模型参数直接用于视觉任务的可能性，解决跨模态适应的固有挑战。

Method: 提出随机标签桥接训练方法，作为模态适应学习器，无需人工标注即可帮助LLM参数适应视觉基础任务。同时发现部分桥接训练策略更有效，即仅对LLM的特定层进行桥接训练。

Result: 研究表明桥接训练阶段能有效对齐LLM参数与视觉任务，随机标签桥接训练无需手动标注即可实现这一目标。更重要的是，部分桥接训练通常比全模型桥接效果更好，因为LLM某些层具有强大的基础特性，即使未经视觉任务微调也保持有益。

Conclusion: 语言预训练模型参数可直接用于视觉模型，桥接训练是实现跨模态适应的有效途径。部分桥接训练策略为实际应用提供了实用路径，这一发现为利用语言预训练参数进行视觉任务开辟了新方向。
...
Method: 提出基于排序的域自适应方法：1) 跨域排序：利用类别顺序信息对跨域样本对进行排序；2) 连续分布对齐：对齐源域和目标域的秩分数分布
...
TL;DR: HieraVid是一个用于视频大语言模型的分层剪枝框架，通过段级、帧级和层级三个层次逐步动态减少视觉冗余，在保留仅30%令牌的情况下实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型输入令牌数量巨大导致计算负担重，现有方法主要在输入级剪枝而忽略了视频内在的分段-帧结构以及大语言模型内部的多模态信息单向传播特性。

Method: 提出三层剪枝框架：1) 段级剪枝：将视频令牌按时间分段并空间合并；2) 帧级剪枝：在同一段内联合剪枝相似帧以保持多样性；3) 层级剪枝：随着LLM层数增加逐渐减少冗余而不影响性能。

Result: 在四个广泛使用的视频理解基准测试上，仅保留30%令牌的情况下，HieraVid实现了新的SOTA性能，分别保持了LLaVA-Video-7B和LLaVA-OneVision-7B超过98%和99%的性能。

Conclusion: HieraVid通过利用视频的分段-帧结构和LLM内部信息传播特性，实现了高效的分层剪枝，显著减少了计算负担同时保持了视频理解性能。
...
TL;DR: 提出一种纹理感知的Transformer模型，直接从网格面的原始像素学习，结合新的分层学习方案进行多尺度特征聚合，显著提升了纹理3D网格的语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 纹理3D网格同时包含几何、拓扑和外观信息，但其不规则结构给基于深度学习的语义分割带来挑战。现有方法通常直接处理网格而忽略丰富的纹理信息，需要一种能够有效利用纹理数据的方法。
...
Conclusion: 该方法通过有效利用纹理信息并设计专门的Transformer架构，显著提升了纹理3D网格的语义分割性能，在城市场景和文化遗产保护等应用领域具有重要价值。
...
Method: 提出置换不变向量量化自编码器（PI-VQ），约束潜在代码不携带位置信息。引入匹配量化算法，基于最优二分匹配进行向量量化，相比朴素最近邻量化提升3.5倍有效瓶颈容量。利用学习代码的组合结构实现基于插值的采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖度指标。代码能够捕捉全局语义特征，支持无需学习先验的直接图像插值，并实现单次前向传播合成新图像。

Conclusion: 位置无关表示在可分离性和可解释性方面存在权衡，为未来工作指明了方向。PI-VQ展示了消除位置约束可以促进更语义化的表示学习，同时保持生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅在CT重建任务中表现出色，但其优势尚不足以推动从成熟算法过渡。现有GS方法存在显著限制，特别是在速度和可扩展性方面，需要改进以充分发挥其潜力。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流程实现快速重建。改进的体素化能够快速将高斯拟合到现有体积，可作为先验用于热启动重建或作为压缩表示。

Result: FaCT-GS在标准512×512投影上比现有GS CT重建快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能适应不同投影和输出体积大小。

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了快速灵活的重建方案，其速度和可扩展性优势使其成为实际应用的有力候选，同时支持从现有体积快速拟合高斯作为先验。
...
TL;DR: 现代视觉语言模型在基本几何变换下存在系统性脆弱性，缺乏鲁棒的空间不变性和等变性
...
Method: 通过系统评估方法，在符号草图、自然照片和抽象艺术等多样化视觉领域中测试模型对旋转、缩放和恒等变换的鲁棒性
...
Method: 提出双损失训练框架：1) 边界回归损失通过单通道边界预测提升时间定位精度；2) CDF基分段级正则化损失通过匹配预测与真实分段的累积分布来增强段内结构一致性。该框架与架构无关，可集成到现有TAS模型中

Result: 在三个基准数据集上，该方法提升了分段一致性和边界质量，提高了F1和Edit分数，而帧级精度基本保持不变。证明通过简单损失设计而非复杂架构即可实现精确分割

Conclusion: 提出的轻量级双损失框架通过最小架构修改显著提升时序动作分割质量，展示了简单损失设计相对于复杂架构的优势，有利于实际部署
...
Conclusion: MAR-MAER通过结合度量感知正则化和概率潜在建模，有效提升了文本到图像生成的质量一致性和对模糊提示的处理能力，在自动指标和人工评估中都验证了其优越性能。
...
TL;DR: A3R：一种基于智能体交互的跨维度证据获取框架，用于复杂3D高斯场景中的细粒度可供性推理，通过序列化证据采集逐步减少歧义，超越传统一次性预测方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D高斯场景中的可供性推理视为一次性预测问题，假设静态观察已提供足够证据。但在复杂场景中，失败案例往往源于固定观察下任务相关证据的不完整性，而非预测能力不足。

Method: 将细粒度可供性推理重新定义为序列化证据获取过程，提出A3R框架：基于MLLM的策略迭代选择证据获取动作，通过跨维度（3D几何和2D语义）证据采集更新可供性信念，并采用GRPO策略学习方法优化序列决策。

Result: 在场景级基准测试中，A3R持续超越静态一次性基线方法，证明了智能体跨维度证据获取在复杂3D高斯场景细粒度可供性推理中的优势。

Conclusion: 通过将可供性推理重构为序列化证据获取过程，A3R框架有效解决了复杂3D场景中证据不完整的问题，展示了智能体交互式跨维度证据采集对提升推理准确性的重要性。
...
TL;DR: GS²通过图基空间分布优化实现紧凑3D高斯泼溅，在保持高质量渲染的同时显著减少内存消耗
...
Method: 提出GS²框架：1）基于证据下界（ELBO）的自适应致密化策略，自动控制高斯点生成；2）透明度感知的渐进式剪枝策略，动态移除低透明度点；3）图基特征编码模块，通过特征引导的点位移调整空间分布。

Result: 相比原始3DGS，GS²仅需约12.5%的高斯点就能获得更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑高斯表示与高质量渲染的平衡。

Conclusion: GS²通过优化高斯点的空间分布，解决了3DGS内存消耗大的问题，在显著减少高斯点数量的同时保持了甚至提升了渲染质量，为实际应用提供了更实用的解决方案。
...
TL;DR: 该论文展示了现代文生图模型仍易受低成本的提示词越狱攻击，通过自然语言提示即可绕过安全过滤器，无需模型访问或优化。


<details>
  <summary>Details</summary>
Motivation: 文生图模型广泛部署于创意工具和在线平台，依赖安全过滤器和审核管道来阻止有害内容。然而，现有系统在检测对抗性意图方面存在不足，需要研究其实际脆弱性。

Method: 提出系统性的提示词越狱攻击研究，引入视觉越狱技术分类学，包括艺术重构、材质替换、伪教育框架、生活方式美学伪装和模糊动作替换等策略，通过良性语义上下文掩盖不安全意图。

Result: 在多个先进文生图系统上评估这些攻击，证明简单语言修改能可靠规避现有安全措施并生成受限图像。在所有测试模型和攻击类别中，攻击成功率（ASR）最高达74.47%。

Conclusion: 研究揭示了表面提示词过滤与检测生成媒体系统中对抗性意图所需语义理解之间的关键差距，现有安全措施无法有效防御基于自然语言的越狱攻击。
...
TL;DR: ProVG：一种通过解耦语言表达为全局上下文、空间关系和对象属性，并采用渐进式跨模态调制器来提高遥感视觉定位精度的新框架。
...
Method: 1. 将语言表达解耦为全局上下文、空间关系和对象属性；2. 采用渐进式跨模态调制器，通过"调查-定位-验证"方案动态调制视觉注意力，实现从粗到细的视觉-语言对齐；3. 引入跨尺度融合模块处理遥感图像的大尺度变化；4. 使用语言引导校准解码器在预测时细化跨模态对齐；5. 统一的多任务头支持指称表达理解和分割任务。

Result: 在RRSIS-D和RISBench两个基准测试上的广泛实验表明，ProVG始终优于现有方法，实现了新的最先进性能。

Conclusion: ProVG通过解耦语言表达和渐进式跨模态调制，有效利用了细粒度语言线索，显著提高了遥感视觉定位的准确性，为处理遥感图像中的大尺度变化和复杂语言描述提供了有效解决方案。
...
TL;DR: SHARC是一个新颖的框架，通过球谐函数表示距离场来合成任意、属不可知的形状，在重建精度和时间效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在形状合成中难以同时兼顾几何细节的精确学习、计算效率和模型简洁性。SHARC旨在通过优化参考点放置和球谐函数表示来解决这些问题。
...
Conclusion: SHARC通过优化参考点放置和球谐函数表示，实现了高效、准确的任意形状合成，为几何处理提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法要么侧重于帧级增强而缺乏充分的时域建模，要么依赖于繁重的时空聚合而牺牲高频细节，难以同时保持时间稳定性和空间细节。

Method: 将特征表示分解为高频和低频分量进行协同建模：高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；低频分支引入时域扰动策略增强对闪烁、抖动和局部错位等复杂视频变化的鲁棒性；设计偏移感知时间一致性约束以显式稳定时域扰动下的跨帧表示。

Result: 在多个公共基准测试上的广泛实验表明，FTPFusion在空间保真度和时间一致性方面均优于现有最先进方法，在多项指标上表现一致优异。
...
TL;DR: 提出Light-ResKAN模型，通过将ResNet中的卷积替换为KAN卷积，使用Gram多项式作为激活函数，并采用参数共享策略，在保持SAR图像识别精度的同时大幅降低计算复杂度和参数量，适用于资源受限的边缘设备。


<details>
  <summary>Details</summary>
Motivation: SAR图像识别在灾害监测、军事侦察和海洋观测中至关重要，但大尺寸SAR图像阻碍了深度学习在资源受限边缘设备上的部署。现有轻量级模型难以平衡高精度特征提取与低计算需求之间的矛盾。新兴的Kolmogorov-Arnold Network（KAN）通过可学习的激活函数增强了拟合能力，同时减少了参数和计算量。

Method: 1. 将ResNet中的卷积层替换为KAN卷积，实现对SAR图像的自适应特征提取；2. 使用Gram多项式作为激活函数，更适合SAR数据以捕捉复杂的非线性关系；3. 采用参数共享策略：每个卷积核在通道维度上共享参数，在保留独特特征的同时减少参数和FLOPs。

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率。在1024×1024尺寸的MSTAR数据集上，与VGG16相比，FLOPs减少82.90倍，参数量减少163.78倍。
...
TL;DR: 提出了一种基于几何驱动的多光斑检测与匹配管道，将角膜反射光斑视为结构化星座而非独立斑点，通过相似性-布局对齐方法实现稳定识别。


<details>
  <summary>Details</summary>
Motivation: 角膜反射光斑检测在瞳孔-角膜反射眼动追踪中至关重要，但现有方法通常作为启发式算法嵌入大型系统，难以在不同硬件设置下实现可复现性。
...
Conclusion: 该框架解决了多LED眼动追踪中光斑检测的可复现性问题，通过星座匹配方法实现了稳定可靠的光斑识别与匹配。
...
TL;DR: KnowMVG：一种用于医学视觉定位的知识先验和全局-局部注意力增强框架，通过编码医学知识嵌入和联合利用全局与局部视觉线索，显著提升了视觉语言模型在医学图像中的空间定位精度。
...
Result: 在四个MVG基准测试中，KnowMVG一致优于现有方法，在AP50指标上提升3.0%，在mIoU指标上提升2.6%，超越了先前的最先进方法。定性和消融研究进一步验证了各组件有效性。

Conclusion: KnowMVG通过显式增强空间感知能力，成功弥合了高级语义理解和细粒度视觉感知之间的差距，显著提升了医学视觉定位的精度，且不引入额外文本推理开销。
...
TL;DR: 该研究探索了能否直接从波束成形前的单天线距离-多普勒张量中学习有意义的空间结构，而无需构建角度域表示或手工信号处理阶段。


<details>
  <summary>Details</summary>
Motivation: 传统汽车雷达感知流程通常在波束成形构建角度域表示后应用学习模型，本研究旨在探索直接从波束成形前的单天线距离-多普勒测量中学习空间结构的可行性。

Method: 使用6TX×8RX（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列FMCW发射方案，其中有效发射孔径在啁啾间变化。使用双啁啾共享权重编码器对波束成形前的单天线RD张量进行端到端数据驱动训练，以BEV占用率作为几何探针评估空间可恢复性，监督信号来自LiDAR并考虑雷达视场和遮挡感知。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束成形前的单天线RD张量中学习空间结构，无需显式角度域构建或手工信号处理阶段。

Conclusion: 空间结构可以直接从波束成形前的单天线距离-多普勒张量中学习，无需构建角度域表示或依赖手工信号处理阶段，为汽车雷达感知提供了新的表征方法。
...
TL;DR: 提出ECAC大规模早教活动图像描述数据集和RSRS混合训练框架，开发KinderMM-Cap-3B模型，显著提升专业物体识别准确率


<details>
  <summary>Details</summary>
Motivation: 现有早教图像描述方法面临两大挑战：缺乏大规模领域特定数据集导致描述泛化不精确；传统训练范式在提升专业物体描述能力方面存在局限，监督学习偏向高频表达，强化学习在困难样本上优化不稳定

Method: 1) 构建ECAC数据集：包含256,121张真实世界图像，配备专家级描述和细粒度标签；2) 提出TTS评估指标：专门衡量专业物体命名准确性；3) 设计RSRS混合训练框架：动态切换强化学习和监督微调，将零奖励困难样本重定向到监督学习；4) 开发KinderMM-Cap-3B：领域适应的多模态大语言模型

Result: KinderMM-Cap-3B模型在TTS指标上达到51.06分，显著优于现有基线方法，同时保持优越的描述质量，展示了在专业教育应用中的潜力

Conclusion: ECAC数据集和RSRS训练框架有效解决了早教图像描述中的领域适应问题，提出的模型在专业物体识别和描述质量方面均表现出色，为专门化教育应用提供了有力工具
...
TL;DR: 该论文提出AMIMV方法解决医学影像分类中的数据稀缺和类别不平衡问题，评估了8种自监督学习方法在11个医学影像数据集上的表现，在多个数据集上取得了性能提升。


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临两大挑战：1) 缺乏大量标注训练数据；2) 数据不平衡问题（常见类别数据丰富，罕见类别数据稀缺）。自监督学习（SSL）方法在一定程度上解决了第一个问题，但SSL在医学图像分类中对不平衡数据的鲁棒性研究较少。
...
Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。AMIMV方法在多个医学影像数据集上表现出对不平衡数据的良好鲁棒性。

Conclusion: AMIMV方法能有效应对医学影像分类中的数据稀缺和类别不平衡问题，在多个医学影像数据集上取得了显著性能提升，为医学影像分析中的不平衡学习提供了有效解决方案。
...
TL;DR: MAVFusion：一种基于运动感知稀疏交互机制的红外与可见光视频融合框架，通过光学流识别动态区域，对动态区域使用计算密集的跨模态注意力，对静态区域使用轻量级弱交互模块，在保持融合质量的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有红外与可见光视频融合方法大多针对静态图像设计，无法有效处理视频中的帧间运动。当前视频融合方法虽然通过引入跨帧交互改善了时间一致性，但计算成本高昂。需要一种既能保持融合质量又能高效处理视频的方法。

Method: 提出MAVFusion端到端视频融合框架，采用运动感知稀疏交互机制：1）利用光学流识别多模态序列中的动态区域；2）对动态区域自适应分配计算密集的跨模态注意力，捕捉显著过渡并促进模态间信息交换；3）对静态背景区域使用轻量级弱交互模块，保持结构和外观完整性；4）通过解耦动态和静态区域处理，同时保持时间一致性和细粒度细节。

Result: 在多个红外与可见光视频基准测试中达到最先进性能，在640×480分辨率下实现14.16 FPS的推理速度，显著加速推理过程。

Conclusion: MAVFusion通过运动感知稀疏交互机制，在保持高质量融合结果的同时显著提升计算效率，为红外与可见光视频融合提供了有效的解决方案，源代码将在GitHub上公开。
...
Method: 使用nnU-Net v2框架构建深度学习模型，整合多模态mpMRI数据（T2加权成像、扩散加权成像DWI、表观扩散系数ADC图），在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证评估性能。

Result: 交叉验证中平均Dice得分为0.96±0.00，在Hospital La Fe的54例独立外部测试集上Dice得分为0.82，表现出良好的泛化能力。相比之下，通用分割工具TotalSegmentator仅获得0.15的Dice分数，主要由于腺体欠分割。
...
TL;DR: MyEgo是首个评估多模态大语言模型在自我中心视频中理解、记忆和推理相机佩戴者能力的自我中心视频问答数据集，揭示了当前模型在自我接地和长期记忆方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要理解相机佩戴者（自我接地）的个性化问答任务中表现不佳，缺乏专门评估模型理解、记忆和推理"我"的能力的数据集，阻碍了自我中心个性化助手的发展。

Method: 提出MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类问题。系统评估了开源与专有、思考与非思考、不同规模等多种MLLM变体，并分析显式推理和模型缩放的影响。

Result: 顶级闭源模型（如GPT-5）准确率仅约46%，开源模型（如Qwen3-VL）约36%，分别落后人类表现近40%和50%。显式推理和模型缩放未带来一致改进，提供相关证据虽能提升性能但增益随时间下降，表明模型在跟踪和记忆"我"及"我的过去"方面存在局限。

Conclusion: 自我接地和长期记忆对于自我中心视频中的个性化问答至关重要，当前MLLMs在这方面存在显著不足。MyEgo数据集和分析将推动自我中心个性化助手领域的发展。
...
TL;DR: SDesc3D：基于短文本描述的3D室内场景生成框架，通过多视角结构先验和区域功能感知提升稀疏文本指导下的3D布局推理能力


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的3D场景生成方法在短文本条件下存在物理合理性差和细节丰富度不足的问题，主要原因是过度依赖显式的组合对象语义关系。需要增强3D推理能力，特别是在先验整合和空间锚定方面。

Method: 提出SDesc3D框架：1）多视角场景先验增强：通过聚合多视角结构知识丰富欠指定的文本输入；2）功能感知布局锚定：利用区域功能锚定进行隐式空间锚定，并进行分层布局推理；3）迭代反思-修正方案：通过自修正进行渐进式结构合理性优化。

Result: 大量实验表明，该方法在短文本条件下的3D室内场景生成任务上优于现有方法。

Conclusion: SDesc3D通过整合多视角结构先验和区域功能含义，有效解决了短文本条件下3D场景生成的物理合理性和细节丰富度问题，为稀疏文本指导下的3D布局推理提供了有效解决方案。
...
TL;DR: 提出交互式跟踪新范式，构建首个大规模基准InteractTrack，评估现有跟踪器在交互场景中的不足，并提出基于动态记忆的IMAT基线方法


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要采用"发射后不管"的非交互模式，无法适应需要人机协同的现实场景，因此需要建立支持自然语言指令交互的跟踪系统

Method: 1) 创建InteractTrack基准：包含150个视频、密集边界框标注和时间戳语言指令；2) 设计评估协议并测试25个代表性跟踪器；3) 提出IMAT方法：采用动态记忆机制学习用户反馈并更新跟踪行为

Result: 评估显示最先进的跟踪器在交互场景中表现失败，传统基准的强性能无法迁移；IMAT作为新基线为交互式跟踪提供了可行方案

Conclusion: 交互式跟踪新范式填补了自动感知与人类指导之间的空白，为开发更智能、自适应、协作的跟踪系统奠定了基础
...
<details>
  <summary>Details</summary>
Motivation: 医学影像快速增长导致放射科医生工作量不可持续，需要基础模型来减轻负担。现有基础模型在CT和MRI分析方面仍有优化空间，特别是如何从复杂的放射学体积数据中学习

Method: 基于Curia框架，改进预训练策略和表示质量，更好地捕捉放射学数据的特性。将架构扩展到十亿参数的Vision Transformers，首次实现多模态CT和MRI基础模型的规模化。将CuriaBench扩展为两个独立评估轨道：针对切片视觉模型的2D轨道和针对体积基准测试的3D轨道

Result: Curia-2在视觉任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型表现相当。模型权重将公开以促进进一步研究

Conclusion: Curia-2通过优化的预训练策略和十亿参数架构，显著提升了医学影像基础模型的性能，为CT和MRI分析提供了更强大的工具，并通过标准化评估框架推动领域发展
...
TL;DR: 该论文发现多模态大语言模型中的视觉注意力存在惯性现象，导致认知推理能力不足，并提出了一种无需训练的惯性感知视觉激发方法来解决这一问题。
...
Method: 提出无需训练的惯性感知视觉激发方法，通过建模认知推理为视觉注意力的动态响应性来打破惯性模式。具体包括：选择相对于历史注意力趋势动态出现的视觉标记，区分表现出惯性行为的标记；引入惯性感知惩罚，防止注意力过度集中和局部区域的持续关注。

Result: 大量实验表明，IVE方法在各种基础MLLM和多个幻觉基准测试中均有效，特别是在认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，提出的IVE方法能够有效打破这种惯性模式，显著提升模型在组合推理和认知幻觉缓解方面的性能。
...
Conclusion: Resonance4D通过创新的双域运动监督和全参数物理恢复方法，解决了现有4D动态模拟框架的计算效率和物理真实性问题，为消费级硬件上的高质量物理驱动4D模拟提供了可行方案
...
TL;DR: MTLSI-Net通过线性注意力机制解决多任务密集预测中全局跨任务交互的二次复杂度问题，在保持线性复杂度的同时实现高效跨任务交互。


<details>
  <summary>Details</summary>
Motivation: 多任务密集预测需要同时处理多个像素级任务，但标准自注意力机制在高分辨率特征上具有二次复杂度，难以有效捕获全局跨任务交互。

Method: 提出MTLSI-Net，包含三个核心组件：1) 多任务多尺度查询线性融合块，利用共享全局上下文矩阵以线性复杂度捕获多尺度跨任务依赖；2) 语义令牌蒸馏器，将冗余特征压缩为紧凑语义令牌，蒸馏关键跨任务知识；3) 跨窗口集成注意力块，通过双分支架构将全局语义注入局部特征，保持全局一致性和空间精度。

Result: 在NYUDv2和PASCAL-Context数据集上的大量实验表明，MTLSI-Net实现了最先进的性能，验证了其在多任务学习中的有效性和效率。

Conclusion: MTLSI-Net通过线性注意力机制有效解决了多任务密集预测中的跨任务交互问题，在保持线性复杂度和减少参数的同时实现了优异的性能。
...
TL;DR: ProDiG：一种渐进式扩散引导框架，通过中间高度视图合成和几何感知因果注意力，实现从航拍图像到地面级逼真渲染的渐进式3D重建


<details>
  <summary>Details</summary>
Motivation: 现有方法在从航拍图像生成地面级视图和连贯3D场景模型时面临挑战：极端视角变化、缺失中间观测和大尺度变化导致几何不一致；现有方法要么后处理渲染效果不佳，要么依赖难以获取的多高度真实数据；高斯泼溅和扩散细化在小变化下有效，但在大视角差距下失效

Method: 提出ProDiG框架：1）渐进式高度高斯泼溅，逐步将航拍3D表示转化为地面级逼真度；2）合成中间高度视图，每阶段使用几何感知因果注意力模块将极线结构注入参考视图扩散；3）距离自适应高斯模块根据相机距离动态调整高斯尺度和不透明度，确保大视角差距下的稳定重建

Result: 在合成和真实数据集上的广泛实验表明，ProDiG能生成视觉逼真的地面级渲染和连贯3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法

Conclusion: ProDiG通过渐进式扩散引导框架，在不依赖额外地面真实视角的情况下，实现了从航拍到地面级视图的几何一致、视觉逼真的3D重建，解决了大视角差距下的重建难题
...
TL;DR: DR-Seg提出了一种解耦与校正框架，用于遥感领域的开放词汇语义分割，通过将CLIP特征解耦为语义主导和结构主导子空间，实现有针对性的结构增强而不破坏语言对齐的语义完整性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，在有效描绘边界的同时可能破坏CLIP的语义完整性。CLIP的全局对齐视觉表示难以捕捉结构细节，而遥感分割需要语言对齐识别和细粒度空间描绘。

Method: DR-Seg将CLIP特征解耦为语义主导和结构主导子空间，使DINO能够进行有针对性的结构增强而不扭曲语言对齐语义。随后，先验驱动的图校正模块在DINO指导下注入高保真结构先验形成细化分支，不确定性引导的自适应融合模块动态集成细化分支与原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: 通过解耦CLIP特征的功能异质性并针对性增强结构信息，DR-Seg有效解决了遥感开放词汇语义分割中语言对齐识别与细粒度空间描绘的平衡问题。
...
TL;DR: LinkS²Bench：首个评估视觉语言模型在无人机-卫星协同空间智能方面的基准，包含17.9k高质量QA对，覆盖12个细粒度任务，揭示现有模型在跨视图动态对齐上的显著差距


<details>
  <summary>Details</summary>
Motivation: 无人机与卫星的协同空间智能对应急响应和安全操作至关重要，但现有基准局限于孤立的无人机视频或静态卫星图像，无法评估动态局部到全局的空间映射能力，视觉语言模型在这方面的能力尚未被充分探索

Method: 构建LinkS²Bench基准，将1022分钟动态无人机视频与覆盖200+ km²的高分辨率卫星图像关联，通过LMM辅助流水线和严格人工标注创建17.9k高质量QA对，包含感知、定位、关系、推理四个维度的12个细粒度任务

Result: 评估18个代表性VLM显示与人类基线存在显著差距，准确跨视图动态对齐是关键瓶颈；设计的跨视图对齐适配器显著提升模型性能；微调实验证明LinkS²Bench在推进VLM复杂空间推理适应方面的潜力

Conclusion: LinkS²Bench填补了评估VLM在无人机-卫星协同空间智能方面的空白，揭示了跨视图动态对齐的关键挑战，提出的对齐适配器有效缓解了这一问题，为推进VLM在复杂空间推理任务中的应用提供了重要基准
...
<details>
  <summary>Details</summary>
Motivation: 自编码器在处理空间非均匀采样的图像时面临挑战，特别是在医学成像、生物学和物理学等领域，信息丰富的模式在特定坐标上出现频率低，而背景占据主导地位，导致模型偏向多数模式，丢失细粒度细节，对罕见空间输入产生模糊重建。

Method: 提出两个互补组件：(1) 基于自熵的损失函数，对统计上不常见的空间位置进行加权；(2) 样本传播机制，一种重放机制，在训练过程中选择性地将难以重建的样本重新暴露给模型。

Result: 在具有受控空间不平衡条件的模拟数据集以及三个不受控的真实世界数据集（涵盖物理、生物和天文领域）上验证，该方法在各种重建指标上优于基线，特别是在空间不平衡分布下表现更佳。
...
TL;DR: IndoorCrowd是一个多场景室内人群数据集，包含31个视频（9,913帧），用于人体检测、实例分割和多目标跟踪，在四个校园地点收集，包含人工验证的实例分割掩码和基准测试。
...
Method: 在四个校园地点（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频（9,913帧，5fps），包含人工验证的实例分割掩码。使用620帧控制子集对三个基础模型自动标注器（SAM3、GroundingSAM、EfficientGroundingSAM）进行基准测试，使用Cohen's κ、AP、精确率、召回率和掩码IoU指标。2,552帧子集支持多目标跟踪，采用MOTChallenge格式。建立检测、分割和跟踪基线，使用YOLOv8n、YOLOv26n、RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT组合。

Result: 场景分析显示难度差异显著，受人群密度、尺度和遮挡影响：ACS-EC场景最具挑战性，包含79.3%的密集帧，平均实例尺度为60.8像素。基准测试提供了基础模型自动标注器的性能评估，并为检测、分割和跟踪任务建立了性能基线。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，提供了人工验证的标注和基准测试，支持计算机视觉任务的研究和开发，特别是在拥挤室内环境下的应用。
...
TL;DR: WISE提出了一种高效的多模态推理范式，通过"思考两次"原则（一次学习，一次加速），在训练时生成结构化序列（简洁推理→答案→详细解释），推理时仅输出简洁推理，显著减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维推理方法在语言引导分割任务中虽然性能显著，但生成冗长推理过程导致计算成本过高，限制了实际应用。需要一种既能保持高性能又能大幅降低推理成本的方法。

Method: WISE训练模型生成结构化序列：简洁推理→最终答案→详细解释。通过将简洁推理置于首位，利用自回归条件迫使简洁推理成为生成详细解释的充分摘要。采用自蒸馏目标联合奖励语义保真度和简洁性，使模型将详细推理内化为紧凑形式。推理时省略详细解释，使用WISE-S策略通过简洁性指令提示激活学习到的简洁策略。

Result: 在ReasonSeg基准测试中，WISE-S实现了58.3 cIoU的零样本最先进性能，同时将平均推理长度从112个token减少到仅23个token，减少了近5倍。
...
<details>
  <summary>Details</summary>
Motivation: 构建高质量的多语言和非英语视觉语言模型面临的主要障碍是其他语言的视觉问答数据集在规模和领域覆盖上都有限。英语可以通过聚合现有VQA资源构建大规模训练数据集，但这一策略不适用于其他语言。

Method: 收集异构源数据（图像、图像-文本对、PDF文档），通过多种策略生成VQA对：基于VLM的QA生成、翻译和文本渲染。构建了包含约920万个实例的Jagle数据集。

Result: 使用Jagle训练的2.2B模型在日语任务上表现优异，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。将Jagle与FineVision结合不仅不会降低英语性能，反而比单独使用FineVision训练有更好的英语表现。

Conclusion: Jagle数据集有效解决了日语VQA数据稀缺问题，为构建高质量日语视觉语言模型提供了重要资源。该数据集、训练模型和代码已公开发布，促进可复现性和未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数虚拟人创建流程依赖未经过色彩校准的摄影输入，这会导致肤色再现不一致和偏差，影响真实感、身份保持和公平性。需要一种系统性的方法来评估肤色保真度。

Method: 提出完整工作流程：1) 肤色和光照提取（基于脸颊区域采样和全脸分析的多维掩码）；2) 使用预训练TRUST框架进行光照隔离；3) 将提取的肤色应用于MetaHuman纹理；4) 多种光照配置下实时渲染；5) 在CIELAB色彩空间中使用ΔE度量和个体类型学角度(ITA)进行客观评估。

Result: 生成并分析了约19,848个渲染实例，结果显示提取策略表现出表型依赖性行为，深色肤色的色度误差始终较高。

Conclusion: 该方法无需人工干预，计算成本低，支持大规模评估，揭示了肤色提取策略的表型依赖性和深色肤色再现的系统性偏差，为改进虚拟人渲染的肤色保真度提供了量化框架。
...
TL;DR: 论文提出多物体可供性定位新任务，解决在混淆物体对场景中基于隐式意图指令选择正确物体的问题，构建首个基准数据集CompassAD并提出CompassNet框架。


<details>
  <summary>Details</summary>
Motivation: 现实场景中多个物体可能具有相同的可供性功能，但只有特定物体适合特定任务上下文（如"切苹果"时选择刀而非剪刀）。现有3D可供性方法主要评估孤立单物体且依赖显式类别名称，无法处理这种混淆物体对场景。

Method: 提出CompassNet框架：1) 实例边界交叉注入(ICI)模块，将语言-几何对齐约束在物体边界内，防止跨物体语义泄漏；2) 双层对比细化(BCR)模块，在几何组和点级别强制执行区分，增强目标与混淆表面间的差异。

Result: 构建首个基准数据集CompassAD，包含30个混淆物体对、16种可供性类型、6,422个场景和88K+查询-答案对。实验显示在可见和未见查询上均达到最先进性能，在机器人操作器上的部署证实了在真实混淆多物体场景中的有效转移。

Conclusion: 该工作形式化了意图驱动的多物体可供性定位新任务，解决了混淆物体对场景中的物体选择问题，提出的CompassNet框架通过ICI和BCR模块有效处理语言-几何对齐和区分学习，为机器人理解隐式意图提供了重要进展。
...
TL;DR: 提出Instance-centric Context Mining Network (InCoM-Net)，通过整合视觉语言模型的语义先验和物体检测器的实例特征，挖掘场景中的多尺度上下文线索，提升人-物交互检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互检测方法虽然利用视觉语言模型引入语义先验，但未能充分利用分布在整个场景中的多样化上下文线索，限制了交互推理的深度和准确性。

Method: 提出InCoM-Net框架，包含两个核心组件：1) Instance-centric Context Refinement (ICR)：从VLM提取的特征中分别挖掘实例内、实例间和全局上下文线索；2) Progressive Context Aggregation (ProCA)：迭代融合这些多上下文特征与检测器生成的实例级特征，支持高级HOI推理。

Result: 在HICO-DET和V-COCO基准测试上进行了广泛实验，InCoM-Net实现了最先进的性能，超越了之前的人-物交互检测方法。

Conclusion: InCoM-Net通过有效整合VLM的语义知识和检测器的实例特征，并挖掘多尺度上下文线索，显著提升了人-物交互检测的性能，证明了上下文建模在HOI检测中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有显式思维链方法存在两个主要问题：1) 产生大量推理开销；2) 将丰富的多模态证据压缩到狭窄的文本瓶颈中。需要一种既能保持中间推理优势又能避免显式理性生成开销的方法。
...
Result: 在78个任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，同时将推理从数百个生成token减少到少于10个潜在步骤，推理速度提升30倍以上。特别适用于证据密集、结构复杂且难以通过显式中间理性组织的检索场景。

Conclusion: 结构化潜在计算可以在不产生显式理性生成开销的情况下保持中间推理的优势，为实际检索系统提供了更强大、更高效的范式。
...
TL;DR: GroundVTS：一种用于视频时间定位的Vid-LLM架构，通过查询引导的细粒度视觉令牌采样来保留关键时空信息，避免均匀帧采样导致的信息丢失


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（Vid-LLMs）依赖均匀帧采样提取视频信息，导致关键帧分布稀疏且丢失重要时间线索，限制了视频时间定位任务的性能

Method: 提出Grounded Visual Token Sampling（GroundVTS）架构：1）使用细粒度、查询引导的机制在将视觉令牌输入LLM前进行过滤；2）引入渐进优化策略使LLM适应视觉特征的非均匀分布

Result: 在三个标准VTG基准测试中超越现有方法：时刻检索任务mIoU提升7.7个百分点，亮点检测任务mAP提升12.0个百分点

Conclusion: GroundVTS通过聚焦信息最丰富的时序片段，有效保留了关键时空信息并保持时间连贯性，显著提升了视频大语言模型在时间定位任务上的性能
...
TL;DR: LatentUM提出了一种新型统一模型，将所有模态表示在共享语义潜在空间中，消除了视觉理解与生成之间像素空间中介的需求，实现了灵活的交错跨模态推理与生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为桥梁，因为它们在理解和生成方面使用分离的视觉表示，这既低效又无效。需要一种更有效的跨模态推理和生成方法，特别是在需要密集视觉思维、通过自我反思改进视觉生成或建模物理世界视觉动态的场景中。

Method: LatentUM将所有模态表示在共享语义潜在空间中，消除了像素空间中介的需求。这种设计自然支持灵活的交错跨模态推理和生成，通过共享表示减轻编解码器偏差并增强跨模态对齐。

Result: LatentUM在视觉空间规划基准测试中实现了最先进的性能，通过自我反思突破了视觉生成的极限，并支持在共享语义潜在空间内预测未来视觉状态的世界建模。

Conclusion: LatentUM通过共享语义潜在空间表示所有模态，提供了一种更有效和高效的方法来实现交错跨模态推理和生成，解决了现有统一模型中像素解码桥接的低效问题。
...
TL;DR: 提出CoRegOVCD框架，通过一致性正则化解决开放词汇变化检测中训练无关设置下的噪声和语义不可靠问题


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法假设固定标签空间，无法响应任意用户查询。开放词汇变化检测(OVCD)虽然能查询任意概念，但在完全训练无关设置下，由于外观变化、弱跨概念竞争和空间连续性等因素，直接比较密集概念响应会产生噪声、碎片化和语义不可靠的变化证据。
...
Result: 在四个基准测试中，CoRegOVCD相比之前最强的训练无关基线在F1_C指标上提升2.24到4.98个百分点，在SECOND数据集上达到六类平均47.50%的F1_C。

Conclusion: CoRegOVCD通过将概念特定变化重新定义为校准后验差异，并引入一致性正则化机制，有效解决了开放词汇变化检测中训练无关设置下的挑战，显著提高了变化检测的准确性和可靠性。
...
TL;DR: 交叉验证本身引入可测量的随机方差，在BP4D+数据集上3折交叉验证产生±0.065的F1分数噪声，留一数据集交叉验证提供更稳定评估


<details>
  <summary>Details</summary>
Motivation: 面部动作单元检测研究中，标准的主体排他交叉验证协议报告的小幅改进可能落在协议方差范围内，需要更可靠的评估方法

Method: 1) 在BP4D+数据集上进行重复3折主体排他分割，量化交叉验证的随机方差；2) 使用留一数据集交叉验证协议在五个AU数据集上评估跨数据集鲁棒性

Result: 3折交叉验证产生±0.065的平均F1分数经验噪声，低流行度AU方差更大；留一数据集交叉验证消除了分区随机性，揭示了单数据集交叉验证中不可见的领域级不稳定性

Conclusion: 交叉验证本身引入显著方差，传统交叉验证报告的改进可能落在协议方差范围内；留一数据集交叉验证提供更稳定和可解释的评估结果
...
Conclusion: 该研究成功解决了反射生成这一未充分探索的问题，通过创新的模型设计和数据集构建，实现了高质量的反射合成效果，为图像合成领域提供了重要的技术进展。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共同骨干，但理解其工作原理仍具挑战性，特别是在视觉领域。现有可解释性工具通常关注孤立组件或专家导向分析，缺乏对完整推理流程的引导式端到端理解。

Method: 开发ViT-Explainer系统，结合动画演示、分块级注意力叠加和视觉适配的Logit Lens，提供引导式和自由探索两种模式，可视化ViT从图像分块到分类的完整推理过程。

Result: 6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了Vision Transformer端到端可解释性工具的空白，通过集成可视化降低了理解复杂度，为模型解释提供了实用工具。
...
TL;DR: UniDriveVLA通过专家解耦的Mixture-of-Transformers架构解决自动驾驶VLA模型中空间感知与语义推理的冲突，在nuScenes和Bench2Drive上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶VLA模型面临空间感知与语义推理的困境：直接采用2D VLM空间感知有限，增强3D表示又会损害VLM的原生推理能力。这种冲突源于空间感知和语义推理在共享模型参数中的耦合优化。

Method: 提出UniDriveVLA统一驾驶VLA模型，基于Mixture-of-Transformers通过专家解耦解决感知-推理冲突。包含三个专家：驾驶理解、场景感知和动作规划，通过掩码联合注意力协调。结合稀疏感知范式和三阶段渐进训练策略，在保持语义推理能力的同时提升空间感知。

Result: 在nuScenes开环评估和Bench2Drive闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛任务上表现强劲，展示了作为自动驾驶统一模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构成功解决了自动驾驶VLA模型中的感知-推理冲突，实现了空间感知和语义推理的协同优化，为自动驾驶提供了一个统一且高性能的VLA模型框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，当动作名称无法充分描述细粒度动态特征且未见类别语义混淆时，这种方法较为脆弱。需要更鲁棒的框架来处理语义不确定性。

Method: 提出SCALE框架：1) 构建文本条件变分自编码器，冻结文本表示参数化先验和解码器；2) 引入语义和置信度感知的列表能量损失，强调语义相似的困难负样本，结合后验不确定性调整决策边界；3) 使用潜在原型对比目标，将后验均值与文本衍生的潜在原型对齐，改善语义组织和类别可分性。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE持续优于基于VAE和对齐的基线方法，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过将ZSAR建模为类条件能量排序问题，结合语义感知的列表能量损失和潜在原型对比，有效解决了语义混淆问题，提供了一种轻量且确定性的零样本骨架动作识别框架。
...
Method: 提出SPAR：单次任意分辨率ViT，通过特征回归损失将细粒度滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中。该方法不要求架构修改或像素级监督，实现了分辨率无关的密集特征提取。
...
Method: 利用视觉-语言基础模型作为现成的监督信号，通过梯度反馈引导生成过程。在每个采样步骤中，通过干净潜在估计注入反馈，将安全引导公式化为基于能量的采样问题。该方法兼容扩散模型和流匹配模型。

Result: 在NSFW红队测试基准上实现了最先进的鲁棒性，能够有效进行多目标引导，同时在良性非目标提示上保持高生成质量。框架展示了良好的泛化能力。
...
TL;DR: Omni123是一个3D原生基础模型，通过统一的自动回归框架将文本到2D和文本到3D生成结合起来，利用2D图像作为几何先验来改善3D表示，解决了3D数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在文本和图像理解与生成方面表现出色，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺。现有方法通常依赖间接的2D编辑和优化提升到3D的流程，牺牲了几何一致性。

Method: 提出Omni123模型，将文本、图像和3D表示为共享序列空间中的离散标记，采用交错的X-to-X训练范式，在异构配对数据集上协调多种跨模态任务，通过语义-视觉-几何循环（如文本到图像到3D到图像）在自动回归序列中联合强制执行语义对齐、外观保真度和多视角几何一致性。

Result: 实验表明，Omni123显著改善了文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: Omni123通过统一的自动回归框架，利用丰富的2D数据作为几何先验来约束3D表示，解决了3D数据稀缺的问题，为构建多模态3D世界模型提供了有前景的方向。
...
TL;DR: 提出一种基于概率度量和切片Wasserstein距离的快速表面配准方法，使用AdamFlow优化算法实现高效鲁棒的医学图像表面配准
...
Method: 将表面网格表示为概率度量，将表面配准建模为分布优化问题；使用计算复杂度为对数线性的切片Wasserstein距离测量网格间差异；提出AdamFlow优化方法，将Adam优化从欧几里得空间推广到概率空间以最小化切片Wasserstein距离。

Result: 理论上分析了AdamFlow的渐近收敛性；实证表明该方法在各种解剖结构的仿射和非刚性表面配准中均表现出优越性能。

Conclusion: 提出的基于概率度量和AdamFlow优化的表面配准方法在效率和鲁棒性方面取得了良好平衡，为医学图像解剖形状分析提供了有效的解决方案。
...
TL;DR: VOID：一个视频物体移除框架，专注于处理物体移除后需要改变下游物理交互的复杂场景，通过视觉语言模型识别受影响区域并引导视频扩散模型生成物理一致的替代结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频物体移除方法擅长处理物体"背后"的内容修复和外观层面的伪影（如阴影、反射），但当移除的物体与其他物体有更显著的交互（如碰撞）时，当前模型无法修正这些交互，产生不真实的结果。

Method: 1. 使用Kubric和HUMOTO生成新的配对数据集，包含反事实物体移除场景；2. 推理时，视觉语言模型识别场景中受移除物体影响的区域；3. 这些区域用于引导视频扩散模型生成物理一致的反事实结果。

Result: 在合成和真实数据上的实验表明，相比先前的视频物体移除方法，该方法在物体移除后能更好地保持一致的场景动态。

Conclusion: VOID框架展示了如何通过高级因果推理使视频编辑模型成为更好的世界模拟器，为处理复杂物理交互的物体移除提供了有效解决方案。
...
TL;DR: 论文提出场景式视觉定位新任务和RSC基准，包含段落级描述查询，需基于角色、意图和关系推理而非字面匹配，并开发了ScenGround课程学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型可通过匹配显著命名类别获得成功。作者探索更具挑战性的场景式视觉定位，目标需从角色、意图和关系上下文推断而非显式命名。

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度文本描述对象角色、用户目标和上下文线索，标注可解释难度标签。提出ScenGround课程推理方法，结合监督预热和难度感知强化学习。

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外样本。实验表明场景式查询暴露了当前模型在标准基准中未揭示的系统性失败，课程训练提升了困难子集性能并可迁移到标准基准。

Conclusion: 场景式视觉定位是现有基准的补充和更具挑战性的设置，RSC基准支持细粒度分析，ScenGround方法为这一新设置提供了参考点，展示了课程训练的有效性。
...
TL;DR: 提出可引导视觉表示（Steerable Visual Representations），通过早期融合文本到视觉编码器中，实现用自然语言引导全局和局部特征，解决现有视觉表示只能关注最显著视觉线索的问题。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）虽然能提供通用图像特征，但只能关注图像中最显著的视觉线索，无法引导到用户感兴趣的不那么突出的概念。而多模态LLM虽然可以通过文本提示引导，但其表示偏向语言中心，在通用视觉任务上效果不佳。

Method: 提出可引导视觉表示，通过轻量级交叉注意力将文本直接注入视觉编码器的各层（早期融合），而不是像CLIP等模型在编码后进行融合（晚期融合）。这种方法允许用自然语言引导视觉特征的关注点。

Result: 开发了衡量表示可引导性的基准，证明该方法可以引导视觉特征关注图像中任何所需对象，同时保持底层表示质量。在异常检测和个性化对象识别任务上达到或超过专用方法，并展示了对分布外任务的零样本泛化能力。

Conclusion: 提出了一种新的可引导视觉表示类别，通过早期文本融合实现了自然语言对视觉特征的精确引导，在保持通用视觉表示质量的同时，提供了对特定概念的定向关注能力，优于现有视觉表示和多模态LLM方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理各个视角，缺乏对多视角和多模态数据的有效整合，需要一种能够同时处理模态和视角间关系的框架

Method: 采用跨模态特征映射范式，学习跨模态和跨视角的特征映射，通过特征调制显式建模视角依赖关系；引入跨视角训练策略利用所有可能的视角组合；训练并公开专门针对工业数据集的基础深度编码器

Result: 在SiM3D基准测试中达到最先进性能，大幅超越先前方法
...
TL;DR: 提出一个从AAA游戏采集的大规模动态数据集，包含400万连续帧的RGB和G-buffer数据，用于提升生成式逆渲染和正向渲染的真实感和时序一致性，并引入基于VLM的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实感和时序一致性方面存在局限，导致生成式逆渲染和正向渲染在真实世界场景中存在领域差距。需要更具真实感、多样性和时序连贯性的数据来推动双向渲染技术的发展。

Method: 1. 采用新颖的双屏拼接采集方法，从视觉复杂的AAA游戏中提取400万连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道数据；2. 提出基于视觉语言模型（VLM）的评估协议，从语义、空间和时序三个维度评估逆渲染性能；3. 开发配套工具包，支持基于G-buffer和文本提示的AAA游戏风格编辑。

Result: 1. 数据集显著提升了逆渲染模型的跨数据集泛化能力和可控生成质量；2. VLM评估结果与人类判断高度相关；3. 正向渲染器结合工具包实现了基于G-buffer和文本提示的AAA游戏风格编辑功能。

Conclusion: 该大规模动态数据集有效缩小了合成数据与真实世界之间的领域差距，推动了双向渲染技术的发展。基于VLM的评估协议为无真实值情况下的逆渲染性能评估提供了可靠方法，工具包则为游戏内容编辑提供了实用解决方案。
...
Method: 引入主体状态标记作为潜在变量，持续捕获场景中每个主体的状态；通过空间偏置机制联合建模状态标记和视频潜在表示，将全局视频帧渲染与个体动作控制的主体更新解耦。

Result: 在Melting Pot基准测试中评估，展示了首个能够同时在46个多样化环境中控制多达7个玩家的视频世界模型；结果显示在动作跟随准确性和身份一致性方面有显著改进，同时能够通过复杂交互实现鲁棒的自回归主体跟踪。

Conclusion: ActionParty成功解决了多主体视频扩散模型中的动作绑定问题，实现了对多个主体的同时控制，为生成式视频游戏提供了有效的世界模型框架。
...
TL;DR: 该论文研究了在线交流中常见的重复延长形式（RLF）在情感分析中的重要性，创建了首个RLF多领域数据集Lengthening，并提出了ExpInstruct框架来提升LLM对RLF的理解和可解释性。
...
Method: 1）基于先前语言学研究，创建了首个专注于RLF情感分析的多领域数据集Lengthening（85万个样本）；2）提出了ExpInstruct（可解释指令微调）框架，这是一个两阶段指令微调框架，旨在提升LLM对RLF的性能和可解释性；3）提出了一种统一的方法来量化语言模型对非正式表达的理解能力。

Result: 1）RLF句子是富有表现力的表达方式，可以作为文档级情感的特征标识；2）RLF对在线内容分析具有潜在价值；3）微调的预训练语言模型在性能上可以超越零样本GPT-4，但在解释性方面不如GPT-4；4）ExpInstruct框架能够用有限样本提升开源LLM，使其在RLF任务上达到与零样本GPT-4相当的性能和可解释性水平。

Conclusion: RLF在情感分析中具有重要性，语言模型能够理解RLF但需要专门优化。提出的ExpInstruct框架有效提升了LLM对RLF的理解能力和可解释性，为在线非正式表达分析提供了新的工具和方法。
...
TL;DR: 通过强化学习和并行思考两种方法扩展推理token预算，用于竞争性编程任务。训练时使用RL优化推理长度，测试时采用多轮并行思考管道，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 竞争性编程需要大量推理token，但单次生成的完整注意力机制成本高昂。需要探索更高效的训练和推理方法来扩展token预算，提升模型在复杂编程问题上的表现。

Method: 1. 训练时：使用强化学习优化推理token生成，发现验证准确率与生成推理token数呈对数线性关系；采用验证RL预热和随机剪枝优化训练轨迹。2. 测试时：引入多轮并行思考管道，将token预算分配到多个线程和轮次中，包括生成、验证和精炼阶段；端到端训练模型以匹配测试时结构。

Result: 从Seed-OSS-36B模型开始，完整系统（16线程，每线程16轮）在AetherCode的456个困难竞争性编程问题上，平均每个问题使用760万token，以pass@1匹配基础RL模型的oracle pass@16性能，并超越GPT-5-high。

Conclusion: 结合训练时强化学习和测试时并行思考管道能有效扩展推理token预算，显著提升模型在复杂编程任务上的性能。该方法为大规模语言模型在需要大量推理的应用中提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏足够的规模、领域多样性和视觉复杂性，无法真实评估科学主张与多模态证据之间的严格一致性，需要构建更全面的评估数据集。

Method: 从PubMed和arXiv收集数据，构建包含超过469K实例的M2-Verify数据集，涵盖16个领域，并通过专家审核进行严格验证，进行广泛的基线实验评估模型性能。

Result: 最先进模型在保持一致性方面表现不佳：在低复杂度医学扰动上达到85.8% Micro-F1，但在解剖结构变化等高复杂度挑战上性能降至61.6%，专家评估还发现模型在生成科学解释时存在幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型中人类偏好的学习仍然具有根本性挑战，因为奖励建模依赖于微妙、主观的比较而非明确的标签。当前方法在标准成对偏好设置下性能有限（ROC AUC低于0.74），需要更好地捕捉人类判断的多维特性。

Method: 使用Anthropic HHRLHF数据集，在标准成对偏好设置下评估十个多样化的大型语言模型。提出特征增强框架，通过向文本表示中添加可解释信号来丰富表示：响应长度、拒绝指标、毒性分数和提示-响应语义相似度，使模型能够明确捕捉帮助性、安全性和相关性等关键方面。采用SHAP和LIME进行细粒度可解释性分析。

Result: 混合方法在所有模型中均带来一致改进，最高达到0.84 ROC AUC和显著更高的成对准确率，其中DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化安全性和支持性框架而非孤立关键词。偏差放大分析表明，虽然单个特征具有弱边际效应，但它们的交互作用影响偏好学习。

Conclusion: 特征增强框架有效提升了语言模型中人类偏好学习的性能，通过整合可解释信号更好地捕捉了人类判断的多维特性。该方法不仅提高了准确性，还提供了对模型决策过程的深入理解，揭示了情境化安全性和支持性框架的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法将每个问题孤立处理，未能系统性地重用先前推理轨迹中的程序性知识（如问题重构、方法选择和验证回溯）。需要更有效地利用这些知识来提升推理能力。

Method: 1) 从逐步推理轨迹语料库中分解出自包含的子问题-子程序对，构建包含3200万条程序性知识条目的数据存储；2) 推理时通过轻量级思维内提示让模型表达核心子问题，在推理轨迹中检索相关子程序，并在多样检索子程序作为隐式程序先验下进行推理。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于使用文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线。在更高推理预算下，相比无检索方法提升达19.2%，相比最强计算匹配基线提升7.9%。

Conclusion: 性能提升主要来自两个关键因素：源轨迹的广泛程序覆盖性，以及分解和检索设计，二者共同实现了程序性知识的有效提取和重用。
...
TL;DR: 该研究发现语言模型通过早期层的稀疏MLP神经元实现实体检索，这些神经元具有因果作用，能通过单神经元干预恢复实体一致预测，支持规范化解释。


<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答许多以实体为中心的事实性问题，但其内部机制尚不清楚。本研究旨在探究语言模型在实体检索过程中的内部工作机制，特别是识别哪些神经元参与实体信息的存储和检索。

Method: 使用模板化提示定位实体选择性MLP神经元，在PopQA数据集上验证这些神经元，通过因果干预（负向消融和受控注入）测试神经元功能，分析200个精选实体的神经元分布和特性。

Result: 定位到的神经元主要集中在早期层；负向消融导致实体特异性遗忘，而受控注入能改善答案检索；许多实体仅激活单个神经元即可恢复实体一致预测；神经元对别名、缩写、拼写错误和多语言形式具有鲁棒性；效果显著但不普遍，流行实体覆盖率更高。

Conclusion: 研究识别了稀疏、具有因果作用的访问点，可用于分析和调节基于实体的事实行为，支持紧凑实体检索而非纯粹深度逐渐丰富的机制，为理解语言模型的实体知识表示提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被用来区分自动化翻译过程和需要认知努力的翻译过程。快速打字被认为反映无挑战的自动化翻译，而较长停顿则表明翻译问题或困难。然而，如何确定区分这两种过程的停顿阈值一直存在争议。

Method: 本文比较了三种最近提出的计算停顿阈值的方法，并提出了一种新的计算生产单元中断的方法。研究基于对停顿阈值确定方法的长期讨论，对现有方法进行了对比分析。

Result: 研究对三种现有方法进行了比较，并评估了新提出的生产单元中断计算方法。具体结果未在摘要中详细说明。

Conclusion: 本文通过比较现有方法并提出新方法，为翻译过程研究中停顿阈值的确定提供了新的视角和方法论工具，有助于更准确地识别翻译过程中的认知努力和问题解决活动。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试数量庞大但冗余，LLM性能可由少量潜在能力因子解释，需要更高效、有原则的基准测试方法。当前缺乏比较不同基准测试方法质量的统一标准，因此提出基于预测效度的评估框架。

Method: 收集WILD数据集（65个模型在163个任务上的109,564个独特项目响应），采用改进的多维项目反应理论模型，结合最优实验设计驱动的自适应项目选择策略，并引入成本感知折扣因子优化评估成本。

Result: 改进IRT模型+自适应选择能在观察仅16个项目后，以小于7%的平均绝对误差预测112个保留基准任务性能。成本感知策略将达到7%MAE所需总token从141,000减少到22,000，降低85%评估成本。

Conclusion: 基于预测效度的基准测试评估框架能显著提高效率，结合改进IRT模型和自适应项目选择策略可在有限预算下准确预测模型在未见任务上的性能，为更高效、有原则的基准测试提供方法论基础。
...
TL;DR: 使用词嵌入和随机森林模型，研究发现印地语中梵语和波斯-阿拉伯语来源的同义词在分布模式上存在可区分的差异，表明上下文编码了词源信号，同义词可能反映与起源相关的系统性区别。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索同义词现象的本质。理论上绝对同义词不应存在，因为它们不会扩展语言的表达能力。但有人提出即使同义词表示相同概念，它们可能反映不同视角或携带不同的文化关联，这些主张很少被定量测试。印地语中梵语和波斯-阿拉伯语借词与其梵语对应词共存，形成了许多同义词对，为研究提供了理想材料。

Method: 研究方法基于分布语义学。使用印地语同义词的词嵌入作为特征，训练随机森林分类器来区分梵语来源和波斯-阿拉伯语来源的词汇。关键创新点是不考虑语义内容，仅基于分布数据（上下文使用模式）进行分类，测试词源信息是否编码在词语的分布模式中。

Result: 随机森林模型成功分类了印地语同义词的词源（梵语或波斯-阿拉伯语），即使这些词在语义上不相关。这表明使用模式保留了词源痕迹，上下文编码了词源信号。同义词可能反映与起源相关的细微但系统性的区别，词源相关的词汇可能形成不同的概念子空间。

Conclusion: 研究提供了定量证据表明上下文编码词源信号，同义词可能反映与起源相关的系统性区别。这支持了同义词可以提供不同视角的观点，词源相关的词汇可能形成由历史起源塑造的新型语义框架。总体而言，结果突显了上下文在捕捉超越传统语义相似性的细微区别方面的强大能力。
...
TL;DR: 研究发现，在归因生成中，过于细粒度的引用（句子级）会损害模型性能，而段落级引用效果最佳，且这种影响随模型规模变化呈非单调性。


<details>
  <summary>Details</summary>
Motivation: 探索引用粒度（句子、段落或文档级）对归因生成模型性能的影响，现有研究多关注细粒度引用便于人工验证，但对其对模型性能的影响了解不足。

Method: 分析四个不同规模的模型（8B-120B），比较不同引用粒度（句子级、段落级、多段落级）下的归因质量，研究粒度选择对模型性能的影响模式。

Result: 细粒度引用使归因质量下降16-276%；段落级引用效果最佳；细粒度约束对大型模型影响更大；引用粒度优化可提升归因质量同时保持或改善答案正确性。

Conclusion: 仅优化人工验证的细粒度引用忽视了模型约束，损害归因忠实性和生成可靠性；有效归因需要使引用粒度与模型自然语义范围对齐。
...
TL;DR: LLMs存在过度自信问题，本文通过电路级机制分析识别了导致过度自信的内部电路，并提出针对性推理时干预方法改善校准


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常表现出过度自信：当产生事实错误答案时，它们倾向于表达过高的置信度而非不确定性。这种言语化的过度自信会误导用户，削弱置信度分数作为可靠不确定性信号的作用，但其内部机制尚不清楚。

Method: 围绕三个轴心进行电路级机制分析：1) 将言语化置信度捕获为可微分的内部信号；2) 识别导致置信度膨胀的因果电路；3) 利用这些洞见进行针对性推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 发现一组紧凑的MLP块和注意力头（集中在中后期层）在最终标记位置持续写入置信度膨胀信号。针对性推理时干预这些电路能显著改善校准。

Conclusion: LLMs中的言语化过度自信由可识别的内部电路驱动，可通过针对性干预缓解。这为理解模型不确定性表达机制和改善校准提供了新途径。
...
TL;DR: 论文提出了Magic, Madness, Heaven, Sin框架，将大语言模型输出变化建模为同质-异质轴，根据任务规范目标评估变化，并分析了四个规范上下文及其交叉影响。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于输出变化的术语碎片化，缺乏明确的任务规范目标框架。需要系统化理解不同任务背景下输出变化的评估标准及其相互影响。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴，识别四个规范上下文（认知性、交互性、社会性、安全性），分析各上下文的失败模式和相关术语，并研究跨上下文交互影响。

Result: 框架揭示了优化单一目标（如安全性）可能无意中损害其他目标（如人口统计代表性或创造性多样性）。不同规范上下文之间存在复杂的权衡关系。

Conclusion: 输出变化应作为由任务目标塑造的属性进行情境感知评估，而非模型的固有特征。需要更细致地理解不同规范目标下的输出变化评估。
...
Method: 通过跨多个概念和提示变体的受控实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态，评估指令遗忘在扩散模型中的效果

Result: 扩散模型在仅依赖自然语言遗忘指令时系统性地无法抑制目标概念，遗忘指令不会导致对目标概念标记的持续注意力减少，目标概念表示在整个生成过程中持续存在
...
TL;DR: 该论文重新审视了网页智能体观察表示的选择问题，发现最优表示取决于模型能力和思考token预算：低能力模型适合紧凑表示（无障碍树），高能力模型则受益于详细HTML表示，且增加思考token会放大HTML的优势。


<details>
  <summary>Details</summary>
Motivation: 先前工作通常将HTML的冗长性视为性能障碍，并采用观察缩减作为标准做法。本文重新审视这一趋势，旨在探索不同模型能力和资源约束下的最优观察表示策略。

Method: 通过系统实验分析不同观察表示（HTML vs 无障碍树）在不同模型能力和思考token预算下的性能表现，进行错误分析以理解模型行为差异，并探索观察历史（包括基于差异的表示）对性能的影响。

Result: 发现两个关键结果：1) 紧凑观察适合低能力模型，详细HTML观察有利于高能力模型，且增加思考token会增强HTML的优势；2) 高能力模型利用HTML中的布局信息进行更好的动作定位，而低能力模型在长输入下更容易产生幻觉。观察历史在大多数模型和设置中都能提升性能，基于差异的表示提供token高效的替代方案。

Conclusion: 提出实用指南：根据模型能力和思考token预算自适应选择观察表示，并使用基于差异的表示来整合观察历史。这些发现为网页智能体设计提供了数据驱动的决策依据。
...
TL;DR: 提出基于模型合并的框架，通过将临床基础模型与通用指令模型合并，解决医学领域微调中的灾难性遗忘问题，实现临床任务性能与指令跟随能力的平衡。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域应用时面临关键挑战：使用特定医学数据集微调会导致模型"遗忘"大量指令跟随能力，限制了通用LLM在临床环境中的有效部署。

Method: 采用模型合并框架，通过插值式合并方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，获得既具备临床专业知识又保留指令跟随能力的领域适应模型。

Result: 在医学基准测试和五个临床生成任务上的综合评估表明，合并模型能有效缓解灾难性遗忘，保持临床领域专业知识，同时保留指令跟随能力。在有限监督下(如64-shot vs 256-shot)达到与完全微调基线相当的性能。

Conclusion: 权重空间合并为开源LLM适应临床应用提供了高度可扩展的解决方案，有助于在资源受限的医疗环境中更广泛地部署，平衡了临床专业知识与通用指令跟随能力。
...
TL;DR: DeltaMem：单智能体记忆管理系统，通过强化学习优化角色中心记忆管理，在多个长期记忆基准测试中超越现有产品级基线


<details>
  <summary>Details</summary>
Motivation: 现有角色中心记忆管理系统多为多智能体框架，存在信息丢失和场景适应性差的问题，导致性能不佳。需要更鲁棒、高效的记忆管理方案。

Method: 提出DeltaMem系统，将角色中心记忆管理建模为单智能体端到端任务。构建用户-助手对话数据集及操作级记忆更新标签，引入基于记忆的Levenshtein距离形式化记忆更新奖励，设计定制强化学习框架优化记忆管理能力。

Result: DeltaMem在LoCoMo、HaluMem和PersonaMem等多个长期记忆基准测试中均超越所有产品级基线，训练免费版和RL训练版均表现出色。

Conclusion: DeltaMem通过单智能体端到端框架和强化学习优化，有效解决了多智能体记忆管理系统的信息丢失和脆弱性问题，在长期记忆管理任务中展现出优越性能。
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，通过增量聚合激活协方差和单次特征值分解，实现无训练、快速且理论最优的层级低秩近似，显著提升LLM压缩效率。
...
Conclusion: Swift-SVD同时保证了理论最优性、实际效率和数值稳定性，为LLM压缩提供了硬件友好的解决方案，显著降低了部署成本。
...
TL;DR: 印尼全国349名K-12教师调查显示：AI在教学、内容开发和教学媒体中的应用增加，但采用不均衡；小学教师使用更一致，高中教师参与较少；教师主要用AI减少教学准备工作量，但通用输出、基础设施限制和情境适配不足阻碍有效课堂整合。
...
Method: 对349名K-12教师（涵盖小学、初中和高中）进行全国性调查，收集关于AI使用模式、感知价值和所需支持的数据。

Result: 发现AI在教学法、内容开发和教学媒体中的应用增加，但采用不均衡：小学教师使用更一致，高中教师参与较少；中年教师更重视AI，东印尼教师感知价值更高；教师主要用AI减少教学准备工作量（评估、课程规划、材料开发），但通用输出、基础设施限制和情境适配不足阻碍有效整合。

Conclusion: 需要开发更情境适配的AI工具，解决基础设施限制，并提供针对性的教师支持，以促进AI在印尼课堂的有效整合。
...
TL;DR: 该研究分析了9个声明验证数据集的推理模式，发现现有基准主要测试证据提取能力，而多句合成和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理。


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对现有基准实际测试推理能力的系统理解。研究者希望揭示这些基准主要测试什么类型的推理，以及不同领域验证系统的错误模式差异。

Method: 使用GPT-4o-mini为9个数据集的24K个声明验证示例生成结构化推理轨迹，分析推理模式分布。使用紧凑的1B参数推理验证器识别五种错误类型，并分析不同领域的错误特征。

Result: 发现直接证据提取占主导地位（约70%），而多句合成和数值推理严重不足（各约5%）。数据集层面存在显著偏差：某些数据集几乎完全测试词汇匹配，而其他数据集约一半案例需要信息合成。错误模式因领域而异：通用领域验证主要受词汇重叠偏差影响，科学验证因过度谨慎而失败，数学验证则因算术推理失败。
...
<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话方法在有效支持深度上下文理解方面面临挑战，需要更好的机制来增强情感推理能力。
...
TL;DR: 研究发现SFT训练中CoT轨迹来源对模型泛化性能有重要影响：DeepSeek-R1数据训练损失更低但泛化更差，gpt-oss-120b数据训练损失更高但泛化更好，揭示了训练损失与泛化性能的悖论。


<details>
  <summary>Details</summary>
Motivation: 研究不同来源的CoT轨迹如何影响大推理模型的泛化性能，特别是当使用不同模型生成的验证CoT轨迹进行SFT时，训练损失与泛化性能之间的关系尚不明确。

Method: 使用DeepSeek-R1-0528和gpt-oss-120b生成的验证CoT轨迹进行对比研究，控制问题集相同。通过多层面分析探究token级SFT损失和步骤级推理行为，揭示推理模式差异，并提出过滤频繁分支轨迹的简单有效方法。

Result: 发现训练损失与泛化性能的悖论：DeepSeek-R1数据训练损失更低但泛化更差，gpt-oss-120b数据训练损失更高但泛化更好。分析显示gpt-oss-120b轨迹收敛且演绎性强，DeepSeek-R1轨迹发散且分支多。过滤频繁分支轨迹后，DeepSeek-R1子集训练使推理性能显著提升：AIME25提升5.1%，BeyondAIME提升5.5%，五个基准平均提升3.6%。

Conclusion: CoT轨迹的推理模式对SFT泛化性能有重要影响，收敛的演绎性推理模式优于发散的分支探索模式。简单的轨迹过滤策略能有效改善模型泛化能力，为高质量SFT数据选择提供了实用指导。
...
TL;DR: EndoASR：一种专为胃肠内窥镜设计的领域自适应语音识别系统，通过两阶段适应策略显著提升转录准确性和临床可用性，在多中心真实世界环境中验证了其鲁棒性。
...
Result: 在回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%。前瞻性多中心研究中，CER从16.20%降至14.97%，Med ACC从61.63%提升至84.16%。实时因子仅0.005，远优于Whisper-large-v3（0.055）。

Conclusion: 领域自适应的ASR可作为胃肠内窥镜中人机协作的可靠接口，在多中心真实临床环境中表现出一致的性能，改进的ASR质量可直接增强下游结构化信息提取和临床医生-AI交互。
...
TL;DR: 该论文对LLM智能体中的记忆方法进行了系统性综述、统一框架构建、实验比较，并提出了一种新的记忆方法


<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长期复杂任务的核心模块，但现有记忆方法缺乏在相同实验设置下的系统性比较

Method: 1) 构建统一框架整合现有记忆方法；2) 在两个知名基准上广泛比较代表性方法；3) 利用现有方法模块设计新的记忆方法

Result: 1) 提供了现有方法的全面分析；2) 提出的新记忆方法优于现有最先进方法；3) 识别了未来研究方向

Conclusion: 对现有记忆方法的深入理解能为未来研究提供有价值的新见解，论文为记忆方法研究提供了系统性框架和实验基准
...
Method: 提出人机协作框架，结合LLM推理和声学特征模型。采用置信度路由机制区分简单和模糊样本，模糊样本交由LLM进行结构化规则指导的深度推理。实施迭代优化策略，通过错误分析和规则更新持续改进系统。

Result: 在包含2,764个样本的越南语语音数据集（三种情感：平静、愤怒、恐慌）上实验，标注一致性高（Fleiss Kappa=0.8574）。方法达到86.59%准确率和0.85-0.86的Macro F1分数，有效处理模糊和难分类样本。
...
TL;DR: 本文提出了一种更细致的保加利亚语毒性内容检测方法，通过构建本体和数据集，训练BERT模型实现分类，同时避免误伤医疗术语和少数群体相关内容。


<details>
  <summary>Details</summary>
Motivation: 当前在线毒性内容检测方案经常误将医疗术语和少数群体相关文本等有价值信息错误地屏蔽，这促使研究者开发一种更细致的方法来识别保加利亚语中的毒性内容，同时保留对重要信息的访问。
...
Conclusion: 本研究提出的方法能够更准确地检测保加利亚语中的毒性内容，同时避免误伤医疗术语和少数群体相关文本等有价值信息，具有在实际在线平台和内容审核系统中应用的潜力。
...
TL;DR: LiveMathematicianBench是一个基于arXiv最新论文的动态多选题基准，用于评估LLMs的研究级数学推理能力，通过逻辑分类、证明草图引导的干扰项设计和抗替换机制来防止数据污染和表面匹配。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准存在合成设置和数据污染的局限性，无法真实评估LLMs的研究级数学能力。随着LLMs越来越多地融入科学工作流，需要建立能够防止记忆偏差、真正测试推理能力的评估框架。

Method: 1) 使用训练截止日期后发布的arXiv论文构建动态基准；2) 建立13类逻辑分类法（蕴含、等价、存在、唯一性等）；3) 采用证明草图引导的干扰项生成管道，基于高层次证明策略构建看似合理但错误的选项；4) 引入抗替换机制区分答案识别与实质推理；5) 设计双模式协议评估证明草图访问对推理的影响。

Result: 基准远未饱和：最佳模型Gemini-3.1-pro-preview仅达43.5%。抗替换评估下准确率大幅下降：GPT-5.4最高30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问能带来一致的准确率提升，表明模型可利用高层次证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的测试平台，用于研究LLMs的研究级数学推理能力，揭示了当前模型在实质性数学理解方面仍存在显著差距，但能够利用高层次证明策略辅助推理。
...
TL;DR: 论文提出基于指令微调和离散控制令牌的领域无关可控自动文本简化框架，发现小模型可达到竞争性性能，但可靠可控性取决于训练数据中目标属性的变化程度，并指出现有评估指标不足以衡量控制效果。


<details>
  <summary>Details</summary>
Motivation: 现有可控自动文本简化方法通常将可控性视为解码问题，并使用不能反映控制效果的指标进行评估。研究发现可控性在ATS中受到数据和评估的显著限制，需要更有效的框架和评估方法。

Method: 提出基于指令微调和离散控制令牌的领域无关CATS框架，使用三个模型家族（Llama、Mistral、Qwen；1-14B）在四个领域（医学、公共管理、新闻、百科全书文本）进行实验，通过采样和分层分析数据分布问题。

Result: 小模型（1-3B）可达到竞争性性能；可读性控制（FKGL、ARI、Dale-Chall）学习一致，但压缩控制因现有语料库中信号变化有限而表现不佳；标准简化和相似性指标不足以衡量控制效果；朴素数据分割会引入分布不匹配问题。

Conclusion: 可靠的可控性取决于训练数据是否编码足够的目标属性变化；需要基于误差的指标来评估目标-输出对齐；数据分割策略对训练和评估有重要影响；领域无关框架在不同领域表现良好。
...
TL;DR: DEFT框架通过数据过滤和分布引导提升LLM对齐效率，在减少训练时间的同时改善对齐能力和泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法（如PPO）成本高且不稳定，替代方法需要大量数据学习偏好且可能削弱LLM的泛化能力，需要更高效的对齐框架

Method: 提出分布引导高效微调（DEFT）框架：基于语言模型输出分布和偏好数据差异分布计算微分分布奖励，从原始数据中过滤出小而高质量的子集，并将其整合到现有对齐方法中以引导模型输出分布

Result: DEFT增强的方法在保持泛化能力的同时，对齐能力优于原始方法，且训练时间显著减少

Conclusion: DEFT通过数据过滤和分布引导有效提升了LLM对齐的效率和性能，同时缓解了泛化能力损失问题
...
TL;DR: PLOT提出基于最优传输的token级损失函数，增强基于微调的对齐方法中的偏好学习，通过将偏好学习建模为最优传输问题，在保持LLM原始分布的同时对齐人类偏好。


<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感、以及对全局token级关系建模不足的问题，需要更有效且理论基础的偏好学习框架。

Method: PLOT通过最优传输问题形式化偏好学习，使用token级损失函数，利用token嵌入捕捉语义关系，实现全局感知的优化，同时保持LLM原始分布以确保稳定性和鲁棒性。

Result: 在两个偏好类别（人类价值观、逻辑与问题解决）的七个子偏好实验中，PLOT一致提升了对齐性能，同时保持了流畅性和连贯性。

Conclusion: 最优传输为偏好学习提供了理论基础的方法论，建立了理论支撑的框架，为LLM偏好学习提供了新的见解。
...
TL;DR: 提出自适应占位符补全(APC)框架，通过在高熵位置输出显式占位符来改进传统硬补全，降低编辑成本同时保持补全质量


<details>
  <summary>Details</summary>
Motivation: 传统硬补全(HC)范式在上下文不足时仍强制生成完全具体的代码，导致大量补全建议被编辑或拒绝。对300万次真实交互的分析显示，61%的生成建议被编辑或拒绝，尽管与用户后续代码有80%以上相似度，表明模型在特定token位置经常做出错误预测

Method: 提出自适应占位符补全(APC)协作框架，在不确定性高的位置输出显式占位符，让用户通过IDE导航直接填充。从过滤的真实编辑日志构建训练数据，设计基于成本的奖励函数进行强化学习。将代码补全形式化为不确定性下的成本最小化问题

Result: 在1.5B-14B参数模型上的广泛评估表明，APC将预期编辑成本降低了19%到50%，同时保持了标准硬补全的性能。理论证明存在临界熵阈值，超过该阈值时APC的预期成本严格低于HC

Conclusion: APC为不确定性感知的代码补全提供了理论基础和实用训练框架，证明自适应弃权可以在不牺牲传统补全质量的情况下端到端学习。该方法通过战略性地输出占位符，实现了模型与用户的协作补全
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于关注融合而忽视了噪声特征的不确定性和细粒度推理，需要更鲁棒且能进行上下文建模的方法
...
<details>
  <summary>Details</summary>
Motivation: 心力衰竭的短期死亡率预测具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要评估不同数据模态和模型架构在预测中的效果

Method: 在法国心力衰竭队列上评估transformer模型，比较纯文本、纯结构化、多模态和LLM方法。包括实体级表示与CLS嵌入的对比，以及监督多模态融合技术

Result: 实体级表示比CLS嵌入能改善预测；监督多模态融合（文本+结构化变量）获得最佳性能；大语言模型在不同模态和解码策略中表现不一致，纯文本提示优于结构化或多模态输入

Conclusion: 实体感知多模态transformer为心力衰竭短期预后预测提供了最可靠的解决方案，而当前LLM提示方法在临床决策支持方面仍有局限
...
<details>
  <summary>Details</summary>
Motivation: 现有基于BERT等语言模型的宣传检测方法容易在训练数据上过拟合，这是由于数据收集过程中的偏差导致的。需要增强分类的鲁棒性并提高对新来源的泛化能力。

Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧等特征，构建混合分类模型。

Result: 结果显示该方法优于等效的纯文本方法，消融研究和可解释性分析证实了添加特征的益处。

Conclusion: 神经符号方法通过结合文本嵌入和符号概念特征，能够提高宣传检测的鲁棒性和泛化能力，为信息失序检测提供了更有效的解决方案。
...
TL;DR: 提出了一种基于排列图（permutohedron）的数学框架，用于衡量词序或手势顺序相对于交换距离最小化的优化程度，并证明跨语言手势至少达到77%的最优性。


<details>
  <summary>Details</summary>
Motivation: 研究语言中词序变化的优化问题，特别是验证关于排列图中交换距离最小化的假设：给定源顺序，在排列图中距离更近的词序应该成本更低、更可能出现。

Method: 1. 使用排列图（permutohedron）作为所有序列排列结构的数学表示；2. 开发测量词序变化相对于交换距离最小化优化程度的方法；3. 将二次分配问题（QAP）引入语言研究作为多种优化问题的统一框架。

Result: 1. 跨语言手势至少达到77%的最优性；2. 多次达到最优性的情况不太可能是偶然的；3. 建立了研究通信系统中词序或手势顺序相对于交换距离最小化优化程度的理论基础。

Conclusion: 提出了一个统一的"最优分配原则"，将交换距离最小化等多种语言原则整合在二次分配问题（QAP）框架下，为语言优化研究提供了新的数学工具和理论视角。
...
TL;DR: 该论文提出了一种三步自动化质量保证方法，用于评估机器翻译基准数据集的质量，并发布了清理后的EU20数据集版本


<details>
  <summary>Details</summary>
Motivation: 机器翻译的基准数据集虽然降低了成本并提供了规模，但噪声、结构丢失和质量不均等问题削弱了其可靠性。需要能够大规模测量和验证翻译质量的方法。

Method: 采用三步自动化质量保证方法：1) 结构语料库审计与针对性修复；2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与翻译服务（DeepL/ChatGPT/Google）比较；3) 基于LLM的跨度级翻译错误分析。

Result: 趋势一致：COMET分数较低的数据集在跨度级别表现出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上针对人工编辑样本也指向相同方向。发布了清理/校正后的EU20数据集和可复现代码。

Conclusion: 自动化质量保证提供了实用、可扩展的指标，有助于优先审查，是对人工黄金标准的补充而非替代。
...
TL;DR: SAFE框架通过可验证的实体序列替换未接地的思维链，在训练时消除噪声监督，在推理时动态检测未接地步骤，显著提升多跳QA的准确性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 多跳QA基准测试经常因虚假正确性而奖励大型语言模型，掩盖了未接地或有缺陷的推理步骤。需要转向更严谨的推理评估框架。

Method: 提出SAFE动态基准测试框架，包含两个阶段：1) 训练时验证：建立原子错误分类法和基于知识图谱的验证管道，消除标准基准中的噪声监督；2) 推理时验证：在已验证数据集上训练反馈模型，实时动态检测未接地步骤。

Result: 识别出高达14%的实例为不可回答，平均准确率提升8.4个百分点，同时保证推理时可验证的轨迹。

Conclusion: SAFE不仅暴露了现有基准测试的关键缺陷，还显著优于标准基线，实现了可验证推理轨迹的同时提升性能。
...
TL;DR: 高斯扩散模型在离散数据生成中面临挑战，DDPM求解器在处理离散分布的混合delta分布表示时存在采样质量问题，通过关键采样区间分析和改进方法提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但在离散数据上的应用仍然具有挑战性。本研究旨在探究为什么使用DDPM求解器的高斯扩散模型难以从在连续空间中表示为delta分布混合的离散分布中进行采样。

Method: 使用玩具随机层次模型识别关键采样区间，在该区间内噪声化数据的密度变为多模态。分析DDPM在该区间进入低密度区域的问题，评估现有启发式方法（包括自条件和q-sampling求解器）的缓解效果。提出将自条件与在关键区间内从DDPM切换到q-sampling相结合的方法。

Result: 发现DDPM在关键采样区间偶尔会进入模态之间的低密度区域，产生模型分布外输入并降低样本质量。自条件和q-sampling有助于缓解此问题。在关键区间内结合自条件并从DDPM切换到q-sampling的方法在真实数据上提高了生成质量，在文本、编程代码和蛋白质等多个领域的条件和非条件任务中得到验证。

Conclusion: 高斯扩散模型在离散数据生成中的挑战源于关键采样区间的多模态密度问题，DDPM求解器在该区间容易进入低密度区域。通过结合自条件和在关键区间切换到q-sampling的策略可以有效改善生成质量，为离散数据上的扩散模型应用提供了重要见解和改进方法。
...
<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能够学习有效的语音表征，但尚不清楚语言结构在模型训练过程中何时以及如何形成。本研究旨在探究不同语言结构在模型训练中的编码时机和模式

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语语音训练中的中间检查点，研究多种语言结构在不同层的编码模式和学习轨迹，考察预训练目标定义层次对结构组织的影响

Result: 发现不同层次的语言结构在层间分布和学习轨迹上存在显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释；预训练目标的定义层次强烈影响语言结构的层间组织和学习轨迹，高阶预测任务（迭代优化的伪标签）能诱导更大的并行性

Conclusion: 自监督语音模型中语言结构的编码具有层次特异性，其形成过程受结构抽象程度、时间尺度整合以及预训练目标层次的多重影响，为理解语音模型的语言表征学习机制提供了重要见解
...
TL;DR: 提出BidirLM方法，将因果生成式语言模型转换为双向编码器，通过系统消融实验确定关键适应因素，引入权重合并和多领域数据混合策略，并融合专用模型提升多模态能力。


<details>
  <summary>Details</summary>
Motivation: 现有将因果生成语言模型转换为双向编码器的方法存在局限性：缺乏最优训练目标的共识、大规模时出现灾难性遗忘、难以灵活集成专用生成模型生态系统。

Method: 1) 在Gemma3和Qwen3模型上进行系统消融，确定成功适应的关键因素，强调常被忽略的先验掩码阶段的重要性；2) 提出线性权重合并与轻量级多领域数据混合的双重策略，无需原始预训练数据；3) 通过合并专用因果模型增强编码器，实现模态和领域特定能力的转移。

Result: 开发出BidirLM家族（5个编码器），在文本、视觉和音频表示基准测试中优于现有替代方案，提供开源实现方案。

Conclusion: 提出了一种通用方法，可将任何因果解码器LLM转换为高性能双向编码器，解决了现有方法的局限性，实现了多模态能力的灵活集成。
...
TL;DR: GOOSE提出了一种自适应脊柱树结构，利用上下文匹配令牌的高接受率构建深度链，同时为低接受率预测令牌创建宽分支，从而优化推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在构建候选令牌树时未区分不同来源令牌的质量差异，而研究发现上下文匹配令牌与统计预测令牌的接受率存在显著差距（中位数约6倍），这为优化树结构提供了机会。

Method: GOOSE采用训练免费框架，构建自适应脊柱树：以高接受率的上下文匹配令牌形成深度链（脊柱），在每个节点处为低接受率的统计预测令牌创建宽分支，实现各向异性的树结构。

Result: 在五个LLM（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线性能提升12-33%。

Conclusion: 当不同令牌来源存在质量差异时，各向异性的树结构（深度链+宽分支）比平衡树更优，GOOSE框架通过自适应脊柱树设计有效利用这一特性，显著提升推测解码效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前重排序模型基于静态人工标注的相关性标签进行优化，与下游生成过程解耦，导致检索到的文档虽然主题相关但无法为LLM提供精确答案生成所需的实际效用

Method: 提出重排序偏好优化（RRPO）强化学习框架，将重排序建模为序列决策过程，利用LLM反馈优化上下文效用；引入参考锚定的确定性基线确保训练稳定性

Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的强基线；框架具有良好泛化性、与查询扩展模块正交集成、对噪声监督具有鲁棒性
...
Method: 扩展ABX判别任务框架为prosodic ABX，用于评估韵律对比；构建并发布英语和日语最小对立对数据集，结合已有汉语数据集，评估英语重音、日语音高重音和汉语声调的对比敏感性
...
TL;DR: 论文提出稳定性过滤方法，通过筛选行为稳定的边界来改进推理行为导向向量的构建，显著提升数学推理性能


<details>
  <summary>Details</summary>
Motivation: 现有方法通过关键词匹配检测推理行为边界，但假设每个检测到的边界都编码了真实行为信号。研究发现这一假设是错误的：93.3%的边界是行为不稳定的，无法在相同前缀下重现检测到的行为，导致导向信号被稀释

Method: 1) 提出概率模型将内在推理行为形式化为具有上下文相关触发概率的随机事件；2) 开发稳定性过滤方法，仅保留模型能一致重现目标行为的边界；3) 结合内容子空间投影去除残差问题特定噪声

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0分）。生成的导向向量可在相同架构家族的模型间迁移而无需重新提取，提升了Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）的性能
...
TL;DR: GaelEval是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答任务，评估19个LLM发现前沿模型在盖尔语语法上超越人类基线，专有模型优于开源模型，盖尔语提示带来小幅稳定优势。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在未官方支持的语言中表现出"影子"能力，但性能参差不齐且缺乏系统评估。对于苏格兰盖尔语这种形态句法丰富的少数民族语言，现有翻译基准无法捕捉结构能力，需要更全面的评估框架。

Method: 开发GaelEval基准，包含三个部分：1)专家编写的形态句法多项选择题任务；2)文化背景翻译基准；3)大规模文化知识问答任务。评估19个LLM，并与30名流利说话者的人类基线对比，分析专有模型vs开源模型、英语vs盖尔语提示的效果。
...
Conclusion: GaelEval揭示了前沿模型在盖尔语语法的多个维度上达到超人类表现，证明了盖尔语提示的效果，并显示了专有模型相对于开源模型的一致性能优势，为少数民族语言评估提供了重要基准。
...
TL;DR: 该论文在SOMD 2026跨文档软件提及共指消解任务中取得第二名，比较了两种无需微调的方法：模糊匹配和上下文感知表示，分析了它们在噪声环境和不同规模语料下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 软件提及共指消解是一个研究不足的任务，需要评估不同方法在真实场景下的性能，特别是在处理噪声输入和大规模语料时的表现差异。

Method: 比较了两种无需微调的方法：1) 模糊匹配(FM)：基于词汇字符串相似性的方法；2) 上下文感知表示(CAR)：结合提及级别和文档级别嵌入的方法。

Result: 两种方法在所有子任务中都取得了竞争性性能(CoNLL F1 0.94-0.96)，CAR在官方测试集上始终比FM高1分。噪声注入研究显示互补的失败模式：边界噪声下CAR更鲁棒，提及替换下FM更稳健。CAR在大规模语料上具有线性扩展优势。

Conclusion: 系统选择应同时考虑上游提及检测器的噪声特征和目标语料的规模。软件名称的高表面规律性降低了对复杂语义推理的需求，使得简单方法也能取得良好效果。CAR在大规模应用中更具效率优势。
...
TL;DR: Neuro-RIT是一种通过神经元引导的鲁棒指令调优框架，从密集适应转向精确驱动的神经元对齐，增强检索增强语言模型对噪声上下文的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强语言模型在面对不相关或噪声检索上下文时性能会下降，现有增强鲁棒性的方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性
...
Conclusion: Neuro-RIT通过从密集适应转向精确驱动的神经元对齐，有效增强了检索增强语言模型对噪声上下文的鲁棒性，为提升模型在知识密集型任务中的性能提供了新范式
...
TL;DR: L3TR：一种基于大语言模型的列表式人才推荐框架，通过块注意力机制和局部位置编码解决位置偏见和并发令牌偏见问题，提升推荐效果


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的人才推荐系统多采用点对点范式，需要重复处理文本且无法捕捉候选人列表中的关系，导致令牌消耗高且推荐效果不佳。此外，LLM在处理多项选择题和多个长文档时存在位置偏见和"迷失在中间"问题。

Method: 提出L3TR框架：1) 采用隐式策略利用LLM的潜在输出进行推荐；2) 设计块注意力机制和局部位置编码方法增强文档间处理，缓解位置偏见和并发令牌偏见；3) 引入ID采样方法解决训练和推理阶段候选集大小不一致问题；4) 设计评估方法检测位置偏见和令牌偏见，并提供无需训练的去偏见方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法展现出持续改进的推荐性能。

Conclusion: L3TR框架通过列表式处理、块注意力机制和局部位置编码等方法，有效解决了基于LLM的人才推荐系统中的位置偏见、令牌偏见和效率问题，为人才推荐任务提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语音命名实体识别研究不足，主要受限于其形态复杂性、缺少短元音以及标注资源有限。需要填补这一研究空白，建立公开基准。

Method: 创建CV-18 NER数据集（基于Arabic Common Voice 18语料库，采用Wojood细粒度标注模式，21种实体类型）。对比级联系统（ASR+文本NER）和基于Whisper、AraBEST-RQ的端到端模型。

Result: 端到端系统在测试集上显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。阿拉伯语特定自监督预训练在ASR上表现良好，多语言弱监督在联合语音到实体学习上更有效。

Conclusion: 端到端方法在阿拉伯语语音NER任务上具有优势，特别是在低资源环境下。公开的数据集和模型为阿拉伯语语音NER研究提供了首个开放基准。
...
TL;DR: 论文研究如何从多个LLM中生成全面的有效回答集，提出多样性覆盖率指标，发现不同模型在不同提示上表现各异，因此开发了路由器来选择最佳模型，显著提升了回答多样性。


<details>
  <summary>Details</summary>
Motivation: 当面对允许多种有效答案的提示时，全面生成这些答案是满足广泛用户需求的第一步。现有研究缺乏评估模型生成全面回答集能力的方法，且不同模型在不同提示上的表现存在显著差异。

Method: 1. 提出多样性覆盖率指标，衡量预测答案集中每个独特答案相对于同等规模最佳答案集的质量得分总和；2. 评估18个LLM在开放提示上的表现；3. 开发路由器模型，根据查询预测最佳生成模型；4. 在NB-Wildchat数据集上训练和评估路由器。

Result: 1. 没有单一模型在所有提示上都能生成最全面的回答集；2. 每个提示都存在一个显著优于其他模型的特定模型；3. 训练的路由器在NB-Wildchat上比单一最佳模型基线提升2.5%（26.3% vs 23.8%）；4. 路由器在跨域数据集（NB-Curated）和不同答案生成策略上表现出良好的泛化能力。

Conclusion: 该研究为在拥有多个模型的情况下生成全面答案集奠定了基础，证明了通过智能路由选择最佳模型可以显著提高回答多样性，为构建更全面的AI回答系统提供了有效方法。
...
TL;DR: 论文分析了语言模型扩展新词汇时的初始化问题，提出基于语言基础的新token初始化方法GTI，在生成推荐任务中优于传统均值初始化


<details>
  <summary>Details</summary>
Motivation: 当前语言模型扩展新词汇时采用均值初始化策略，但研究发现这种初始化会使新token坍缩到退化子空间，抹除token间的区分度，导致后续微调难以完全恢复，成为词汇扩展的关键瓶颈

Method: 提出Grounded Token Initialization (GTI)方法：在微调前，通过配对的语言监督将新token映射到预训练嵌入空间中具有语义意义的独特位置，实现新token的语言基础化

Result: GTI在多个生成推荐基准测试（包括工业规模数据集和公开数据集）中，在大多数评估设置下优于均值初始化和现有的辅助任务适应方法；分析显示基础化嵌入能产生更丰富的token间结构，并在微调过程中保持

Conclusion: 初始化质量是词汇扩展的关键瓶颈，基于语言基础的token初始化（GTI）能更好地利用模型的通用知识处理新token领域，为语言模型词汇扩展提供了更有效的策略
...
### [155] [Non-Rigid 3D Shape Correspondences: From Foundations to Open Challenges and Opportunities](https://arxiv.org/abs/2604.01274)
...
TL;DR: 该报告综述了变形形状对应估计的三大范式：基于功能映射的谱方法、施加离散约束的组合方法、直接恢复全局对齐的变形方法，并讨论了新兴挑战和机会。


<details>
  <summary>Details</summary>
Motivation: 变形形状实例之间的对应估计是计算机图形学中长期存在的问题，从纹理转移到统计建模等众多应用都依赖于恢复准确的对应映射。不同下游应用需要不同的方法，因此需要系统梳理该领域的最新进展和趋势。

Method: 将发展分为三大范式：1）基于功能映射的谱方法；2）施加离散约束的组合方法；3）直接恢复全局对齐的变形方法。报告系统讨论每种方法的优缺点，并突出各领域的最新进展。

Result: 提供了该领域的全面综述，帮助研究人员、从业者和学生理解最新趋势和进展。报告不仅总结了现有方法，还指出了新的研究方向，包括零样本对应的视觉基础模型应用和部分形状匹配等挑战性任务。

Conclusion: 该报告系统梳理了变形形状对应估计领域的三大范式，分析了各自的优缺点，并指出了新兴挑战和机会，为零样本对应和部分形状匹配等前沿问题提供了研究方向的建议。
...
### [156] [ColorGradedGaussians: Palette-Based Color Grading for 3D Gaussian Splatting via View-Space Sparse Decomposition](https://arxiv.org/abs/2604.01551)
*Cheng-Kang Ted Chao,Yotam Gingold*

Main category: cs.GR

TL;DR: 提出用于3D高斯泼溅的实时交互式色彩编辑框架，支持基于调色板的重新着色、色彩感知的亮度调整和像素级约束，通过视图空间分解解决现有方法在alpha混合后分解稀疏性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 专业色彩编辑需要精确控制色彩（色调和饱和度）和亮度，理想情况下应通过独立控制实现。现有基于调色板的3DGS方法在原始级别分解颜色，但稀疏的原始级别权重无法保证alpha混合后的像素级分解稀疏性，导致调色板编辑影响非目标区域，降低编辑质量。

Method: 采用视图空间调色板分解方法，泼溅权重而非颜色以优化场景的可观察外观。引入基于逆重心坐标的几何损失来强制执行一致的稀疏模式，确保相似颜色具有相似分解。支持基于调色板的重新着色、每个调色板的色调曲线用于色彩感知亮度调整，以及精确的像素级约束。

Result: 相比原始空间方法实现了更优的编辑质量，能够为3DGS场景提供专业色彩分级工作流程，并保持实时交互性能。

Conclusion: 提出的视图空间调色板分解方法解决了现有3DGS色彩编辑方法的局限性，实现了高质量的实时交互式色彩编辑，支持专业色彩分级工作流程。
...
### [157] [Topology-First B-Rep Meshing](https://arxiv.org/abs/2604.02141)
...
<details>
  <summary>Details</summary>
Motivation: 传统B-Rep到网格的转换方法存在拓扑不一致问题，现有修复策略缺乏理论保证且对复杂模型经常失败

Method: 将B-Rep拓扑作为网格生成过程的不变量，算法强制保持精确拓扑，仅通过用户定义的单一容差控制网格与参数曲面的偏差

Result: 在ABC和Fusion 360库的数千个真实CAD模型上验证，包括标准网格工具失败的实例，证明方法能生成适合下游应用的可靠网格

Conclusion: 算法层面的拓扑保证实现了可靠的网格生成，解耦了拓扑正确性和几何精度，消除了后处理修复需求
