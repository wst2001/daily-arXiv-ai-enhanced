...
Motivation: 现有3D化身建模方法面临保真度与泛化性的关键权衡：多视角工作室数据能实现高保真度但泛化性差，而大规模野外数据训练的模型泛化性好但质量低。需要一种既能保持高保真度又能泛化到大规模人群的解决方案。

Method: 采用预训练/后训练范式：首先在100万个野外视频上进行预训练，学习外观和几何的广泛先验；然后在高质量策划数据上进行后训练，增强表达性和保真度。模型采用前馈推理，支持精确的面部表情和手指级关节控制。

Result: LCA能够泛化到不同发型、服装和人口统计特征，同时保持强身份保留。模型展现出对重新照明和宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性，尽管没有直接监督。

Conclusion: LCA通过创新的预训练/后训练范式成功解决了3D化身建模中保真度与泛化性的权衡问题，为大规模高质量3D化身生成提供了有效解决方案，展现出强大的涌现能力和零样本泛化特性。
...
TL;DR: DOne框架通过解耦结构理解和元素渲染，解决了VLMs在Design-to-Code生成中的整体瓶颈问题，在复杂布局任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在Design-to-Code生成中存在"整体瓶颈"问题，无法同时处理高层结构层次和细粒度视觉细节，导致布局扭曲或使用通用占位符。

Method: DOne框架包含三个核心组件：1) 学习的布局分割模块，避免启发式裁剪的局限性；2) 专门化的混合元素检索器，处理UI组件的极端宽高比和密度；3) 模式引导的生成范式，连接布局和代码。

Result: 在HiFi2Code基准测试中，DOne在高层视觉相似度（如GPT Score提升超过10%）和细粒度元素对齐方面均优于现有方法。人工评估显示生产力提升3倍，视觉保真度更高。

Conclusion: DOne通过解耦结构理解和元素渲染的方法，有效解决了Design-to-Code生成中的整体瓶颈问题，在复杂布局任务上实现了显著性能提升。
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进预训练多模态大语言模型使用多模态证据能力的框架，通过注意力模式识别相关视觉区域和文本元素，并使用提示标记引导模型重新关注关键证据。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答需要外部知识的图像问题时，难以识别最相关的视觉和文本证据，特别是在需要整合噪声或部分相关文本证据以及定位图像中细粒度视觉信息的场景下。

Method: 利用模型注意力模式估计与查询相关的视觉区域和检索到的文本元素，然后基于这些突出显示的证据生成答案。通过轻量级提示级标记突出显示选定的线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的视觉问答基准测试中，相比零样本多模态大语言模型取得了持续改进。在视觉中心和幻觉导向的基准测试中，仅视觉证据突出显示就能在没有文本上下文的环境中提升模型性能，且无需额外训练或架构修改。

Conclusion: Look Twice框架通过推理时的证据突出显示机制，有效提升了预训练多模态大语言模型在多模态证据利用方面的能力，为知识密集型视觉问答任务提供了一种无需训练的高效解决方案。
...
TL;DR: MedQwen：一种参数高效的医学视觉语言模型，通过频谱路由的专家混合和理论支撑的缩放规则，在保持基础架构不变的情况下，实现了接近全参数微调的性能，同时大幅减少可训练参数并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用基准上表现优异，但在医学影像领域缺乏鲁棒性。医学数据的异质性监督导致跨数据集干扰和对数据机制的敏感性。在临床工作流中，数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。需要解决这些挑战。

Method: 提出MedQwen模型，采用参数高效的频谱路由专家混合架构。从预训练权重的非重叠奇异值分解片段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。使用理论支撑的缩放规则，将低秩更新与全秩、全参数微调的MoE对齐。

Result: 在23个医学数据集上评估，涵盖视觉问答、报告生成、放射学分类和幻觉缓解任务。MedQwen在零样本分类任务上接近全参数微调性能，可训练参数减少339倍。在顺序学习中将遗忘率降低至约5%，而强基线模型性能下降20-50%。

Conclusion: MedQwen通过创新的频谱路由MoE架构和理论支撑的缩放规则，有效解决了医学视觉语言模型中的跨数据集干扰、数据机制敏感性和灾难性遗忘问题，实现了参数高效且鲁棒的医学AI系统。
...
TL;DR: 提出基于视觉Transformer的视频分析方法，用于检测美式足球训练中的危险擒抱动作，并在扩展数据集上实现比基线模型更高的风险召回率


<details>
  <summary>Details</summary>
Motivation: 在接触性运动中早期识别危险动作可以及时干预并提高运动员安全性，需要开发可靠的方法来检测美式足球训练中的危险擒抱动作

Method: 使用基于视觉Transformer的模型，采用不平衡感知训练策略，在733个单运动员-假人擒抱视频剪辑的数据集上进行交叉验证，每个剪辑都标注了SATT-3标准中的击打区域组件

Result: 获得0.67的风险召回率和0.59的风险F1分数，相比先前基线（风险召回率0.58，风险F1 0.56）在更大数据集上风险召回率提高了超过8个百分点

Conclusion: 视觉Transformer视频分析结合对类别不平衡的仔细处理，可以可靠地检测罕见但安全关键的擒抱模式，为教练中心的伤害预防工具提供了实用途径
...
TL;DR: 通过合成蹦床姿态数据集微调ViTPose模型，显著提升极端姿态下的姿态估计性能


<details>
  <summary>Details</summary>
Motivation: 蹦床体操涉及极端人体姿态和非常规视角，现有姿态估计模型在此类数据上表现不佳。需要专门的方法来处理这种具有挑战性的姿态估计任务。

Method: 1) 从蹦床动作的运动捕捉记录生成合成蹦床姿态数据集(STP)；2) 开发管道将噪声运动捕捉数据拟合到参数化人体模型；3) 生成多视角真实感图像；4) 使用该数据微调ViTPose模型；5) 在真实多视角蹦床图像上测试。

Result: 1) 在2D姿态估计上获得最先进结果，缩小了常规姿态与极端姿态之间的性能差距；2) 在3D姿态估计中，MPJPE降低了12.5毫米，相比预训练ViTPose模型提升了19.6%；3) 2D精度的提升直接转化为3D三角测量性能的改善。

Conclusion: 通过合成数据微调姿态估计模型是解决极端姿态估计问题的有效方法，特别是在蹦床体操等具有挑战性的场景中。该方法显著提升了模型在非常规视角和极端姿态下的性能。
...
TL;DR: 提出Attention Regularization方法，通过自举法量化注意力分数的不确定性，去除噪声产生的虚假注意力，提高注意力图的稀疏性和可解释性


<details>
  <summary>Details</summary>
Motivation: Vision Transformers的注意力分数通常非零，导致注意力图噪声大、扩散性强，限制了可解释性。需要量化注意力分数的不确定性并获取正则化注意力分数

Method: 将ViT注意力分数置于统计框架中，引入自举法（bootstrapping）通过重采样输入特征生成注意力分数的基线分布，估计注意力分数的显著性和后验概率

Result: 在自然图像和医学图像上，Attention Regularization方法能有效去除噪声产生的虚假注意力，显著提高注意力图的收缩性和稀疏性。通过模拟和真实数据集进行了定量评估
...
<details>
  <summary>Details</summary>
Motivation: 研究CNN特征相关性（Gram矩阵）是否与人类纹理感知内容对齐，以及被认为是更好视觉系统模型的CNN是否具有更类似人类的纹理表示

Method: 比较不同CNN计算的特征相关性的感知内容，并将其与Brain-Score测量的模型与哺乳动物视觉系统的感知对齐进行比较
...
TL;DR: AffordTissue是一个用于预测胆囊切除术中工具-动作特定组织可操作性区域的密集热图的多模态框架，通过结合时间视觉编码器、语言条件化和DiT风格解码器，显著优于现有视觉语言模型基线。


<details>
  <summary>Details</summary>
Motivation: 当前手术自动化方法在临床部署中存在局限性：缺乏对器械在组织表面交互位置的预测性，以及缺少强制执行工具-动作特定安全交互区域的显式条件输入。这阻碍了从桌面实验到临床部署的转化。

Method: 提出AffordTissue多模态框架，包含：1) 时间视觉编码器，从多视角捕获工具运动和组织动态；2) 语言条件化，实现跨不同器械-动作对的泛化；3) DiT风格解码器，用于密集可操作性预测。建立了首个组织可操作性基准数据集，包含15,638个视频片段，覆盖103个胆囊切除术中的6种工具-动作对。

Result: 实验显示显著优于视觉语言模型基线（20.6像素ASSD vs. 60.2像素的Molmo-VLM），表明任务特定架构在密集手术可操作性预测方面优于大规模基础模型。

Conclusion: 通过预测工具-动作特定组织可操作性区域，AffordTissue为安全手术自动化提供了显式空间推理，可能解锁对适当组织区域的显式策略指导，并在器械偏离预测安全区域时实现早期安全停止。
...
TL;DR: GRAZE：无需训练的无监督方法，用于在美式足球训练视频中定位首次接触点（FPOC），通过目标检测、运动感知时序推理和像素级接触验证实现
...
Method: GRAZE采用无训练流水线：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时序推理进行精炼；3) 使用SAM2作为显式像素级接触验证器，而非仅依赖检测置信度。候选发现与接触确认分离使方法对杂乱场景和冲击附近不稳定的定位具有鲁棒性

Result: 在738个擒抱训练视频上，GRAZE在97.4%的片段中产生有效输出，在77.5%的所有片段中将FPOC定位在±10帧内，在82.7%的所有片段中定位在±20帧内

Conclusion: 在真实世界训练视频中实现帧级准确的接触起始定位是可行的，无需特定任务训练。分离候选发现与接触确认的方法对杂乱场景具有鲁棒性，为生物力学分析提供了可靠基础
...
TL;DR: 提出SVRaster框架解决3DGS在开放词汇3D场景理解中的空间模糊性和多级语义模糊性问题，通过结构化几何表示实现确定性特征注册，利用AM-RADIO模型解决多级语义模糊，在细粒度查询任务上取得SOTA性能。
...
Method: 提出基于稀疏体素光栅化的结构化几何表示框架，通过单目深度和法线先验进行正则化，建立稳定几何基础。实现确定性、置信感知的特征注册过程，抑制3DGS中常见的语义渗漏伪影。利用基础模型AM-RADIO的密集对齐特性解决多级语义模糊，避免分层训练的计算开销。

Result: 在开放词汇3D对象检索和点云理解基准测试中达到最先进性能，特别是在传统注册方法通常失败的细粒度查询任务上表现出色。

Conclusion: SVRaster框架通过结构化几何表示解决了3DGS在开放词汇3D场景理解中的关键限制，实现了更精确的特征注册和语义理解，为细粒度3D场景分析提供了有效解决方案。
...
Result: 在HD-EPIC、EgoExo4D和HOT3D等真实世界数据集上，EgoFlow在准确性、泛化性和物理真实性方面优于基于扩散和Transformer的基线方法，碰撞率降低高达79%，并在未见场景中表现出强泛化能力。

Conclusion: 基于流的生成建模在可扩展和物理基础的自我中心运动理解方面具有前景，EgoFlow通过结合生成建模与显式物理约束，实现了连贯可控的运动生成。
...
<details>
  <summary>Details</summary>
Motivation: 无标记姿态估计技术使得使用标准视频捕捉自然场景中的人体运动成为可能，但姿态数据的高维度、噪声和时间复杂性给提取有意义的协调和行为变化模式带来了挑战
...
TL;DR: UniRecGen是一个统一框架，将前馈重建与扩散生成相结合，通过共享规范空间和对齐学习，解决稀疏视图3D建模中重建保真度与生成合理性之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的基本矛盾：前馈重建方法效率高且输入对齐好，但缺乏全局先验导致结构不完整；而基于扩散的生成方法能提供丰富几何细节，但难以保持多视图一致性。

Method: 提出UniRecGen统一框架，将两种范式整合到单一协作系统中。通过共享规范空间对齐坐标空间、3D表示和训练目标冲突，采用解耦协作学习保持训练稳定性。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和完善几何结构。

Result: 实验结果表明，UniRecGen在从稀疏观测创建完整一致的3D模型方面，实现了卓越的保真度和鲁棒性，优于现有方法。

Conclusion: UniRecGen成功整合了前馈重建和扩散生成的优势，通过共享规范空间和对齐学习，有效解决了稀疏视图3D建模中的核心矛盾，实现了高质量、一致的3D模型生成。
...
TL;DR: 提出TAG（热反鬼影）通用计算热成像框架，解决材料非均匀性问题，实现高保真夜视，首次恢复鬼影人脸表情


<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受鬼影效应（光子流中细节纹理丢失）困扰。现有HADAR技术仅适用于均匀材料场景，而真实世界中材料非均匀性普遍存在，需要通用抗鬼影成像解决方案
...
<details>
  <summary>Details</summary>
Motivation: 无人机图像语义分割面临极端尺度变化、复杂边界和边缘设备计算资源有限等挑战。现有方法要么计算开销大（如基于Transformer的方法），要么难以捕捉高分辨率航拍场景的细节（轻量级方法）。

Method: 提出PBSeg框架：1）原型交叉注意力（PBCA）利用特征冗余降低计算复杂度；2）高效多尺度特征提取模块结合可变形卷积（DConv）和上下文感知调制（CAM）来捕捉局部细节和全局语义。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在动态区域初始化不完整，导致重建和运动估计不稳定，通常需要依赖外部预计算的光流等密集运动指导，这增加了复杂性和潜在错误传播。

Method: ReFlow包含三个核心模块：1) 完整规范空间构建模块，增强静态和动态区域的初始化；2) 分离式动态场景建模模块，解耦静态和动态组件进行针对性运动监督；3) 自校正流匹配机制，包括全流匹配（对齐3D场景流与时间变化的2D观测）和相机流匹配（强制静态对象的多视图一致性）。
...
TL;DR: VideoZeroBench：用于长视频问答的分层基准测试，通过时空证据验证来严格评估模型性能，揭示当前模型在基于证据的推理方面存在显著不足


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键缺陷：(1) 虚高的分数掩盖了细粒度视觉理解和推理的不足；(2) 答案正确性评估通常不验证模型是否识别了支持其预测的精确时空证据。需要建立更严格的评估方法来衡量真实的基于证据的推理能力。

Method: 提出了VideoZeroBench基准测试，包含500个手动标注的问题，涵盖13个领域，每个问题都配有时间区间和空间边界框作为证据。引入了五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro的正确率也低于17%。当施加定位约束时，性能急剧下降：在需要正确答案和精确时空定位的情况下（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。这揭示了表面答案正确性与真实基于证据的推理之间存在显著差距。

Conclusion: 基于证据的视频理解仍然是长视频问答的瓶颈。VideoZeroBench暴露了当前模型在时空证据验证方面的严重不足，为未来基于证据的视频推理研究提供了重要见解和评估框架。
...
TL;DR: 提出卫星图像无关训练框架，通过无人机多视角序列重建3D场景并生成伪正射影像，实现无卫星数据下的跨视图地理定位


<details>
  <summary>Details</summary>
Motivation: 现有无人机地理定位方法依赖卫星图像进行训练（配对监督或无监督对齐），但在卫星数据不可用或受限时部署受限。需要开发不依赖卫星图像的训练方法。

Method: 提出卫星无关训练框架：1) 使用3D高斯溅射从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影将重建几何投影为伪正射影像；3) 轻量级几何引导修复获得纹理完整的无人机侧视图；4) 从生成的正射影像提取DINOv3特征，仅用无人机数据学习Fisher向量聚合模型，测试时复用该模型编码卫星瓦片进行跨视图检索。

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星无关泛化基线，缩小了与使用卫星图像训练方法的性能差距。

Conclusion: 提出的卫星无关训练框架通过无人机侧3D场景重建和伪正射影像生成，实现了不依赖卫星图像的地理定位训练，为卫星数据受限场景提供了实用解决方案。
...
TL;DR: SHOE是一个新的开放词汇人-物交互检测评估框架，通过语义相似度而非精确字符串匹配来评估预测质量，使用LLM计算动词和对象组件的语义相似度，与人类判断一致性达85.73%。
...
Method: SHOE将每个HOI预测分解为动词和对象组件，使用多个大型语言模型的平均结果估计它们的语义相似度，然后将这些相似度组合成一个综合评分，支持超越精确字符串匹配的语义对齐评估。

Result: SHOE评分与人类判断的一致性达到85.73%，优于现有的基于LLM和嵌入的基线方法，能够更准确地反映人类对交互的理解。

Conclusion: 需要基于语义的HOI评估来更好地反映人类对交互的理解，SHOE提供了一个灵活可扩展的评估框架，适用于现有HOI检测方法和开放生成模型，将在标准基准测试中促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 开集测试时适应（OSTTA）面临协变量偏移下ID样本分类与OOD样本检测的双重挑战。现有方法结合熵最小化（保持ID性能）和熵最大化（增强OOD检测）存在内在冲突，导致csID分类与csOOD检测之间的权衡问题

Method: 提出ROSETTA方法：1）分析熵最大化在OSTTA中的局限性；2）引入角度损失调节特征范数大小；3）设计特征范数损失抑制csOOD样本的logits输出。这些组件共同提升OOD检测能力

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强大的OOD检测能力，同时保持高ID分类性能。在Cityscapes数据集上验证了真实世界语义分割的有效性，在HAC数据集上展示了不同开集TTA设置的适用性

Conclusion: ROSETTA通过角度损失和特征范数损失有效解决了熵最小化与最大化之间的冲突，在多种数据集和任务上实现了开集测试时适应的鲁棒性能，为协变量偏移环境下的模型适应提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS流水线需要集中访问所有观测数据，这在分布式机器人系统中不适用，因为智能体独立运行且集中数据聚合可能受限。直接扩展集中式训练到多智能体系统会引入通信开销和几何不一致问题。

Method: 首先通过注册多个客户端本地合并的LiDAR点云构建共享几何骨架来初始化全局3DGS模型。在联邦优化期间，固定高斯位置以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度和球谐系数）。服务器使用可见性感知聚合来聚合这些更新，根据每个客户端观察每个高斯的频率加权其贡献。

Result: 实验表明F3DGS在实现与集中式训练相当的重建质量的同时，能够支持跨智能体的分布式优化。研究团队收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集用于评估。

Conclusion: F3DGS为分布式机器人系统中的3D重建提供了一种有效的联邦学习解决方案，解决了多智能体探索中的部分可观测性挑战，数据集、开发工具包和源代码将公开发布。
...
TL;DR: Tex3D：首个在VLA模拟环境中端到端优化3D对抗纹理的框架，通过前景-背景解耦和轨迹感知优化，显著降低VLA在机器人操作任务中的性能


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作中表现良好，但对物理可实现对抗攻击的鲁棒性研究不足。现有攻击方法（语言扰动和2D视觉攻击）要么代表性不足，要么物理真实性有限。相比之下，对抗性3D纹理更具物理可行性和破坏性，但标准3D模拟器缺乏从VLA目标函数到物体外观的可微分优化路径，难以实现端到端优化。

Method: 提出Tex3D框架，包含两个核心技术：1) 前景-背景解耦(FBD)：通过双渲染器对齐实现可微分纹理优化，同时保留原始模拟环境；2) 轨迹感知对抗优化(TAAO)：优先处理行为关键帧，并使用基于顶点的参数化稳定优化，确保攻击在物理世界中的长时程和多视角有效性。

Result: 在模拟和真实机器人实验中，Tex3D显著降低了VLA模型在多个操作任务中的性能，任务失败率高达96.7%。实验结果表明VLA系统对物理基础的3D对抗攻击存在严重脆弱性。

Conclusion: Tex3D是首个在VLA模拟环境中端到端优化3D对抗纹理的框架，成功揭示了VLA系统对物理基础3D对抗攻击的关键脆弱性，强调了鲁棒性感知训练的必要性。
...
TL;DR: 利用稀疏自编码器从基础模型特征中提取单义、空间定位的神经元，构建自动化的形态特征标注流水线，创建包含8万标注的昆虫形态特征数据集


<details>
  <summary>Details</summary>
Motivation: 形态特征对生态研究至关重要，但传统的人工标注方法耗时费力，限制了大规模生态研究的开展。主要瓶颈在于缺乏连接生物图像与特征级标注的高质量数据集

Method: 在基础模型特征上训练稀疏自编码器，获得单义且空间定位的神经元；构建特征标注流水线，定位显著区域并使用视觉语言提示生成可解释的特征描述；通过消融研究评估设计敏感性

Result: 创建了Bioscan-Traits数据集，包含8万个特征标注，覆盖BIOSCAN-5M中的1.9万张昆虫图像；人类评估确认了生成形态描述的生物合理性；消融研究系统评估了关键设计选择对标注质量的影响

Conclusion: 通过模块化流水线而非昂贵的人工标注，为大规模形态分析提供了可扩展的方法，在基础模型中注入生物学意义监督，弥合生态相关性与机器学习实用性之间的差距
...
TL;DR: LivingWorld是一个从单张图像生成具有环境动态的4D世界的交互式框架，通过渐进构建全局一致的运动场，支持云、水、烟雾等环境动态的生成。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法主要关注静态几何重建，而忽略了场景尺度的环境动态（如云、水、烟雾）。建模这类动态具有挑战性，因为运动需要在扩展场景中保持一致性，同时支持低延迟用户反馈。

Method: 1. 渐进构建全局一致的运动场；2. 引入几何感知对齐模块解决视图间的方向和尺度模糊性；3. 使用基于哈希的紧凑运动场表示，支持高效查询和动态稳定传播；4. 支持渲染时的双向运动传播，无需昂贵的视频细化。

Result: 在单个RTX 5090 GPU上，每个新场景扩展步骤生成需要9秒，运动对齐和运动场更新需要3秒，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架能够从单张图像生成具有连贯环境动态的4D世界，解决了场景扩展中运动一致性的挑战，实现了交互式生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但在真实场景中由于3D标注成本高，通常只有部分目标被标注，需要解决稀疏标注下的检测问题

Method: 提出包含两个关键模块的框架：1) 道路感知补丁增强(RAPA)：将分割的目标补丁增强到道路区域，保持3D几何一致性；2) 原型过滤(PBF)：通过原型相似性和深度不确定性生成高质量伪标签，维护全局2D RoI特征原型并选择与学习原型特征一致且深度估计可靠的伪标签

Result: 大量实验证明了所提方法的有效性，源代码已公开

Conclusion: 通过结合几何保持增强和原型引导的伪标注训练策略，在稀疏监督下实现了鲁棒的3D目标检测
...
TL;DR: 该论文提出了一种基于物理学的视频认证方法，利用莫尔效应产生的运动不变量来区分真实拍摄视频与AI生成视频。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分。需要一种基于物理原理的认证方法来可靠地区分真实视频和AI生成视频。

Method: 利用莫尔效应：当相机观察紧凑的双层光栅结构时产生的干涉条纹。推导出莫尔运动不变量，证明条纹相位和光栅图像位移通过光学几何线性耦合，且与观察距离和光栅结构无关。验证器从视频中提取这两个信号并测试它们的相关性。

Result: 在真实拍摄视频和多个最先进生成器的AI生成视频上验证了该不变量，发现真实视频和AI生成视频产生显著不同的相关特征，表明这是一种鲁棒的区分方法。

Conclusion: 确定性光学现象可以作为物理基础的、可验证的签名来对抗AI生成视频，为视频认证提供了新的物理基础方法。
...
TL;DR: DynaVid：利用合成运动数据增强视频扩散模型动态运动生成与控制的框架
...
Method: 提出DynaVid框架，利用计算机图形流水线渲染的光流作为合成运动数据。采用两阶段生成：运动生成器先合成运动（光流），然后运动引导的视频生成器基于该运动生成视频帧。这种解耦设计使模型能从合成数据学习动态运动模式，同时从真实视频保持视觉真实感。

Result: 在剧烈人体运动生成和极端相机运动控制两个具有挑战性的场景中验证了框架有效性。实验表明DynaVid在动态运动生成和相机运动控制方面显著提升了真实感和可控性。

Conclusion: 通过利用合成运动数据（光流）并采用解耦的两阶段生成框架，DynaVid成功解决了视频扩散模型在动态运动生成和控制方面的局限性，实现了从合成数据学习运动模式同时保持真实视觉外观的目标。
...
TL;DR: 提出了一种无需域标识和样本存储的增量学习框架，用于具身多媒体系统在动态环境分布漂移下的连续自适应


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间连续交互时面临动态环境分布漂移的挑战，现有方法依赖测试阶段预先获取的域标识，限制了在未知交互场景中的实用性，且模型容易过拟合特定环境的感知噪声，导致泛化能力不足和灾难性遗忘

Method: 设计了解耦表示机制去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；采用权重融合策略在参数空间动态整合新旧环境知识，无需存储历史数据即可适应新分布并保留旧环境的判别能力

Result: 在多个标准基准数据集上的大量实验表明，该方法在完全无需样本和域标识的设置下显著减少了灾难性遗忘，其准确率优于现有的最先进方法

Conclusion: 提出的框架为具身多媒体系统提供了无需域标识和样本存储的增量学习解决方案，能够有效应对动态环境分布漂移，实现鲁棒的连续环境自适应
...
Method: 提出频率域适应(FDA)策略，通过转移低频光谱分量编码域相关外观特征；引入谐波约束最优传输(HOT)，利用心脏信号的谐波特性指导原始和FDA转移表示之间的对齐

Result: 广泛的跨数据集实验表明，FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力

Conclusion: FDA和HOT框架为解决rPPG中的域偏移问题提供了原则性策略，通过建模外观变化和利用心脏信号的谐波特性，实现了更好的跨域泛化性能
...
TL;DR: Director提出了一种统一的时空高斯表示方法，联合建模人体表演、高保真渲染和实例级语义，通过嵌入实例一致语义增强4D建模，支持动态场景理解和语言对齐查询。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法主要关注外观渲染，但缺乏对实例级结构的建模，限制了在高度动态场景中的稳定跟踪和语义推理能力。需要一种能够同时处理高质量渲染和语义理解的统一表示方法。

Method: 1) 利用时间对齐的实例掩码和MLLM生成的句子嵌入，通过两个MLP解码器监督高斯点的可学习语义特征；2) 将2D光流与4D高斯连接，微调其运动以获得可靠初始化和减少漂移；3) 引入几何感知的SDF约束和正则化项，增强动态前景建模的时间一致性。

Result: 实验表明Director能够实现时间一致的4D重建，同时支持实例分割和开放词汇查询，在动态场景理解和语义推理方面表现出色。

Conclusion: 嵌入实例一致语义自然补充了4D建模，实现了更准确的场景分解并支持鲁棒的动态场景理解，为体积视频的语义感知表示提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频对象移除方法难以消除对象引发的副作用（如阴影、反射、光照变化），同时保持整体连贯性。这一限制源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 1. 外部引导：提出知识蒸馏方案，从视觉基础模型向视频扩散模型传递对象与其引发效应之间的关系知识。2. 内部引导：提出帧间上下文交叉注意力机制，使每个去噪块都能基于目标区域周围的信息丰富、未掩码的上下文进行推理。

Result: 实验证明该方法达到最先进性能，并建立了首个真实世界视频对象移除基准数据集，促进未来研究和社区发展。

Conclusion: 外部和内部引导的联合使用使模型能够理解目标对象、其引发的效应以及全局背景上下文，从而实现清晰连贯的对象移除。代码、数据和模型已开源。
...
Method: 采用两步过程：1）基于个体注视注意力热图和群体推理中估计的群体成员标量生成共享注意力热图；2）根据初始共享注意力热图细化初始群体成员关系，并最终预测共享注意力热图。
...
Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的大量实验表明，SteerFlow比现有方法始终实现更好的编辑质量，并能自然扩展到多轮编辑范式而不累积漂移。
...
TL;DR: 提出DPMO方法从点标注生成密集实例分割，以及RPS框架通过强化学习选择最佳预测点，在多个数据集上实现SOTA的密集人群实例分割性能


<details>
  <summary>Details</summary>
Motivation: 当前人群数据集主要使用点标注，区域标注（如边界框）稀缺且不准确。直接应用SAM等基础模型在密集人群场景效果不佳，需要专门的方法从点标注生成准确的实例分割

Method: 1. Dense Point-to-Mask Optimization (DPMO)：结合SAM与最近邻排他圆约束，从点标注生成密集实例分割；2. Reinforced Point Selection (RPS)框架：使用Group Relative Policy Optimization (GRPO)训练，从初始点预测采样中选择最佳预测点；3. 设计基于掩码监督的新损失函数

Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上实现了最先进的密集人群实例分割性能，基于掩码监督的损失函数显著提升了不同模型的计数准确率

Conclusion: 提出的DPMO和RPS框架有效解决了密集人群实例分割问题，掩码标注在提升计数准确率方面发挥重要作用，为人群分析任务提供了新的解决方案
...
TL;DR: 提出几何感知的无人机跨视图地理定位框架，通过3D场景重建和虚拟鸟瞰图渲染统一粗粒度位置检索与细粒度姿态估计，在GNSS拒止环境中实现米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 解决无人机在GNSS拒止环境中跨视图地理定位的挑战，现有方法将透视畸变视为外观噪声而非几何变换，采用解耦的检索-姿态估计流程，无法充分利用几何信息。

Method: 提出视觉几何基础Transformer(VGGT)从多视角无人机图像序列重建局部3D场景，渲染虚拟鸟瞰图作为几何中介对齐卫星图像；引入卫星注意力块隔离候选位置交互；发布重新校准的University-1652数据集。

Result: 在精炼的University-1652基准和SUES-200数据集上显著优于现有方法，实现鲁棒的米级定位精度，在复杂城市环境中具有更好的泛化能力。

Conclusion: 通过显式建模3D场景几何，将位置识别和姿态估计统一到单次推理流程中，为无人机跨视图地理定位提供了更准确、鲁棒的解决方案，特别适用于复杂城市环境。
...
TL;DR: 本文构建了US-365K大规模超声图像-文本数据集，提出超声诊断分类学(UDT)包含两个层次知识框架，并开发了Ultrasound-CLIP语义感知对比学习框架，在超声图像分类和检索任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言预训练模型（如CLIP）主要针对其他模态设计，难以直接应用于超声数据，因为超声图像具有异质性解剖结构和多样化诊断属性。需要专门针对超声模态的预训练方法来弥补这一差距。

Method: 1) 构建US-365K数据集（36.5万配对样本，52个解剖类别）；2) 建立超声诊断分类学(UDT)，包含超声层次解剖分类学和超声诊断属性框架；3) 提出Ultrasound-CLIP框架，引入语义软标签和语义损失优化样本区分；4) 基于UDAF文本表示构建异构图模态，实现病变-属性关系的结构化推理。

Result: 在患者级数据划分下，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务上表现出强大的泛化能力。

Conclusion: 通过构建大规模超声数据集和专门的知识框架，结合语义感知对比学习，成功开发了针对超声模态的预训练模型，显著提升了超声图像的理解和分析能力，为临床诊断提供了有效工具。
...
TL;DR: 提出一种利用自监督学习特征作为条件信号控制预训练视频扩散模型的方法，实现视频域迁移和3D到视频生成，通过轻量架构解耦外观特征以增强可控性。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型主要依赖感知、几何或简单语义信号作为条件，而自监督学习获得的高维特征虽然包含丰富场景信息，但风格、光照和语义信息纠缠，限制了生成能力。需要探索如何有效利用这些特征作为通用条件信号来控制视频扩散模型。

Method: 提出轻量级架构和训练策略，解耦外观特征与其他需要保留的特征（如几何、语义），使模型能够鲁棒控制外观变化（如风格化、重光照）。同时发现低空间分辨率可通过更高特征维度补偿，提升从显式空间表示的生成渲染可控性。

Result: 方法成功应用于视频域迁移和从3D生成视频等任务，实现了对外观变化（如风格化、重光照）的鲁棒控制，并展示了高维特征对低空间分辨率的补偿效果，提升了生成渲染的可控性。

Conclusion: 自监督学习特征可作为有效的通用条件信号用于预训练视频扩散模型，通过解耦外观特征的方法能够实现对外观变化的鲁棒控制，为视频生成和域迁移任务提供了新思路。
...
TL;DR: 论文提出了RebusBench基准测试，用于评估大视觉语言模型在解决需要多步骤神经符号推理的谜语谜题上的能力，发现当前模型表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步骤推理的问题上存在认知鸿沟，特别是当视觉输入仅作为线索而非答案时。谜语谜题需要模型整合视觉感知、语言知识和抽象映射能力，这反映了人类认知中的神经符号推理能力。

Method: 作者构建了RebusBench基准测试，包含1,164个谜语谜题，专门测试感知与知识的整合能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，使用精确匹配和语义准确率作为评估指标，并研究了模型缩放和上下文学习的影响。

Result: 评估结果显示模型表现严重不足：精确匹配低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 当前大视觉语言模型在需要神经符号推理的复杂任务上存在根本性缺陷，它们拥有感知和语言组件，但缺乏将这些组件整合为连贯推理的认知"胶水"。这为未来模型开发指出了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAM）主要关注2D外观或潜在表示建模，缺乏对物理世界具身系统至关重要的几何基础。需要开发一个既能统一推理和指令跟随能力，又能进行时空世界建模的几何感知驾驶模型。

Method: 采用大型语言模型处理语言指令、多视角图像和动作，随后通过三个轻量级生成器分别生成深度、未来视频和动作。模型学习几何感知的世界表示，并在统一框架内指导未来预测和规划，保持模块化和可控延迟。

Result: 在Navsim v1和v2基准测试中，模型在闭环规划和世界生成任务上表现优异：Navsim v1达到89.2 PDMS，Navsim v2达到88.7 EPDMS，超越现有基于世界模型的方法，同时生成更高质量的深度和未来视频预测。

Conclusion: DriveDreamer-Policy通过集成深度生成、未来视频生成和运动规划，实现了几何感知的驾驶世界-动作建模。显式深度学习为视频想象提供互补优势并提升规划鲁棒性，证明了统一几何感知框架在具身驾驶系统中的价值。
...
TL;DR: FSKD是一个LiDAR到RGB-红外知识蒸馏框架，通过多模态教师融合RGBI影像与LiDAR数据，训练仅使用RGBI的SegFormer学生模型来预测森林结构指标，在零样本CHM预测上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR虽然能提供高分辨率森林结构数据，但成本高、采集频率低，限制了其在碳监测、生物多样性评估等应用中的可扩展性。需要开发仅使用更易获取的RGBI影像就能准确预测森林结构指标的方法。

Method: 提出FSKD知识蒸馏框架：多模态教师模型通过交叉注意力融合RGBI影像与LiDAR衍生的平面指标和垂直剖面，训练仅使用RGBI输入的SegFormer学生模型来复现这些输出。在德国萨克森州384平方公里森林数据上训练，使用20厘米地面采样距离数据。

Result: 学生模型在8个地理分布不同的测试区域上实现零样本SOTA CHM性能：MedAE 4.17米，R²=0.51，IoU 0.87，MAE（5.81米）比HRCHM/DAC基线提升29-46%。多模态融合比仅使用RGBI训练提升10-26%性能。方法能同时预测CHM、PAI和FHD，且在时间不匹配（冬季LiDAR、夏季RGBI）情况下仍有效。

Conclusion: FSKD框架通过知识蒸馏实现了仅使用RGBI影像就能准确预测森林结构指标，突破了严格同步采集的限制，为德国数字孪生和国家数字正射影像计划等应用提供了可扩展的20厘米分辨率操作监测方案。
...
TL;DR: 提出GardenDesigner框架，通过编码江南园林美学原则并集成基于程序化建模的智能体链，实现江南园林的自动生成，支持非专家用户通过文本输入在1分钟内构建园林。


<details>
  <summary>Details</summary>
Motivation: 江南园林作为重要数字资产在影视游戏和数字旅游中潜力巨大，但传统手工建模依赖专家经验且耗时，需要自动化解决方案来降低创作门槛。

Method: 提出GardenDesigner框架：1) 编码江南园林建造的美学原则；2) 基于程序化建模构建智能体链，包括地形分布、道路生成、资产选择和布局优化智能体；3) 引入GardenVerse知识库增强资产布置；4) 在Unity中开发交互界面和工具。

Result: 实验和人工评估表明，GardenDesigner能够生成多样化且美观的江南园林，非专家用户可通过文本输入在1分钟内完成园林构建，项目页面已公开。

Conclusion: GardenDesigner成功实现了江南园林的自动化生成，通过编码美学原则和智能体链降低了创作门槛，为数字资产创作提供了高效解决方案。
...
TL;DR: 提出一种利用轮式里程计实现时间一致性的单目深度估计框架，通过光流三角化估计相机位姿和稀疏深度，使用递归贝叶斯估计度量尺度，结合预训练深度基础模型实现稳定深度预测。
...
Method: 利用移动机器人的轮式里程计，通过连续帧间的光流进行三角化，估计相机位姿和稀疏深度。使用稀疏深度更新递归贝叶斯估计的度量尺度，然后将该尺度应用于预训练深度估计基础模型预测的相对深度进行重缩放。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，展示了鲁棒且准确的深度估计性能，解决了时间一致性问题。

Conclusion: 提出的基于轮式里程计的一致性感知单目深度估计框架能够实现稳定、连贯的深度预测，解决了现有方法的时间不一致问题，适用于自动驾驶和移动机器人应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于群体的策略优化方法在处理视频问答时，当回答具有相似正确性时会出现奖励方差低的问题，导致优势估计弱或不稳定，限制了强化学习在视频推理任务中的效果。

Method: STRIVE通过构建输入视频的多个时空变体，在文本生成和视觉变体之间进行联合归一化。引入重要性感知采样机制，优先选择与问题最相关的帧，同时保持时间覆盖。通过结构化视觉扰动扩展群体比较，丰富奖励信号。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上，相比强大的强化学习基线，在多个大型多模态模型中均取得了一致的改进。

Conclusion: 结构化时空探索作为一种原则性机制，能够稳定多模态强化学习并提高视频推理性能。STRIVE通过互补的视觉视角促进鲁棒推理，而不是过度拟合单一时空配置。
...
TL;DR: SafeRoPE：针对MMDiT的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间，利用RoPE扰动抑制不安全内容


<details>
  <summary>Details</summary>
Motivation: 基于整流流变换器（如SD3、FLUX）的文本到图像模型虽然生成保真度高，但仍易受不安全语义影响，特别是在多令牌交互触发时。现有缓解方法主要依赖微调或注意力调制进行概念遗忘，但计算开销大且专为U-Net去噪器设计，难以直接适配变换器扩散模型（如MMDiT）。

Method: 深入分析MMDiT注意力机制，发现不安全语义集中在头部级别的可解释低维子空间中，有限的安全关键头部负责不安全特征提取。提出SafeRoPE框架：1）通过分解安全关键头部中的不安全嵌入构建头部不安全子空间，计算潜在风险分数；2）引入头部RoPE扰动，在不降低良性内容或图像质量的情况下抑制不安全语义；3）结合头部LRS和RoPE扰动对查询和关键向量嵌入进行风险特定的头部旋转。

Result: 广泛实验表明，SafeRoPE在平衡有效有害内容缓解和效用保持方面达到最先进性能，实现MMDiT的安全生成。

Conclusion: SafeRoPE是一种轻量级、细粒度的安全生成框架，能够精确抑制不安全输出同时保持生成保真度，为变换器扩散模型提供了有效的安全解决方案。
...
TL;DR: 本文提出了一种随机标签桥接训练方法，通过添加桥接训练阶段使大语言模型参数适应视觉任务，无需人工标注，并发现部分桥接训练通常更有效。


<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例存在显著差异，使得跨模态（语言和视觉）适应比跨域适应更具挑战性。先前研究多关注跨域迁移而非桥接语言和视觉模态，认为语言预训练模型由于参数空间差异不适合下游视觉任务。

Method: 提出随机标签桥接训练方法，作为模态适应学习器，无需人工标注。该方法通过桥接训练阶段对齐大语言模型参数与视觉任务，并探索部分桥接训练策略，保留某些具有强基础特性的层不变。

Result: 研究表明桥接训练阶段能有效对齐LLM参数与视觉任务，随机标签桥接训练方法简单而强大。更重要的是，部分桥接训练通常更有利，因为LLM中的某些层展现出强基础特性，即使不针对视觉任务进行微调也保持有益。

Conclusion: 这一发现为直接在视觉模型中利用语言预训练参数开辟了新途径，并突显了部分桥接训练作为跨模态适应实用路径的潜力，挑战了语言预训练模型不适合视觉任务的假设。
...
TL;DR: 提出一种基于排序的半监督域适应方法，利用类别顺序信息解决医学图像严重程度分类中的域偏移问题


<details>
  <summary>Details</summary>
Motivation: 现有半监督域适应方法在处理具有自然顺序标签的严重程度分类时效果不佳，因为类别边界不清晰且存在域偏移问题

Method: 提出跨域排序和连续分布对齐：1) 跨域排序通过类别顺序对跨域样本对进行排序；2) 连续分布对齐对齐源域和目标域的排序分数分布

Result: 在溃疡性结肠炎和糖尿病视网膜病变分类任务上验证了方法的有效性，成功对齐了类别特定的排序分数分布

Conclusion: 利用类别顺序信息的排序方法能有效解决医学图像严重程度分类中的半监督域适应问题
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在处理大量输入视频令牌时计算负担重，现有方法主要在输入级别剪枝，忽略了视频和LLM内部固有的信息结构。

Method: 提出三级分层剪枝框架：1)段级-视频令牌先按时间分段和空间合并；2)帧级-同一段内相似帧联合剪枝以保持多样性；3)层级-随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准测试中，仅保留30%令牌的情况下，HieraVid达到新的SOTA性能，分别保持LLaVA-Video-7B和LLaVA-OneVision-7B超过98%和99%的性能。

Conclusion: HieraVid通过利用视频的段-帧结构和LLM内部信息传播特性，有效减少计算负担，在保持高性能的同时显著提升视频大语言模型的部署效率。
...
TL;DR: 本文提出了一种纹理感知的Transformer模型，用于纹理3D网格的语义分割，通过结合几何和纹理信息，在SUM基准和新文化遗产数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 纹理3D网格同时包含几何、拓扑和外观信息，但其不规则结构给深度学习语义分割带来挑战。现有方法通常忽略网格提供的丰富纹理信息，需要一种能够有效利用纹理和几何信息的解决方案。

Method: 提出纹理感知Transformer：1）从每个网格面关联的原始像素学习；2）采用新的分层学习方案进行多尺度特征聚合；3）纹理分支将所有面级像素汇总为可学习token；4）与几何描述符融合后通过Two-Stage Transformer Blocks处理，支持局部和全局信息流。

Result: 在Semantic Urban Meshes基准上达到81.9% mF1和94.3% OA，在新文化遗产数据集（带损坏类型三角形级标注的纹理屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 纹理感知Transformer有效结合了几何和纹理信息，通过分层学习和两阶段Transformer块实现了对纹理3D网格的高质量语义分割，在城市和文化遗产领域都表现出优越性能。
...
TL;DR: PI-VQ提出位置无关的离散表示学习，通过置换不变约束使隐码捕获全局语义特征，无需学习先验即可直接插值，并引入匹配量化提升瓶颈容量。


<details>
  <summary>Details</summary>
Motivation: 传统VQ方法学习的离散表示具有位置依赖性，隐码空间排列且上下文纠缠，需要自回归或扩散先验建模依赖关系。本文探讨位置信息是否对空间对齐数据的离散表示必要，旨在消除位置依赖性。

Method: 提出置换不变向量量化自编码器(PI-VQ)，约束隐码不携带位置信息；引入基于最优二分图匹配的匹配量化算法，提升瓶颈容量3.5倍；利用学习码的组合结构实现基于插值的单前向传播采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上获得竞争力的精度、密度和覆盖度指标；隐码捕获全局语义特征，支持无需学习先验的直接图像插值；匹配量化显著提升信息容量。

Conclusion: 位置无关表示在可分离性和可解释性方面存在权衡，但能学习更具语义性的离散表示，支持高效采样和插值，为未来工作指明方向。
...
TL;DR: FaCT-GS是一个用于快速灵活CT重建的框架，通过优化体素化和光栅化流程，相比现有GS方法实现了4-13倍的速度提升，并支持从现有体积快速拟合高斯分布作为先验。


<details>
  <summary>Details</summary>
Motivation: 虽然高斯泼溅(GS)已成为图像渲染的主导技术并应用于CT重建，但其优势不足以推动从成熟重建算法的过渡。本文旨在解决GS方法在CT重建中的主要限制，特别是速度和可扩展性问题。

Method: FaCT-GS通过深入优化体素化和光栅化流程，实现了显著的速度提升。改进的体素化能够快速将高斯分布拟合到现有体积，可作为重建的先验或压缩表示。该方法在标准512x512投影上比现有GS CT重建快4倍以上，在2k投影上快13倍以上。

Result: FaCT-GS在速度和可扩展性方面显著优于现有GS CT重建方法。在标准512x512投影上实现4倍以上加速，在2k投影上实现13倍以上加速。改进的体素化支持快速拟合高斯分布到现有体积，可作为先验或压缩表示。

Conclusion: FaCT-GS通过优化体素化和光栅化流程，解决了GS在CT重建中的速度和可扩展性限制，实现了显著的速度提升，并提供了灵活的先验集成能力，为CT重建提供了更高效实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管现代视觉语言模型在语义任务上表现出色，但它们在基本几何变换下的脆弱性尚未得到充分研究。本文旨在揭示VLMs在空间不变性和等变性方面的系统性缺陷，这些缺陷影响了它们在简单变换下可靠识别物体的能力。

Method: 通过系统评估方法，在不同视觉领域（符号草图、自然照片、抽象艺术）测试VLMs对基本几何变换（旋转、缩放、恒等变换）的鲁棒性。研究涵盖了多种架构、模型容量和提示策略。

Result: VLMs在语义内容稀疏时性能急剧下降，对基本几何变换缺乏鲁棒性。这种缺陷在不同架构、模型容量和提示策略中普遍存在，表明当前VLMs在空间推理方面存在系统性弱点。

Conclusion: 当前VLMs在语义理解与空间推理之间存在系统性差距，缺乏对基本几何变换的鲁棒性。这凸显了未来多模态系统需要更强的几何基础，以实现更全面的视觉理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有时序动作分割方法依赖复杂架构，不利于实际部署，需要更轻量化的解决方案

Method: 提出双损失训练框架：边界回归损失通过单通道边界预测提升时序定位精度；CDF段级正则化损失通过匹配预测段与真实段的累积分布来增强段内结构一致性

Result: 在三个基准数据集上，该方法提高了段级一致性和边界质量，在三种不同模型上获得了更高的F1和Edit分数，帧级精度基本保持不变

Conclusion: 通过简单的损失设计而非复杂架构或推理时优化，可以实现精确的时序动作分割，框架具有架构无关性，可集成到现有TAS模型中
...
Conclusion: MAR-MAER通过结合度量感知嵌入正则化和概率潜在建模，有效解决了自回归文本到图像生成中的质量控制和模糊提示处理问题，在人类评估和自动度量上都验证了其优越性能。
...
TL;DR: A3R：一种基于智能体跨维度证据获取的3D高斯场景可承受性推理框架，通过序列化证据收集逐步减少歧义，优于静态单次预测方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D高斯场景中的可承受性推理视为静态观察下的单次预测问题，假设已有足够证据。但在复杂3D场景中，许多失败案例源于固定观察下任务相关证据的不完整性，而非预测能力不足。

Method: 将细粒度可承受性推理重新定义为序列化证据获取过程，通过互补的3D几何和2D语义证据逐步减少歧义。提出A3R框架，使基于MLLM的策略能够迭代选择证据获取动作，并通过跨维度证据获取更新可承受性信念。引入GRPO-based策略学习方法优化序列决策。

Result: 在场景级基准测试上的广泛实验表明，A3R始终优于静态单次预测基线方法，证明了智能体跨维度证据获取在复杂3D高斯场景中细粒度可承受性推理方面的优势。

Conclusion: 通过将可承受性推理重新定义为序列化证据获取过程，并采用智能体跨维度证据获取框架，能够有效解决复杂3D场景中因证据不完整导致的推理失败问题，显著提升推理准确性和效率。
...
Method: 提出基于图的空间分布优化框架：1）基于证据下界（ELBO）的自适应致密化策略自动控制致密化过程；2）透明度感知的渐进式剪枝策略动态移除低透明度高斯点；3）基于图的特征编码模块通过特征引导的点位移调整空间分布

Result: GS^2在仅使用约12.5%高斯点的情况下，相比原始3DGS实现了更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑的高斯表示

Conclusion: 通过图基空间分布优化，GS^2成功解决了3DGS内存消耗大的问题，在保持高质量渲染的同时显著减少高斯点数量，为实际应用提供了更实用的解决方案
...
TL;DR: 该论文系统研究了仅使用自然语言提示即可绕过文本到图像生成模型安全过滤器的低努力越狱攻击，提出了视觉越狱技术的分类法，并在多个最先进系统中验证了攻击有效性，成功率高达74.47%。


<details>
  <summary>Details</summary>
Motivation: 尽管文本到图像生成模型已部署安全过滤器和审核管道来防止滥用，但现有系统仍存在漏洞。研究旨在揭示仅通过自然语言提示就能绕过安全机制的低努力攻击方法，暴露当前安全防护的不足。

Method: 提出系统性的提示策略研究，无需模型访问、优化或对抗训练。引入视觉越狱技术分类法，包括艺术重构、材料替换、伪教育框架、生活方式美学伪装和模糊动作替换等策略，通过将不安全意图隐藏在良性语义上下文中来利用提示审核和视觉安全过滤的弱点。

Result: 在多个最先进的文本到图像系统中评估这些攻击，发现简单的语言修改能可靠地规避现有安全防护并生成受限图像。所有测试模型和攻击类别的攻击成功率（ASR）高达74.47%。

Conclusion: 研究结果突显了表面级提示过滤与检测生成媒体系统中对抗意图所需语义理解之间的关键差距。现有安全机制无法有效识别隐藏在良性上下文中的恶意意图，需要更深入的内容理解能力来应对此类越狱攻击。
...
TL;DR: ProVG：一种新颖的遥感视觉定位框架，通过解耦语言表达为全局上下文、空间关系和对象属性，采用渐进式跨模态调制器实现从粗到细的视觉-语言对齐，显著提升了定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法主要依赖句子级的视觉-语言对齐，难以充分利用细粒度语言线索（如空间关系和对象属性），而这些线索对于区分相似特征的对象至关重要。不同定位阶段这些线索应发挥不同作用，需要相应利用以提供更明确的指导。
...
TL;DR: SHARC框架通过球谐函数表示距离场合成任意拓扑形状，使用最优放置的参考点最大化表面细节学习，在重建精度和时间效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有形状合成方法在保持几何保真度和计算效率方面存在不足，需要一种能够处理任意拓扑结构（genus-agnostic）且能有效学习表面细节的方法。

Method: 1. 在表面内部体积中优化放置参考点，最大化稀疏性、中心性和表面可见性；2. 通过光线投射采样可见距离场；3. 使用快速球谐变换计算SH系数；4. 应用可配置低通滤波器增强几何保真度；5. 基于邻近性使用局部一致性约束进行细化。

Result: SHARC在重建精度和时间效率方面均优于现有最先进方法，同时保持模型简洁性。

Conclusion: SHARC提供了一种有效的形状合成框架，通过球谐函数表示距离场和优化参考点放置，实现了高精度、高效率的形状重建，适用于任意拓扑结构。
...
<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法在保持时间稳定性和空间细节之间存在矛盾：要么侧重于逐帧增强但时间建模有限，要么依赖繁重的时空聚合而牺牲高频细节
...
Conclusion: FTPFusion通过频率分解、稀疏跨模态交互和时序扰动策略，有效解决了红外与可见光视频融合中的时空稳定性与细节保持难题，为智能监控应用提供了高质量的融合解决方案
...
Method: 1) 将ResNet中的卷积替换为KAN卷积，实现自适应特征提取；2) 使用Gram多项式作为激活函数，适合SAR数据捕捉复杂非线性关系；3) 采用参数共享策略，每个核在通道间共享参数，在保留独特特征的同时减少参数和FLOPs。

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率。在1024×1024尺寸的MSTAR数据集上，相比VGG16减少82.90倍FLOPs和163.78倍参数。

Conclusion: Light-ResKAN模型在SAR图像识别中实现了精度与效率的良好平衡，为边缘设备上的SAR图像识别提供了高效解决方案。
...
TL;DR: 提出了一种基于几何驱动的多光点检测与匹配管道，将角膜反射光点视为结构化星座而非独立斑点，提高眼动追踪系统的可复现性。


<details>
  <summary>Details</summary>
Motivation: 角膜反射光点检测在瞳孔-角膜反射眼动追踪中很重要，但实际应用中通常作为启发式方法嵌入在大型系统中，导致不同硬件设置下的可复现性差。

Method: 采用2D几何驱动的星座式管道，提出相似性-布局对齐（SLA）程序，将星座匹配技术适配到多LED眼动追踪的特定约束中。框架包含控制性过检测、自适应候选回退、外观感知评分和可选语义布局先验，同时保持检测与对应关系明确分离。

Result: 在公开的多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系。

Conclusion: 该框架提高了多光点检测与匹配的可复现性和透明度，通过发布代码、预设和评估脚本支持透明复制、比较和数据集标注。
...
TL;DR: KnowMVG：一种用于医学视觉定位的知识先验和全局-局部注意力增强框架，通过知识增强提示和全局-局部注意力机制提高VLMs的空间定位精度
...
Conclusion: KnowMVG通过显式增强解码过程中的空间感知能力，有效桥接了高层语义理解和细粒度视觉感知，无需额外文本推理开销，显著提升了医学视觉定位的精度
...
TL;DR: 该研究探索了直接从波束形成前的单天线距离-多普勒张量中学习空间结构的可行性，而非传统方法中先进行波束形成再应用学习模型。


<details>
  <summary>Details</summary>
Motivation: 传统汽车雷达感知流程通常先通过波束形成构建角度域表示，再应用学习模型。本研究旨在探索能否绕过显式的角度域构建和手工信号处理阶段，直接从波束形成前的单天线距离-多普勒测量中学习有意义的空间结构。

Method: 使用6发射×8接收（48个虚拟天线）的商用汽车雷达，采用A/B啁啾序列FMCW发射方案，其中有效发射孔径在啁啾间变化。构建双啁啾共享权重编码器，以端到端、完全数据驱动的方式处理波束形成前的单天线RD张量。使用鸟瞰图占用率作为几何探测指标，而非性能驱动目标。监督信号来自激光雷达，具有可见性感知和跨模态特性，通过射线可见性建模雷达视场和遮挡感知的激光雷达可观测性。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束形成前的单天线RD张量中学习空间结构，无需显式的角度域构建或手工信号处理阶段。发射配置对几何可恢复性有影响。

Conclusion: 空间结构可以直接从波束形成前的单天线距离-多普勒张量中学习，无需传统流程中的显式角度域构建或手工信号处理阶段，为汽车雷达感知提供了新的表示学习范式。
...
TL;DR: 论文提出了ECAC大规模早教图像描述基准和RSRS混合训练框架，开发了KinderMM-Cap-3B模型，显著提升了早教场景下的专业物体识别精度。
...
Method: 1) 构建ECAC基准：包含256,121张真实世界图像，配有专家级描述和细粒度标签，引入领域导向评估协议TTS；2) 提出RSRS混合训练框架：动态切换强化学习和监督微调，将零奖励困难样本重定向到监督微调；3) 开发KinderMM-Cap-3B：基于ECAC和RSRS训练的领域适应多模态大语言模型。

Result: KinderMM-Cap-3B模型在TTS指标上达到51.06，显著优于现有最先进基线，同时保持优越的描述质量，证明了其在专业教育应用中的潜力。

Conclusion: ECAC基准和RSRS框架有效解决了早教图像描述的关键挑战，KinderMM-Cap-3B模型在专业物体识别精度和描述质量方面均取得显著提升，为专业教育应用提供了有力工具。
...
TL;DR: 本文提出AMIMV方法解决医学图像分类中的数据稀缺和类别不平衡问题，评估了8种SSL方法在11个MedMNIST数据集上的表现，在长尾分布和有限监督下取得了性能提升。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临两个主要问题：1）缺乏大量标注训练数据；2）数据不平衡，常见类别数据丰富而罕见类别数据稀缺。自监督学习（SSL）方法在一定程度上解决了第一个问题，但SSL方法对不平衡数据的鲁棒性在医学图像分类领域很少被研究。

Method: 1）将先前提出的MIMV方法扩展为AMIMV，采用新的增强策略构建非对称多图像多视图对，同时解决数据稀缺和数据集不平衡问题；2）进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性；3）在11个MedMNIST数据集上评估8种代表性SSL方法在长尾分布和有限监督下的表现。

Result: 实验结果显示，在MedMNIST数据集上，AMIMV方法在retinaMNIST上提升4.25%，在tissueMNIST上提升1.88%，在DermaMNIST上提升3.1%。

Conclusion: AMIMV方法能有效处理医学图像分类中的数据稀缺和类别不平衡问题，在多个医学图像数据集上表现出优于现有SSL方法的性能，为不平衡医学图像分析提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法多为静态图像融合设计，无法有效处理视频中的帧间运动；现有视频融合方法虽然通过跨帧交互提升时间一致性，但计算成本高

Method: 提出运动感知稀疏交互机制：利用光流识别多模态序列中的动态区域，自适应地将计算密集的跨模态注意力分配到这些稀疏区域；对静态背景区域使用轻量级弱交互模块；通过解耦动态与静态区域处理

Result: 在多个红外与可见光视频基准测试中达到最先进性能，在640×480分辨率下实现14.16 FPS的推理速度

Conclusion: MAVFusion通过运动感知稀疏交互机制，在保持时间一致性和细粒度细节的同时显著加速推理，为红外与可见光视频融合提供了高效解决方案
...
TL;DR: 基于nnU-Net v2框架的深度学习模型，利用多模态mpMRI数据（T2加权、DWI、ADC）进行前列腺自动分割，在PI-CAI数据集上训练，在外部验证中表现优于通用分割工具。


<details>
  <summary>Details</summary>
Motivation: 前列腺在mpMRI中的精确分割对于临床和研究应用至关重要，但手动分割耗时且存在观察者间差异，通用分割工具在前列腺特定任务中精度不足。

Method: 使用nnU-Net v2框架构建深度学习模型，整合多模态mpMRI数据（T2加权成像、扩散加权成像DWI、表观扩散系数ADC图），在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证（54例患者）。

Result: 交叉验证中平均Dice分数为0.96±0.00，外部测试集上为0.82；相比之下，通用分割工具TotalSegmentator的Dice分数仅为0.15，主要由于腺体欠分割。

Conclusion: 任务特定的多模态分割策略对于前列腺分割至关重要，所提方法展现出良好的泛化能力，适合集成到临床研究流程中；模型已容器化并提供为即用推理工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要自我定位的个性化问答任务中存在能力空白，缺乏专门评估模型理解自我中心视频中相机佩戴者的数据集，这限制了模型在个性化辅助应用中的发展。

Method: 构建MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类问题。对多种MLLMs变体进行基准测试，包括开源vs专有、思考vs非思考、小vs大规模模型，并分析显式推理、模型缩放和证据提供对性能的影响。

Result: 顶级闭源和开源模型（GPT-5和Qwen3-VL）准确率仅约46%和36%，分别落后人类性能近40%和50%。显式推理和模型缩放未能带来一致改进，提供相关证据虽有帮助但增益随时间下降，表明模型在跟踪和记忆"我"及"我的过去"方面存在根本限制。

Conclusion: 自我定位和长期记忆是实现自我中心视频中个性化问答的关键能力，当前MLLMs在这方面存在显著不足。MyEgo数据集和分析为推动该领域发展提供了重要基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本描述的3D场景生成方法在短文本条件下存在物理合理性差和细节丰富度不足的问题，主要因为它们过度依赖显式的组合对象语义关系线索，需要增强3D推理能力特别是先验整合和空间锚定能力

Method: 提出SDesc3D框架：1）多视角场景先验增强，通过聚合多视角结构知识丰富欠指定的文本输入；2）功能感知布局锚定，利用区域功能锚定进行隐式空间锚定和分层布局推理；3）迭代反射-修正方案，通过自修正进行渐进式结构合理性优化

Result: 大量实验表明，该方法在短文本条件下的3D室内场景生成任务上优于现有方法

Conclusion: SDesc3D通过多视角结构先验和区域功能感知实现了在稀疏文本指导下的有效3D布局推理，解决了短文本条件下场景生成的物理合理性和细节丰富度问题
...
TL;DR: 该论文提出了交互式跟踪新范式，创建了首个大规模交互式跟踪基准InteractTrack，并开发了基于动态记忆机制的基线模型IMAT，以解决传统跟踪器缺乏人机交互能力的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要工作在非交互的"发射后不管"模式，无法适应需要人机协作的真实场景。传统跟踪器缺乏接受用户自然语言指令进行实时调整的能力，限制了其在实际应用中的实用性。

Method: 1) 创建InteractTrack基准：包含150个视频，带有密集边界框标注和时间戳语言指令；2) 设计全面的评估协议，评估了25个代表性跟踪器；3) 提出IMAT基线模型：采用动态记忆机制学习用户反馈并相应更新跟踪行为。

Result: 评估显示最先进的跟踪器在交互场景中表现失败，传统基准上的强性能无法迁移到交互场景。IMAT模型能够有效学习用户反馈并调整跟踪行为，为交互式跟踪建立了新的性能基准。

Conclusion: 该工作为开发更智能、自适应和协作的跟踪系统奠定了基础，弥合了自动感知与人类指导之间的差距。提出的基准、协议和基线模型推动了交互式跟踪研究的发展。
...
TL;DR: Curia-2是一个用于医学影像分析的基础模型，通过改进预训练策略和表示质量，首次将架构扩展到十亿参数级别的Vision Transformers，并在2D和3D基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 医学影像的快速增长给放射科医生带来了不可持续的工作负担，需要开发基础模型来减轻这一负担。虽然现有的基础模型在CT和MRI分析中显示出大规模预训练的优势，但在如何从复杂的放射学体积数据中学习方面仍有很大优化空间。

Method: 基于Curia框架，提出了Curia-2，改进了原始预训练策略和表示质量，更好地捕捉放射学数据的特性。首次将架构扩展到十亿参数级别的Vision Transformers，支持多模态CT和MRI。同时将CuriaBench扩展和重构为两个独立轨道：针对切片视觉模型的2D轨道和针对体积基准测试的3D轨道。

Result: Curia-2在视觉聚焦任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型竞争激烈。模型权重将公开以促进进一步研究。

Conclusion: Curia-2通过改进的预训练策略和表示学习，显著提升了医学影像基础模型的性能，为大规模多模态医学影像分析提供了有效的解决方案，并通过公开模型权重促进该领域的研究发展。
...
TL;DR: 该论文发现多模态大语言模型中的视觉注意力存在惯性现象，导致认知推理能力不足，并提出了一种无需训练的惯性感知视觉激励方法来缓解认知幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉注意力方面表现出明显的惯性特征，注意力一旦在早期解码步骤中稳定下来就基本保持静态，无法支持认知推理所需的组合性理解。现有的幻觉缓解方法主要针对感知幻觉（物体存在或属性），但对需要对象间关系推理的认知幻觉效果有限。

Method: 提出了无需训练的惯性感知视觉激励方法，通过将认知推理建模为视觉注意力的动态响应来打破惯性模式。具体包括：1）选择相对于历史注意力趋势动态出现的视觉标记，同时区分表现出惯性行为的标记；2）引入惯性感知惩罚机制，防止注意力过度集中并限制在局部区域的持久性。

Result: 广泛的实验表明，IVE方法在各种基础MLLM和多个幻觉基准测试中均有效，特别是在缓解认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，提出的IVE方法通过打破注意力惯性模式，有效提升了模型在组合性推理和认知幻觉缓解方面的性能。
...
TL;DR: Resonance4D：一种物理驱动的4D动态仿真框架，通过双域运动监督和全参数物理恢复，在单张消费级GPU上实现高保真4D仿真，显著降低计算成本和内存开销。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的4D动态仿真方法存在两个主要问题：1）运动监督依赖计算成本高昂的在线视频扩散或光流管道，其计算开销超过仿真器本身；2）仅优化部分材料参数简化了逆物理建模，限制了复杂材料和动态场景的真实性。

Method: Resonance4D将3D高斯泼溅与物质点法通过轻量级但物理表达性强的监督耦合。核心创新包括：1）双域运动监督（DMS），结合空间结构一致性（局部变形）和频域谱一致性（振荡和全局动态模式）；2）零样本文本提示分割与仿真引导初始化相结合，自动将高斯分解为对象部件级区域，支持全材料参数的联合优化。

Result: 在合成和真实场景上的实验表明，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，使高保真物理驱动的4D仿真能够在单张消费级GPU上运行。

Conclusion: Resonance4D通过双域运动监督和全参数物理恢复，解决了现有4D动态仿真方法中计算成本高和物理建模简化的矛盾，实现了高效、高保真的物理驱动4D仿真，为复杂动态场景的建模提供了实用解决方案。
...
TL;DR: MTLSI-Net通过线性注意力实现多任务密集预测，以线性复杂度捕获跨任务交互，在NYUDv2和PASCAL-Context上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 传统自注意力在高分辨率特征上具有二次复杂度，难以有效捕获全局跨任务交互，限制了多任务密集预测的性能
...
Conclusion: MTLSI-Net通过线性注意力机制成功解决了多任务密集预测中跨任务交互的复杂度问题，在保持参数效率的同时实现了优异的性能
...
<details>
  <summary>Details</summary>
Motivation: 解决从仅有航拍图像生成地面级视图和一致3D场景模型的挑战，包括极端视角变化、缺失中间观测和大尺度变化。现有方法要么产生几何不一致的结果，要么依赖难以获取的多高度地面真值数据。

Method: 提出ProDiG框架：1）渐进式变换航拍3D表示到地面级保真度；2）几何感知因果注意力模块，将极线结构注入参考视图扩散；3）距离自适应高斯模块，根据相机距离动态调整高斯尺度和不透明度；4）合成中间高度视图并在每个阶段细化高斯表示。

Result: 在合成和真实世界数据集上的广泛实验表明，ProDiG能生成视觉逼真的地面级渲染和一致的3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式扩散引导框架成功解决了从航拍图像生成地面级视图的挑战，无需额外地面真值视角，实现了几何一致的高质量渲染和3D重建。
...
Method: 提出DR-Seg框架：1) 基于CLIP特征通道功能异质性的观察，将CLIP特征解耦为语义主导和结构主导子空间；2) 在DINO指导下进行针对性结构增强而不扭曲语言对齐语义；3) 先验驱动图校正模块在DINO指导下注入高保真结构先验形成精炼分支；4) 不确定性引导自适应融合模块动态集成精炼分支与原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征为语义和结构子空间，实现了针对性的结构增强，有效解决了遥感开放词汇语义分割中语言对齐识别与细粒度空间划分的平衡问题，在保持CLIP语义完整性的同时提升了边界划分能力。
...
TL;DR: LinkS²Bench：首个评估视觉语言模型在无人机-卫星协同空间智能的基准，包含17.9k高质量QA对，覆盖12个细粒度任务，揭示了动态跨视图对齐是当前VLM的主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 无人机与卫星的协同空间智能对应急响应和安全操作至关重要，但现有基准局限于孤立的无人机视频或静态卫星图像，无法评估动态局部到全局的空间映射能力，导致VLM在这方面的能力尚未得到充分探索。

Method: 构建LinkS²Bench基准，包含1,022分钟动态无人机视频和高分辨率卫星图像（覆盖200+ km²），通过LMM辅助流水线和人工标注创建17.9k高质量QA对，涵盖感知、定位、关系和推理四个维度的12个任务。提出跨视图对齐适配器来缓解对齐瓶颈。

Result: 评估18个代表性VLM显示与人类基线存在显著差距，准确跨视图动态对齐是关键瓶颈。跨视图对齐适配器显著提升模型性能，微调实验证明LinkS²Bench在推进VLM复杂空间推理适应方面的潜力。

Conclusion: LinkS²Bench填补了评估VLM在无人机-卫星协同空间智能方面的空白，揭示了动态跨视图对齐是主要挑战，提出的对齐适配器有效提升性能，为复杂空间推理的VLM发展提供了重要基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 自编码器在处理空间非均匀采样的图像时面临挑战，特别是在医学成像、生物学和物理学领域，信息丰富的模式往往只出现在特定图像坐标上，而背景在大多数样本中占据主导地位，导致重建偏向多数外观，丢失细粒度细节并产生模糊重建。

Method: 提出两个互补组件：(1) 基于自熵的损失函数，对统计上不常见的空间位置进行加权；(2) 样本传播，一种重放机制，在训练期间选择性地将难以重建的样本重新暴露给模型。

Result: 在模拟数据集（具有受控空间不平衡条件）和三个真实世界数据集（物理、生物和天文领域）中验证，该方法在各种重建指标上优于基线，特别是在空间不平衡分布下。
...
TL;DR: IndoorCrowd是一个用于室内人群检测、实例分割和多目标跟踪的多场景数据集，包含31个视频（9,913帧），涵盖四个校园场景，提供人工验证的实例分割掩码，并建立了检测、分割和跟踪的基准。
...
Method: 在四个校园位置（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频（9,913帧，5fps），提供人工验证的实例分割掩码。使用620帧控制子集评估SAM3、GroundingSAM和EfficientGroundingSAM三个基础模型自动标注器，使用Cohen's κ、AP、精度、召回率和掩码IoU指标。2,552帧子集支持MOTChallenge格式的多目标跟踪。使用YOLOv8n、YOLOv26n和RT-DETR-L结合ByteTrack、BoT-SORT和OC-SORT建立基准。

Result: 场景分析显示难度差异显著，受人群密度、尺度和遮挡影响：ACS-EC场景（79.3%密集帧，平均实例尺度60.8像素）最具挑战性。建立了检测、分割和跟踪的基准性能。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群分析数据的空白，提供了评估自动标注器和多目标跟踪算法的基准，揭示了不同室内场景的挑战性差异。
...
TL;DR: WISE提出了一种高效推理范式，通过"思考两次"原则：训练时生成结构化序列（简洁理由→最终答案→详细解释），推理时仅输出简洁理由，实现5倍推理长度压缩，在ReasonSeg基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维推理方法在语言引导分割任务中性能显著，但生成冗长理由导致计算成本过高，限制了实际应用。需要一种既能保持推理质量又能大幅减少计算开销的高效推理方法。

Method: WISE采用"思考两次"原则：训练时生成结构化序列（简洁理由→答案→详细解释），通过自回归条件约束确保简洁理由足以生成详细解释。使用自蒸馏目标联合奖励语义保真度和简洁性。推理时采用WISE-S策略，通过简洁性指令提示激活学习的简洁策略，省略详细解释。

Result: 在ReasonSeg基准上实现零样本SOTA性能（58.3 cIoU），同时将平均推理长度从112个token减少到23个token，压缩近5倍，显著提升计算效率。

Conclusion: WISE通过结构化序列生成和自蒸馏训练，实现了高效推理范式，在保持性能的同时大幅降低计算成本，为大型多模态模型的实际部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 构建高质量的多语言和非英语视觉语言模型面临的主要障碍是其他语言的视觉问答数据集在规模和领域覆盖上都很有限。英语可以通过聚合现有VQA资源构建大规模数据集，但这种策略不适用于日语等其他语言。
...
Conclusion: Jagle数据集为构建高质量日语视觉语言模型提供了有效解决方案，通过创新的数据生成策略克服了日语VQA数据稀缺的问题。该数据集、训练模型和代码已开源，促进可重复性和未来研究。
...
TL;DR: 提出一种全自动、可扩展的方法来系统评估虚拟人渲染流程中的肤色保真度，通过肤色提取、纹理重着色、实时渲染和定量色彩分析，发现提取策略存在表型依赖性，深色肤色的色度误差更高。


<details>
  <summary>Details</summary>
Motivation: 虚拟人渲染中准确再现肤色对于真实感、身份保持和公平性至关重要，但现有可访问的虚拟人创建流程依赖未经色彩校准的摄影输入，可能导致不一致性和偏见。

Method: 提出全自动工作流程，整合肤色与光照提取、纹理重着色、实时渲染和定量色彩分析。使用芝加哥面部数据库图像，比较基于脸颊区域采样和全脸分析多维掩码的肤色提取策略，结合TRUST框架进行光照隔离。提取的肤色应用于MetaHuman纹理并在多种光照配置下渲染，在CIELAB色彩空间中使用ΔE指标和个体类型学角度进行客观评估。

Result: 生成并分析了约19,848个渲染实例，结果显示提取策略存在表型依赖性行为，深色肤色始终表现出更高的色度误差。

Conclusion: 提出的方法无需人工干预，计算成本低，支持大规模评估，揭示了肤色提取策略的表型依赖性和深色肤色更高的色度误差，为虚拟人渲染中的肤色保真度评估提供了系统框架。
...
TL;DR: 论文提出Multi-Object Affordance Grounding under Intent-Driven Instructions新任务，要求在多物体点云中基于隐式语言意图预测正确物体的可操作性掩码，并构建CompassAD基准和CompassNet框架解决混淆物体对问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D可操作性方法主要评估孤立单物体，且常依赖查询中显式类别名称，忽略了真实场景中多个物体共享相同可操作性但只有特定物体适合给定任务上下文的情况（混淆对）。需要解决在多物体杂乱场景中基于隐式意图进行可操作性定位的挑战。

Method: 提出CompassNet框架，包含两个专用模块：1) Instance-bounded Cross Injection (ICI)：约束语言-几何对齐在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR)：在几何组和点级别强制区分，增强目标表面与混淆表面之间的差异。

Result: 构建CompassAD基准，包含30个混淆物体对、16种可操作性类型、6,422个场景和88K+查询-答案对。实验表明在可见和未见查询上均取得最先进结果，在机器人机械臂上的部署证实了在真实世界混淆多物体场景中有效抓取的迁移能力。

Conclusion: 该工作形式化了意图驱动的多物体可操作性定位新任务，通过CompassAD基准和CompassNet框架有效解决了混淆物体对问题，实现了从语言意图到3D几何的精确可操作性映射，并在真实机器人任务中验证了实用性。
...
TL;DR: PLUME提出了一种潜在推理框架，用连续的潜在状态自回归展开替代显式思维链，显著提升多模态嵌入模型的推理效率和性能


<details>
  <summary>Details</summary>
Motivation: 现有通用多模态嵌入方法通过生成显式思维链来提升复杂查询意图推理，但存在推理开销大、多模态证据被压缩为文本瓶颈的问题

Method: PLUME采用潜在推理框架：1) 用短序列连续潜在状态自回归展开替代显式思维链；2) 引入语义锚点引导的转换适配器，在固定计算预算下引导不同推理轨迹；3) 采用渐进式显式到潜在课程学习，逐步将显式推理行为转移到隐藏状态计算

Result: 在78个任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，推理步骤从数百个生成token减少到少于10个潜在步骤，推理速度提升30倍以上，特别适用于视频和视觉文档检索等证据密集、结构复杂的场景

Conclusion: 结构化潜在计算能够保留中间推理的优势，同时避免显式理由生成的开销，为实际检索系统提供了更强、更高效的范式
...
TL;DR: LatentUM是一种新型统一模型，通过共享语义潜在空间表示所有模态，无需像素空间中介，实现跨模态推理和生成，在视觉空间规划、自反思生成和世界建模方面取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解和生成之间的桥梁，这种方法既低效又无效。需要一种能直接在语义层面进行跨模态推理和生成的统一模型。

Method: 提出LatentUM模型，将所有模态表示在共享的语义潜在空间中，消除了视觉理解和生成之间对像素空间中介的需求。这种设计自然支持灵活的跨模态推理和生成。

Result: LatentUM在视觉空间规划基准上达到最先进性能，通过自反思推动视觉生成的极限，并支持在共享语义潜在空间中预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间的统一模型设计不仅提高了计算效率，还显著减轻了编解码器偏差，增强了跨模态对齐，为跨模态推理和生成提供了更有效的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法通常假设固定标签空间，无法响应任意用户定义查询。开放词汇变化检测(OVCD)虽能查询特定概念的变化掩码，但在完全训练无关设置下，密集概念响应直接跨时间比较存在困难：外观变化、弱跨概念竞争以及许多土地覆盖类别的空间连续性常导致噪声、碎片化和语义不可靠的变化证据。

Method: 提出一致性正则化开放词汇变化检测(CoRegOVCD)框架，包含三个核心组件：1) 竞争性后验校准(CPC)和语义后验差异(SPD)将原始概念响应转换为竞争感知的查询概念后验并量化跨时间差异；2) 几何-标记一致性门(GeoGate)通过几何感知结构验证抑制不受支持的响应；3) 区域共识差异(RCD)通过区域共识提高空间一致性。所有组件均无需训练。

Result: 在四个基准测试（涵盖建筑导向和多类设置）中，CoRegOVCD相比先前最强的训练无关基线持续提升2.24到4.98个F1_C点，在SECOND数据集上达到六类平均47.50% F1_C。

Conclusion: CoRegOVCD通过一致性正则化有效解决了开放词汇变化检测中的噪声和语义不可靠问题，无需训练即可显著提升检测性能，为遥感变化检测提供了更灵活和可靠的解决方案。
...
TL;DR: 研究发现面部动作单元检测中标准的主体排他交叉验证存在显著随机方差，在BP4D+数据集上产生±0.065的F1噪声，且低频率动作单元波动更大；留一数据集交叉验证能提供更稳定可解释的结果


<details>
  <summary>Details</summary>
Motivation: 当前面部动作单元检测研究中，主体排他交叉验证是标准评估协议，但报告的改进往往很小。研究发现交叉验证本身引入了可测量的随机方差，这可能掩盖了真实的模型性能差异

Method: 1) 在BP4D+数据集上进行重复3折主体排他分割，量化交叉验证的随机方差；2) 使用留一数据集交叉验证协议在五个动作单元数据集上评估跨数据集鲁棒性

Result: 1) 重复3折主体排他分割产生±0.065的平均F1噪声地板，低频率动作单元波动更大；2) F1等操作点指标比AUC等阈值无关指标波动更大；3) 不同折分配下模型排名可能改变；4) 留一数据集交叉验证能揭示单数据集交叉验证中不可见的领域级不稳定性

Conclusion: 交叉验证中报告的改进可能落在协议方差范围内，留一数据集交叉验证能提供更稳定和可解释的发现，建议在动作单元检测研究中采用更严格的评估协议
...
<details>
  <summary>Details</summary>
Motivation: 图像合成涉及将前景对象插入背景并生成环境一致的效果（如阴影和反射）。虽然阴影生成已被广泛研究，但反射生成在很大程度上仍未得到充分探索。本工作专注于反射生成，旨在填补这一研究空白。
...
Conclusion: 该研究在反射生成领域取得了重要进展，通过创新的方法设计和首个大规模数据集，实现了高质量的反射合成效果，为图像合成的完整性提供了重要支持。
...
TL;DR: ViT-Explainer：一个基于Web的交互式系统，用于可视化Vision Transformer的完整推理过程，从图像分块到最终分类，提供动画演示、注意力覆盖和视觉适应的Logit Lens等功能。


<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但理解这些模型在视觉场景中的工作原理仍然具有挑战性。现有可解释性工具通常关注孤立组件或面向专家的分析，缺乏对完整推理流程的引导式端到端理解。

Method: 开发了ViT-Explainer系统，这是一个基于Web的交互式可视化工具，集成了动画演示、分块级注意力覆盖和视觉适应的Logit Lens。系统提供引导探索和自由探索两种模式，支持从图像分块到最终分类的完整推理流程可视化。

Result: 通过6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能够帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了现有可解释性工具在引导式端到端理解Vision Transformer推理流程方面的空白，为模型理解提供了集成可视化工具。
...
TL;DR: UniDriveVLA通过专家解耦的混合Transformer架构解决了自动驾驶VLA模型中空间感知与语义推理的冲突，在保持推理能力的同时提升空间感知性能。
...
Result: 在nuScenes开环评估和Bench2Drive闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛任务上表现出色，展示了作为自动驾驶统一模型的广泛适用性。
...
TL;DR: SCALE是一个轻量级确定性框架，通过语义和置信度感知的列表式能量排序解决零样本骨架动作识别问题，避免显式的骨架-文本对齐，在NTU数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，当动作名称无法充分描述细粒度动态或未见类别语义混淆时，这种方法较为脆弱。需要一种更鲁棒的方法来处理语义模糊和类别混淆问题。

Method: 提出SCALE框架，将ZSAR建模为类条件能量排序问题。构建文本条件变分自编码器，冻结文本表示参数化潜在先验和解码器，实现未见类别的似然评估。引入语义和置信度感知的列表式能量损失，强调语义相似的困难负样本，结合后验不确定性调整决策边界和重加权模糊训练实例。使用潜在原型对比目标对齐后验均值与文本导出的潜在原型，改善语义组织和类别可分性。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE持续优于先前的VAE和对齐基线方法，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过能量排序框架有效解决了零样本骨架动作识别中的语义模糊和类别混淆问题，避免了脆弱的显式对齐，提供了一种轻量级且确定性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，因为其固定的预训练分辨率和固有的粗粒度补丁级表示。在密集预测场景（如开放词汇分割）中，高分辨率输入对像素级推理至关重要，但现有滑动窗口方法计算成本高昂。
...
Conclusion: SPAR通过知识蒸馏实现了高效的高分辨率密集特征提取，解决了ViT在细粒度空间理解任务中的局限性，为密集预测任务提供了计算效率更高的解决方案。
...
TL;DR: 提出一种基于推理时引导的文本到图像生成安全控制框架，利用冻结预训练基础模型的梯度反馈进行引导，无需修改底层生成器，实现了模块化、无需训练的安全控制。


<details>
  <summary>Details</summary>
Motivation: 现有安全方法通常依赖于模型微调或精选数据集，这会降低生成质量或限制可扩展性。需要一种无需修改生成器、能保持高质量且可扩展的安全控制方法。

Method: 提出推理时引导框架，利用冻结预训练视觉语言基础模型的丰富语义表示作为现成的监督信号。通过在每个采样步骤注入基于干净潜在估计的梯度反馈，将安全引导表述为基于能量的采样问题。该方法兼容扩散模型和流匹配模型。

Result: 在NSFW红队基准测试中表现出最先进的鲁棒性，能有效进行多目标引导，同时在良性非目标提示上保持高生成质量。框架为利用基础模型作为语义能量估计器提供了原则性方法。

Conclusion: 该框架实现了可靠且可扩展的文本到图像生成安全控制，通过利用基础模型的语义表示作为推理时引导信号，提供了一种模块化、无需训练的安全控制解决方案。
...
TL;DR: Omni123是一个3D原生基础模型，在单一自回归框架中统一了文本到2D和文本到3D生成，利用跨模态一致性作为隐式结构约束来改善3D表示。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在文本和图像理解与生成方面表现优异，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺导致3D合成约束不足。现有方法通常依赖间接流程，先在2D中编辑再通过优化提升到3D，牺牲了几何一致性。

Method: 提出Omni123模型，将文本、图像和3D表示为共享序列空间中的离散token。采用交错X-to-X训练范式，在异构配对数据集上协调多样跨模态任务，无需完全对齐的文本-图像-3D三元组。通过自回归序列中的语义-视觉-几何循环（如文本→图像→3D→图像），联合强制执行语义对齐、外观保真度和多视图几何一致性。
...
TL;DR: 提出一种基于概率度量表示和切片Wasserstein距离的快速表面配准方法，使用AdamFlow优化算法在概率空间最小化距离，实现高效鲁棒的表面配准。
...
Method: 将表面网格表示为概率度量，将表面配准转化为分布优化问题。使用计算复杂度为对数线性的切片Wasserstein距离度量网格间差异。提出AdamFlow优化方法，将Adam优化从欧几里得空间推广到概率空间以最小化切片Wasserstein距离。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长处理对象"背后"的内容修复和外观层面的伪影（如阴影、反射），但当移除的对象与其他对象有更显著的物理交互（如碰撞）时，现有模型无法修正这些交互，产生不合理的物理结果。

Method: 1. 使用Kubric和HUMOTO生成新的配对数据集，包含需要改变下游物理交互的反事实对象移除场景；2. 推理时，使用视觉语言模型识别受移除对象影响的场景区域；3. 使用视频扩散模型，以识别区域为指导，生成物理一致的反事实结果。

Result: 在合成和真实数据上的实验表明，与现有视频对象移除方法相比，VOID方法在对象移除后能更好地保持一致的场景动态，产生更物理合理的结果。

Conclusion: VOID框架通过高级因果推理，使视频编辑模型成为更好的世界模拟器，为处理复杂物理交互场景的视频对象移除提供了有效解决方案。
...
TL;DR: 提出场景化视觉定位新任务，引入RSC基准和ScenGround课程学习方法，揭示现有模型在基于角色、意图和关系推理方面的系统性缺陷


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型可通过匹配显著命名类别获得成功。需要探索更具挑战性的场景化视觉定位，要求根据角色、意图和关系上下文进行推理而非显式命名。

Method: 1. 引入Referring Scenario Comprehension (RSC)基准，包含段落长度的查询文本描述对象角色、用户目标和上下文线索；2. 提出ScenGround课程推理方法，结合监督预热和难度感知强化学习；3. 为每个实例标注可解释的难度标签（唯一性、杂乱度、大小、重叠、位置）。

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外分割；实验表明场景化查询暴露了当前模型在标准基准中未揭示的系统性失败，课程训练提高了在挑战性切片上的性能并能迁移到标准基准。

Conclusion: 场景化视觉定位是一个互补且更具挑战性的设置，需要超越字面匹配的深度理解；RSC基准支持细粒度分析，ScenGround方法为这一设置提供了参考点，展示了课程训练的有效性。
...
TL;DR: 提出可操控视觉表征（Steerable Visual Representations），通过早期融合文本到视觉编码器中，实现用自然语言引导全局和局部特征，解决现有视觉表征只能关注最显著视觉线索而无法关注次要概念的问题。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）的通用图像特征倾向于关注图像中最显著的视觉线索，无法引导其关注次要概念；而多模态大语言模型虽然可通过文本提示引导，但其表征偏向语言中心化，在通用视觉任务中效果下降。需要一种既能用自然语言引导又能保持通用视觉任务性能的视觉表征方法。

Method: 提出可操控视觉表征，采用早期融合策略：在视觉编码器的各层中通过轻量级交叉注意力直接注入文本信息，而不是像CLIP等模型那样在编码后进行后期融合。这种方法允许文本提示直接引导视觉特征的提取过程。

Result: 1）建立了衡量表征可操控性的基准；2）可操控视觉特征能够聚焦于图像中任何所需对象，同时保持底层表征质量；3）在异常检测和个性化对象识别任务上匹配或优于专用方法；4）在分布外任务上表现出零样本泛化能力。

Conclusion: 通过早期融合文本到视觉编码器，实现了既能用自然语言引导又能保持通用视觉任务性能的可操控视觉表征，解决了现有视觉表征方法在关注特定概念和保持通用性之间的权衡问题。
...
TL;DR: ModMap：首个原生多视角多模态3D异常检测与分割框架，通过跨模态特征映射和视角依赖调制实现多视角协同分析


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法通常独立处理不同视角，缺乏对多视角和多模态数据协同分析的统一框架，限制了异常检测性能

Method: 1）跨模态特征映射范式学习模态和视角间的特征映射；2）特征级调制显式建模视角依赖关系；3）跨视角训练策略利用所有可能的视角组合；4）多视角集成与聚合进行异常评分；5）为工业数据集训练并发布基础深度编码器

Result: 在SiM3D基准测试中（首个多视角多模态3D异常检测与分割基准），ModMap取得最先进性能，大幅超越先前方法

Conclusion: ModMap通过原生多视角多模态框架，实现了3D异常检测与分割的显著性能提升，证明了跨视角协同分析和特征映射的有效性
...
TL;DR: 提出一个从AAA游戏提取的大规模动态数据集，包含400万连续帧的RGB和G-buffer数据，用于提升生成式逆渲染和正向渲染的真实感和时域一致性，并提出基于VLM的无真值评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集的真实感和时域一致性不足，限制了生成式逆渲染和正向渲染在真实场景中的扩展。需要弥合这一领域差距，为双向渲染提供高质量的训练和评估数据。

Method: 1. 使用新颖的双屏拼接捕获方法从视觉复杂的AAA游戏中提取数据；2. 收集400万连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道数据；3. 涵盖多样场景、视觉效果和环境条件（包括恶劣天气和运动模糊变体）；4. 提出基于视觉语言模型（VLM）的评估协议，从语义、空间和时域一致性三个维度评估逆渲染性能。

Result: 1. 数据集显著提升了逆渲染器的跨数据集泛化能力和可控生成质量；2. 基于VLM的评估结果与人类判断高度相关；3. 结合工具包，正向渲染器能够通过文本提示从G-buffer编辑AAA游戏的风格。

Conclusion: 该大规模动态数据集有效解决了生成式双向渲染中的领域差距问题，提出的VLM评估协议为无真值逆渲染评估提供了可靠方案，为高质量可控视频生成和游戏风格编辑开辟了新途径。
...
TL;DR: ActionParty：首个能同时控制多个主体的视频世界模型，通过引入主体状态令牌解决动作绑定问题，在46个环境中控制最多7个玩家
...
Result: 在Melting Pot基准测试中，首次实现能同时控制最多7个玩家的视频世界模型，覆盖46个多样化环境；在动作跟随准确性和身份一致性方面显著提升，支持复杂交互中的稳健自回归主体追踪

Conclusion: ActionParty解决了视频扩散模型中的动作绑定问题，实现了首个可同时控制多个主体的视频世界模型，为生成式视频游戏提供了新能力
...
TL;DR: 该论文研究了在线交流中常见的重复拉长形式（RLF）对情感分析的重要性，创建了首个RLF多领域数据集Lengthening，并提出ExpInstruct框架提升LLM对RLF的理解和可解释性。
...
Method: 1）基于先前语言学研究，创建首个专注于RLF情感分析的多领域数据集Lengthening（85万样本）；2）提出ExpInstruct两阶段指令调优框架，提升LLM对RLF的性能和可解释性；3）提出统一方法来量化语言模型对非正式表达的理解能力。

Result: 1）RLF句子是富有表现力的表达方式，可作为文档级情感的特征标记；2）RLF对在线内容分析具有潜在价值；3）微调的预训练语言模型在性能上可超越零样本GPT-4，但在解释性上不如；4）ExpInstruct能使用有限样本提升开源LLM，使其在RLF任务上达到零样本GPT-4的性能和可解释性水平。

Conclusion: RLF是情感分析中被忽视但重要的表达形式，通过专门的ExpInstruct框架和Lengthening数据集，可以显著提升语言模型对这类非正式表达的理解能力和可解释性，为在线内容分析提供新工具。
...
TL;DR: 通过训练时强化学习和测试时并行思考两种互补方法，扩展竞争性编程的推理token预算，实现高效的问题解决


<details>
  <summary>Details</summary>
Motivation: 解决竞争性编程中单次推理token预算有限的问题，通过扩展推理能力提高模型在复杂编程问题上的表现

Method: 1) 训练时强化学习：采用验证RL预热和随机剪枝优化训练轨迹；2) 测试时并行思考：引入多轮并行思考管道，将token预算分配到多个线程和轮次中，包括生成、验证和精炼阶段

Result: 从Seed-OSS-36B开始，完整系统使用16线程和每线程16轮，平均每个问题使用760万token，在pass@1上匹配基础RL模型的oracle pass@16，并在AetherCode的456个困难竞争性编程问题上超越GPT-5-high

Conclusion: 结合训练时强化学习和测试时并行思考的互补方法，能够有效扩展推理token预算，显著提升模型在复杂竞争性编程问题上的性能表现
...
TL;DR: M2-Verify是一个大规模多模态数据集，用于评估科学主张与多模态证据之间的一致性，包含超过469K个实例，涵盖16个领域，模型在复杂场景下表现显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏规模、领域多样性和视觉复杂性，无法真实评估科学主张与其多模态证据之间的一致性，需要构建更全面的评估数据集。

Method: 从PubMed和arXiv获取数据构建M2-Verify数据集，包含超过469K个实例，涵盖16个领域，通过专家审核进行严格验证，并进行广泛的基线实验评估模型性能。

Result: 最先进模型在低复杂度医学扰动上达到85.8% Micro-F1，但在高复杂度挑战（如解剖结构变化）上性能降至61.6%；专家评估发现模型在生成科学解释时存在幻觉问题。

Conclusion: M2-Verify填补了科学主张一致性评估的空白，揭示了现有模型的局限性，提供了数据集实用性和使用指南，为多模态科学推理研究提供了重要资源。
...
TL;DR: 该研究提出了一种特征增强框架来改进语言模型中人类偏好的学习，通过添加可解释信号（如响应长度、拒绝指标、毒性分数等）来捕捉人类判断的多维性，在Anthropic HHRLHF数据集上显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型中人类偏好学习面临根本性挑战，因为奖励建模依赖于微妙、主观的比较而非明确的标签。现有方法在标准成对偏好设置下性能有限（ROC AUC低于0.74），难以捕捉人类判断的多维特性。

Method: 提出混合特征增强框架，在文本表示基础上添加可解释信号：响应长度、拒绝指标、毒性分数和提示-响应语义相似度。使用Anthropic HHRLHF数据集评估10个多样化大语言模型，采用SHAP和LIME进行细粒度可解释性分析。

Result: 特征增强方法在所有模型上带来一致改进，ROC AUC最高达到0.84，成对准确率显著提高，其中DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化的安全性和支持性框架而非孤立关键词。偏差分析表明特征交互影响偏好学习。

Conclusion: 通过增强文本表示与可解释信号，能够更有效地捕捉人类偏好的多维特性，显著提升语言模型在偏好学习任务上的性能，同时提供对模型决策过程的深入理解。
...
Method: 1. 从逐步推理轨迹语料库中分解每个轨迹为自包含的子问题-子程序对，构建包含3200万条程序性知识条目的数据存储；2. 推理时使用轻量级思维内提示让模型表达核心子问题；3. 在推理轨迹中检索相关子程序；4. 在多样化检索到的子程序作为隐式程序先验下进行推理。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory一致优于使用文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线。在更高推理预算下，相比无检索方法提升达19.2%，相比最强计算匹配基线提升7.9%。消融研究表明收益来自源轨迹的广泛程序覆盖以及分解和检索设计。

Conclusion: Reasoning Memory通过系统性地检索和重用程序性知识，有效提升了语言模型在复杂推理任务上的性能。该方法的关键在于将推理轨迹分解为可检索的子问题-子程序对，并在推理时将其作为隐式先验使用。
...
TL;DR: 该研究通过因果干预实验发现，语言模型在回答实体相关事实问题时，存在稀疏的、实体选择性的MLP神经元，这些神经元主要分布在早期层，能够实现紧凑的实体检索而非纯粹的渐进式信息丰富化。


<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答许多以实体为中心的事实性问题，但其内部机制尚不明确。研究者希望探究语言模型在处理实体相关事实查询时的内部工作机制，特别是实体信息是如何被检索和利用的。

Method: 使用模板化提示定位实体选择性MLP神经元，在PopQA基准的200个实体上进行因果干预验证。通过负向消融（negative ablation）和受控注入（controlled injection）实验，分析神经元对实体检索的影响。同时测试了模型对别名、缩写、拼写错误和多语言形式的鲁棒性。

Result: 定位到的神经元主要集中在早期层；负向消融导致实体特异性遗忘，而受控注入能提高答案检索准确性；单个神经元的激活足以恢复实体一致的预测；模型对别名等形式具有鲁棒性，支持规范化解释；效果显著但非普遍，流行实体覆盖率更高。

Conclusion: 研究识别出了稀疏的、因果可操作的访问点，可用于分析和调节实体条件化的事实行为，揭示了语言模型通过紧凑的实体检索机制而非纯粹的渐进式信息丰富化来处理实体相关事实查询。
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被认为反映了翻译问题或困难，快速打字则被视为自动化翻译过程。然而，如何确定区分自动化与反思性翻译过程的停顿阈值一直存在争议，需要更可靠的方法来识别生产单元中断。

Method: 比较了三种最近的计算停顿阈值方法，并提出了一种新的计算生产单元中断的方法进行评估

Result: 论文比较了不同方法的性能，并展示了新方法在识别翻译生产单元中断方面的有效性

Conclusion: 新提出的生产单元中断计算方法为翻译过程研究提供了更可靠的工具，有助于更准确地识别翻译中的认知处理单元和问题点
...
TL;DR: MiCP是首个用于多轮推理的保形预测框架，通过跨轮次分配错误预算，在保证整体覆盖保证的同时实现早期停止，减少推理轮次和成本。


<details>
  <summary>Details</summary>
Motivation: LLMs在多轮推理（如自适应RAG和ReAct）中面临何时停止的关键挑战。现有方法依赖启发式停止规则或固定轮次预算，无法提供最终预测包含正确答案的正式保证，这在金融、医疗等高风险领域尤其成问题。

Method: 提出Multi-Turn Language Models with Conformal Prediction (MiCP)框架，将保形预测扩展到多轮推理场景。MiCP通过跨轮次分配错误预算，使模型能够在保持整体覆盖保证的同时实现早期停止。

Result: 在自适应RAG和ReAct上验证MiCP，在单跳和多跳问答基准测试中达到目标覆盖度，同时减少了推理轮次、成本和预测集大小。还引入了联合评估覆盖有效性和回答效率的新指标。

Conclusion: MiCP是首个为多轮推理提供正式覆盖保证的保形预测框架，解决了现有方法在何时停止方面的局限性，在保持准确性的同时提高了效率。
...
TL;DR: 提出基于预测效度的基准测试质量评估框架，通过WILD数据集分析不同技术在预算约束下预测模型在未见任务上性能的能力，展示结合改进的多维项目反应理论和自适应项目选择能高效预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有大量基准测试用于评估大语言模型，但研究表明模型性能通常可由少量潜在能力因子解释。当前缺乏系统方法比较不同基准测试框架的质量，需要基于预测效度建立更高效、有原则的基准测试方法。

Method: 收集WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务（来自27个数据集）的109,564个独特项目-模型响应对。采用改进的多维项目反应理论模型结合基于最优实验设计的自适应项目选择，并引入成本感知折扣因子优化评估成本。

Result: 该方法在仅观察16个项目后，能在112个保留基准测试任务上实现平均绝对误差低于7%。通过成本感知选择标准，达到7% MAE所需的总token数从141,000减少到22,000，评估成本降低85%。

Conclusion: 基于预测效度的基准测试框架能显著提高评估效率，结合多维IRT和自适应项目选择可在有限预算下准确预测模型在未见任务上的性能，为更高效、有原则的基准测试提供了方法论基础。
...
TL;DR: 使用词嵌入和随机森林分类器，研究发现即使语义无关的印地语同义词也能根据其梵语或波斯-阿拉伯语起源被准确分类，表明使用模式保留了词源信号，同义词反映了与起源相关的系统性差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证同义词理论：虽然绝对同义词理论上不应存在，但即使表示相同概念的同义词可能反映不同视角或携带不同的文化关联。印地语中梵语和波斯-阿拉伯语借词形成大量同义词对，为定量测试这些理论主张提供了理想案例。

Method: 使用印地语同义词的词嵌入数据，训练随机森林分类器来区分梵语起源和波斯-阿拉伯语起源的词汇。该方法特别关注语义无关的同义词对，以排除语义内容的影响，仅基于分布模式进行分类。

Result: 随机森林分类器成功区分了梵语和波斯-阿拉伯语起源的词汇，即使这些词汇在语义上无关。这表明使用模式保留了词源信号，上下文编码了词源信息，同义词之间存在与起源相关的系统性差异。

Conclusion: 研究提供了定量证据，表明上下文编码词源信号，同义词可能反映与起源相关的微妙但系统性区别。同义词可以提供不同视角，词源相关的词汇可能形成不同的概念子空间，创造由历史起源塑造的新型语义框架。结果突显了上下文在捕捉超越传统语义相似性的细微区别方面的强大能力。
...
TL;DR: 研究发现引用粒度（句子级、段落级、文档级）对模型归因质量有显著影响，中等粒度（段落级）表现最佳，而细粒度引用会破坏模型性能


<details>
  <summary>Details</summary>
Motivation: 在归因生成中，引用粒度是一个关键设计选择。虽然细粒度引用通常被认为便于人工验证，但其对模型性能的影响尚未得到充分研究。本文旨在探索不同引用粒度对模型归因质量的影响，特别是细粒度引用是否真的能提高模型性能。

Method: 研究分析了四种不同规模的模型（8B-120B），比较了不同引用粒度（句子级、段落级、文档级）对归因质量的影响。通过系统实验评估了不同粒度下模型的归因质量和答案正确性，并分析了性能差异的原因。

Result: 1. 强制细粒度引用会降低归因质量16-276%；2. 归因质量在中等粒度（段落级）达到峰值；3. 细粒度引用破坏了证据归因所需的语义依赖关系；4. 过粗的引用引入干扰噪声；5. 性能差距随模型规模非单调变化，细粒度约束对大型模型影响更大；6. 最优引用粒度能显著提高归因质量，同时保持甚至提高答案正确性。

Conclusion: 仅为了人工验证而优化细粒度引用忽视了模型约束，会损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐，中等粒度（段落级）通常是最佳选择。
...
TL;DR: 论文通过电路级机制分析揭示了LLM中过度自信的根源，发现特定MLP块和注意力头在中间到后期层写入过度自信信号，并提出针对性推理时干预来改善校准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常表现出过度自信的错误：当产生事实错误的答案时，它们倾向于表达过高的置信度而非传达不确定性。这种过度自信会误导用户并削弱置信分数作为可靠不确定性信号的可靠性，但其内部机制尚不清楚。

Method: 采用电路级机制分析方法，围绕三个轴展开：1) 将语言化置信度捕获为可微分的内部信号；2) 识别导致置信度膨胀的因果电路；3) 利用这些见解进行针对性推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 发现在中间到后期层中，一组紧凑的MLP块和注意力头在最终标记位置一致地写入置信度膨胀信号。进一步证明对这些电路的针对性推理时干预能显著改善校准效果。

Conclusion: LLM中的语言化过度自信是由可识别的内部电路驱动的，可以通过针对性干预来缓解。这为理解模型不确定性机制和改善校准提供了新的见解。
...
TL;DR: 该研究通过计算分析129,451首波斯诗歌，将反复出现的象征形式整合为可追溯的家族，构建多层图展示其关系演变，发现波斯象征体系是随时间变化的动态系统而非固定剧目。


<details>
  <summary>Details</summary>
Motivation: 传统计算研究将波斯诗歌材料扁平化为孤立词语或宽泛文档语义，忽略了波斯诗学中相关形式以家族形式传播并通过重复关系获得力量这一实用组织单元。研究旨在捕捉这种象征结构的动态演变。

Method: 使用129,451首诗歌语料库，将反复出现的象征形式整合为可追溯的家族，分离意象材料与神圣及宫廷参照，构建多层图展示其关系。按11个希吉拉世纪分箱分析时间演变。

Result: 象征核心相对稀疏，参照成分更密集，两者连接区域具有选择性。某些家族（如夜、日、土）分布广泛，酒器、花园空间、火焰、抒情声音后期增强，而声望编码和英雄宫廷词汇早期权重更高。模块性增加，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。枢纽位置变化：苏菲长袍后期显著，紫罗兰等衰退，酒杯在整个时间线上保持中心地位。

Conclusion: 波斯象征体系并非固定剧目，而是内部权重和连接随时间变化的长期系统。计算分析揭示了象征家族的关系结构和历史演变模式。
...
TL;DR: 论文提出了Magic, Madness, Heaven, Sin框架，将大语言模型的输出变化建模为同质-异质轴，根据任务和规范性目标进行价值评估，并分析了四个规范性语境及其交互影响。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于输出变化的术语碎片化，缺乏统一的框架来理解不同任务背景下输出变化的规范性目标。研究者需要明确任务背后的规范性目标，以系统化地分析输出变化的价值和影响。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴。将任务组织到四个规范性语境中：认知性（事实性）、交互性（用户效用）、社会性（代表性）和安全性（鲁棒性）。分析每个语境下的失败模式和词汇，并应用框架分析所有成对的跨语境交互。

Result: 框架揭示了优化一个目标（如提高安全性）可能无意中损害人口统计代表性或创造性多样性。跨语境分析显示不同规范性目标之间存在复杂的权衡和潜在冲突。

Conclusion: 输出变化应作为由任务目标塑造的属性进行情境感知评估，而非模型的固有特征。需要明确任务的规范性目标，以更好地理解和评估大语言模型的输出变化。
...
<details>
  <summary>Details</summary>
Motivation: 探索指令遗忘范式是否适用于扩散模型等非语言生成模型，验证自然语言指令能否在推理时有效修改扩散模型行为

Method: 通过多概念多提示变体的控制实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态，研究指令遗忘在扩散模型中的效果

Result: 扩散模型无法通过自然语言遗忘指令有效抑制目标概念，遗忘指令未能持续降低对目标概念标记的注意力，导致目标概念表征在生成过程中持续存在

Conclusion: 扩散模型存在基于提示的指令遗忘的根本性限制，有效的遗忘需要在推理时语言控制之外进行干预
...
### [120] [Read More, Think More: Revisiting Observation Reduction for Web Agents](https://arxiv.org/abs/2604.01535)
*Masafumi Enomoto,Ryoma Obara,Haochen Zhang,Masafumi Oyamada*

Main category: cs.CL

TL;DR: 该论文重新评估了网页智能体观察表示的选择，发现最优表示取决于模型能力和思考token预算：低能力模型适合紧凑表示（可访问性树），高能力模型则受益于详细HTML表示，增加思考token会放大HTML优势。


<details>
  <summary>Details</summary>
Motivation: 重新审视网页智能体中观察表示的选择趋势，传统做法将HTML的冗长视为性能障碍而采用观察缩减，但作者认为最优表示可能取决于模型能力和资源约束。

Method: 通过实验分析不同模型能力（低/高能力LLMs）在不同思考token预算下，使用不同观察表示（HTML vs 可访问性树）的性能差异，并进行错误分析，同时评估观察历史（包括diff-based表示）的影响。

Result: 发现紧凑观察适合低能力模型，详细HTML观察对高能力模型更有利，增加思考token会增强HTML优势；高能力模型利用HTML中的布局信息进行更好的动作定位，低能力模型在长输入下产生更多幻觉；观察历史（特别是diff-based表示）能提升大多数模型和设置下的性能。

Conclusion: 提出实用指南：基于模型能力和思考token预算自适应选择观察表示，并使用diff-based表示整合观察历史，为网页智能体设计提供数据驱动的建议。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学临床文档应用中面临关键挑战：使用特定医学数据集微调时会出现显著的指令跟随能力"遗忘"，这阻碍了通用LLM在临床应用中的采用。

Method: 提出模型合并框架，通过基于插值的合并方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，以在权重空间进行高效领域适应。

Result: 综合评估显示合并模型能有效缓解灾难性遗忘，保持临床领域专业知识，保留指令跟随能力，在有限监督下(如64-shot vs 256-shot)达到与完全微调基线相当的性能。

Conclusion: 权重空间合并为开源LLM适应临床应用提供了高度可扩展的解决方案，有助于在资源受限的医疗环境中更广泛部署，同时解决遗忘问题并保持模型能力。
...
TL;DR: DeltaMem：单智能体端到端人物记忆管理系统，通过基于记忆的Levenshtein距离和强化学习框架优化记忆管理性能


<details>
  <summary>Details</summary>
Motivation: 现有的人物中心记忆系统在多智能体设置中常出现信息丢失和场景适应性差的问题，导致性能不佳，需要更稳健的记忆管理方案

Method: 提出DeltaMem单智能体端到端记忆管理系统；合成用户-助手对话数据集及操作级记忆更新标签；引入基于记忆的Levenshtein距离形式化记忆更新奖励；设计定制强化学习框架增强管理能力

Result: DeltaMem在LoCoMo、HaluMem、PersonaMem等多个长期记忆基准测试中均优于所有产品级基线，包括无训练版本和RL训练版本

Conclusion: DeltaMem通过单智能体端到端框架有效解决了多智能体记忆管理中的信息丢失和脆弱性问题，在长期记忆管理任务中表现出色
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，通过增量聚合激活协方差和单次特征值分解，实现无训练、快速、最优的层间低秩近似，在保证理论最优的同时提供3-70倍加速。


<details>
  <summary>Details</summary>
Motivation: 大语言模型部署受限于静态权重和动态KV缓存的存储与带宽需求。现有SVD压缩方法存在两个关键局限：一些方法在重构误差上不是最优的，另一些方法理论最优但实际效率低下。

Method: 提出Swift-SVD框架，通过增量聚合给定输入批次输出激活的协方差，并在聚合后执行单次特征值分解，实现无训练、快速、最优的层间低秩近似。采用有效秩分析局部层间可压缩性，并设计动态秩分配策略，同时考虑局部重构损失和端到端层重要性。

Result: 在6个LLM和8个数据集上的广泛实验表明，Swift-SVD优于最先进的基线方法，在实现最优压缩精度的同时，端到端压缩时间加速3-70倍。

Conclusion: Swift-SVD提供了一个硬件友好的压缩解决方案，同时保证理论最优、实际效率和数值稳定性，有效解决了大语言模型部署中的存储和带宽瓶颈问题。
...
TL;DR: 印尼全国性教师调查显示AI在教学中的应用增加但分布不均，主要用于减轻教学准备工作量，但仍面临通用输出、基础设施和情境适应性等挑战


<details>
  <summary>Details</summary>
Motivation: 尽管AI在印尼课堂中逐渐应用，但缺乏大规模、以教师为中心的实证证据，阻碍了开发适合当地情境的AI系统和政策制定

Method: 对349名K-12教师（涵盖小学、初中、高中）进行全国性调查

Result: 1. AI在教学法、内容开发和教学媒体方面的使用增加但分布不均；2. 小学教师使用更一致，高中教师参与较少；3. 职业生涯中期教师更重视AI，印尼东部教师感知价值更高；4. 教师主要用AI减轻教学准备工作量（评估、课程规划、材料开发）；5. 通用输出、基础设施限制和情境适应性不足阻碍有效课堂整合

Conclusion: 需要开发更情境化、教师友好的AI工具，并解决基础设施和培训需求，以支持印尼教育中的AI有效整合
...
TL;DR: 论文分析发现当前声明验证基准主要测试证据提取能力，而多句合成和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理能力


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对这些基准实际测试推理能力的系统性理解。需要了解当前基准主要测试什么类型的推理，以及是否存在系统性偏差

Method: 使用GPT-4o-mini为9个数据集的24K声明验证示例生成结构化推理轨迹，分析推理类型分布。使用紧凑的1B参数推理验证器分析五种错误类型及其在不同领域的分布

Result: 发现直接证据提取占主导地位，多句合成和数值推理严重不足。数据集层面存在明显偏差：有些几乎只测试词汇匹配，有些约一半情况需要信息合成。错误类型分布随领域变化：通用领域主要受词汇重叠偏差影响，科学领域因过度谨慎，数学领域因算术推理失败
...
TL;DR: PRCCF框架通过人物角色引导的检索和因果感知的认知过滤，提升情感支持对话中的深度上下文理解和情感推理能力


<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话方法在有效支持深度上下文理解方面面临挑战，需要更有效的方法来提升情感推理和上下文认知能力
...
TL;DR: 研究发现监督微调中思维链轨迹来源对模型泛化能力有重要影响：DeepSeek-R1数据训练损失更低但泛化更差，因其发散式推理模式导致模型陷入冗余探索分支；通过过滤频繁分支轨迹可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究不同来源的思维链轨迹如何影响大型推理模型的泛化性能，揭示训练损失与泛化能力之间的悖论关系。

Method: 使用DeepSeek-R1-0528和gpt-oss-120b生成的已验证思维链轨迹进行对比研究，控制问题集相同。进行多层面分析，包括token级SFT损失和步骤级推理行为分析。提出通过过滤频繁分支轨迹的方法改进SFT泛化。

Result: DeepSeek-R1数据训练损失更低但泛化性能显著更差。分析显示gpt-oss-120b采用收敛式演绎推理，而DeepSeek-R1偏好发散式分支探索。过滤频繁分支轨迹后，DeepSeek-R1子集训练在AIME25上提升5.1%，BeyondAIME上提升5.5%，五个基准平均提升3.6%。

Conclusion: 思维链轨迹的推理模式对模型泛化能力有决定性影响，收敛式推理轨迹优于发散式探索轨迹。简单的轨迹过滤方法能有效提升监督微调的泛化性能，为高质量思维链数据选择提供了指导。
...
Method: 采用基于合成内窥镜报告的两阶段适应策略：1) 领域特定语言建模；2) 噪声鲁棒性增强。系统保持紧凑模型规模（220M参数），支持高效边缘部署。

Result: 在回顾性评估中，CER从20.52%降至14.14%，Med ACC从54.30%提升至87.59%；前瞻性多中心研究中，CER从16.20%降至14.97%，Med ACC从61.63%提升至84.16%。实时因子RTF为0.005，显著优于Whisper-large-v3。

Conclusion: EndoASR证明了领域自适应ASR可作为胃肠内窥镜中人机协作的可靠接口，在多中心真实临床环境中表现一致，且改进的ASR质量直接增强了下游结构化信息提取和临床医生-AI交互。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时程复杂任务中记忆模块至关重要，但现有记忆方法缺乏系统性比较和评估，需要建立统一框架进行综合分析

Method: 1) 提出统一框架整合现有智能体记忆方法；2) 在两个知名基准上广泛比较代表性记忆方法；3) 基于实验分析设计新的记忆方法，利用现有方法模块

Result: 1) 系统评估了现有记忆方法的有效性；2) 设计的新记忆方法超越了现有最先进方法；3) 提供了对记忆方法行为的深入理解

Conclusion: 通过统一框架和系统性比较，为LLM智能体记忆研究提供了新见解，并指出了未来研究方向，加深了对现有方法行为的理解
...
<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临声学模式模糊和标注数据不足的挑战，特别是在真实场景中情感边界不清晰的情况下。传统数据驱动模型在处理模糊样本时效果有限，需要结合人类知识来提升识别性能。

Method: 提出基于LLM推理的人机协作框架：1) 使用声学特征模型提供置信度和特征级证据；2) 引入置信度路由机制区分简单和模糊样本，将不确定案例委托给LLM进行深度推理；3) LLM推理基于从人类标注行为中提取的结构化规则；4) 采用迭代优化策略，通过错误分析和规则更新持续改进系统性能。

Result: 在包含2,764个样本的越南语语音数据集（三种情感：平静、愤怒、恐慌）上进行实验，标注者间一致性高（Fleiss Kappa = 0.8574）。方法达到86.59%的准确率和0.85-0.86的Macro F1分数，在处理模糊和难分类案例方面表现出色。

Conclusion: 该工作强调了数据驱动模型与人类推理结合的重要性，为低资源环境下的语音情感识别提供了一种鲁棒且模型无关的方法。人机协作框架能有效处理模糊样本，提升系统在真实场景中的性能。
...
TL;DR: 本文提出了一种更细致的保加利亚语文本毒性检测方法，通过构建毒性词汇本体和标注数据集，训练BERT模型实现毒性分类，同时避免误判医学术语和少数群体相关内容。


<details>
  <summary>Details</summary>
Motivation: 当前在线内容毒性检测系统存在过度屏蔽问题，常常误将医学术语和少数群体相关文本识别为有毒内容，导致有价值信息被过滤。针对保加利亚语，需要一种更细致的方法来准确识别毒性内容，同时保留重要信息。
...
Conclusion: 本文提出的方法为保加利亚语毒性检测提供了更细致的解决方案，能够准确识别有毒内容，同时避免误判医学术语和少数群体相关文本，具有实际应用价值。
...
TL;DR: LiveMathematicianBench是一个基于arXiv最新论文的动态多选基准测试，用于评估大语言模型在研究生水平数学推理上的能力，通过证明草图引导的干扰项和替换抵抗机制来防止数据污染和表面匹配。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准测试存在合成设置和数据污染的局限性，无法真实评估大语言模型在研究生水平数学推理上的能力。随着大语言模型越来越多地集成到科学工作流程中，对其数学能力进行严格评估成为实际需求。

Method: 1. 使用模型训练截止日期后发布的arXiv论文构建动态多选基准测试；2. 引入13类逻辑分类法（如蕴含、等价、存在性、唯一性）进行细粒度评估；3. 采用证明草图引导的干扰项生成管道，利用高级证明策略构建看似合理但无效的答案选项；4. 引入替换抵抗机制以区分答案识别与实质性推理；5. 采用双模式协议评估模型利用证明草图进行推理的能力。

Result: 基准测试远未饱和：最佳模型Gemini-3.1-pro-preview仅达到43.5%准确率。在替换抵抗评估下，准确率大幅下降：GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%，低于20%的随机基线。双模式协议显示，访问证明草图能带来一致的准确率提升，表明模型可以利用高级证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的研究生水平数学推理测试平台，能够更真实地评估大语言模型的数学推理能力，揭示当前模型在实质性数学理解方面仍存在显著局限。
...
TL;DR: 论文提出基于指令微调和离散控制标记的领域无关可控自动文本简化框架，发现小模型在特定条件下可竞争大模型，但可控性受训练数据中目标属性变化程度影响，同时指出现有评估指标不足以衡量控制效果
...
Method: 提出基于指令微调和离散控制标记的领域无关CATS框架，使用三种模型家族(Llama、Mistral、Qwen，1-14B参数)在四个领域(医学、公共管理、新闻、百科全书文本)进行实验，通过采样和分层实验分析数据分布问题。

Result: 小模型(1-3B)在特定条件下可竞争大模型，但可靠的可控性取决于训练数据是否编码足够的目标属性变化。可读性控制(FKGL、ARI、Dale-Chall)学习一致，而压缩控制因现有语料库中信号变化有限而表现不佳。标准简化和相似性指标不足以衡量控制效果。

Conclusion: 需要基于误差的度量来评估目标-输出对齐，朴素的数据分割可能引入分布不匹配问题，影响训练和评估效果。训练数据中目标属性的变化程度对可控性学习至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法（如PPO）成本高、不稳定，而替代方法需要大量数据且可能削弱LLM的泛化能力，需要更高效的对齐框架
...
TL;DR: PLOT提出了一种基于最优传输的token级损失函数，用于增强基于微调的对齐中的偏好学习，通过将偏好学习建模为最优传输问题来对齐模型输出与人类偏好，同时保持LLM的原始分布以确保稳定性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感以及全局token级关系建模不足等问题。需要一种更有效、稳定且理论基础的偏好学习方法。

Method: PLOT通过最优传输问题形式化偏好学习，使用token级损失函数，利用token嵌入捕获语义关系，实现全局感知的优化，同时保持LLM原始分布以确保稳定性和鲁棒性。

Result: 在人类价值观和逻辑与问题解决两个偏好类别、七个子偏好的实验中，PLOT一致提高了对齐性能，同时保持了流畅性和连贯性，验证了最优传输作为偏好学习原则性方法的有效性。

Conclusion: 最优传输为LLM偏好学习提供了理论基础框架，PLOT方法在保持模型稳定性的同时有效提升了对齐性能，为LLM偏好学习提供了新的理论见解。
...
<details>
  <summary>Details</summary>
Motivation: 分析300万真实交互发现硬补全（HC）的局限性：61%的生成建议被编辑或拒绝，尽管与用户后续代码相似度超过80%，表明模型在特定token位置经常做出错误预测

Method: 将代码补全形式化为不确定性下的成本最小化问题，基于填充占位符比纠正错误成本更低的观察，构建从真实编辑日志过滤的训练数据，设计基于成本的奖励函数进行强化学习

Result: 在1.5B-14B参数模型上的广泛评估表明，APC将预期编辑成本降低19%到50%，同时保持标准HC性能
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于强调融合而忽视了噪声特征的不确定性和细粒度推理，需要更鲁棒且能建模上下文的方法
...
<details>
  <summary>Details</summary>
Motivation: 心衰短期死亡率预测具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要评估不同方法（仅文本、仅结构化、多模态、LLM）在预测性能上的差异

Method: 在法国心衰队列上评估transformer模型，比较四种方法：仅文本、仅结构化数据、多模态融合、LLM方法。特别研究了实体级表示与CLS嵌入的对比，以及监督多模态融合策略

Result: 1) 用实体级表示丰富临床文本比仅用CLS嵌入能改善预测；2) 文本与结构化变量的监督多模态融合获得最佳整体性能；3) 大语言模型在不同模态和解码策略中表现不一致，仅文本提示优于结构化或多模态输入

Conclusion: 实体感知多模态transformer为心衰短期结局预测提供了最可靠的解决方案，而当前LLM提示方法在临床决策支持中仍有局限，需要进一步改进
...
<details>
  <summary>Details</summary>
Motivation: 宣传性新闻特别隐蔽，因为它们倾向于将导向性信息与看似可靠的事实报道混合。现有基于BERT等语言模型的方法虽然有效，但由于数据收集偏差容易过拟合训练数据集，需要提高分类鲁棒性和对新来源的泛化能力。

Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧。通过混合方法增强分类鲁棒性，减少对特定训练数据集的过拟合。

Result: 结果显示该方法优于等效的纯文本方法。消融研究和可解释性分析证实了添加特征的好处，验证了神经符号方法的有效性。
...
TL;DR: 提出衡量词序变异相对于交换距离最小化最优性的数学框架，证明跨语言手势至少77%最优，引入二次分配问题作为语言优化研究的统一框架。


<details>
  <summary>Details</summary>
Motivation: 研究语言词序是否遵循交换距离最小化原则，即给定源顺序，在置换多面体中距离更近的词序应该成本更低、更可能被采用。需要建立衡量词序变异最优性的数学方法。

Method: 1. 使用置换多面体（permutohedron）表示序列所有排列的结构；2. 开发衡量词序变异相对于交换距离最小化最优程度的数学框架；3. 引入二次分配问题（QAP）作为语言优化研究的统一框架。

Result: 跨语言手势至少达到77%的最优性，多次达到最优的情况不太可能是偶然的。建立了词序或手势顺序相对于交换距离最小化最优性的理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 机器翻译基准数据集虽然降低了成本并提供了规模，但存在噪声、结构丢失和质量不均等问题，削弱了对其可靠性的信心。需要能够大规模测量和验证翻译可靠性的方法

Method: 采用三步自动化质量保证方法：1) 结构性语料审计与针对性修复；2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与翻译服务（DeepL/ChatGPT/Google）比较；3) 基于LLM的片段级翻译错误分析

Result: 趋势一致：COMET分数较低的数据集在片段级别显示出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上与人编辑样本的对比也指向相同方向。发布了清理/校正后的EU20数据集和可复现代码
...
TL;DR: SAFE框架通过可验证的实体序列取代不可靠的思维链，在训练时验证消除噪声监督，在推理时动态检测未接地步骤，显著提升多跳问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有多跳问答基准测试经常奖励大型语言模型虚假的正确性，掩盖了未接地或有缺陷的推理步骤。需要转向更严格的推理评估框架。

Method: 提出SAFE动态基准测试框架，包含两个阶段：(1)训练时验证：建立原子错误分类和知识图谱接地验证管道，消除标准基准中的噪声监督；(2)推理时验证：基于验证数据集训练的反馈模型实时动态检测未接地步骤。

Result: 实验结果显示：SAFE在训练时暴露现有基准的关键缺陷（识别高达14%的实例为不可回答），在推理时显著优于标准基线，平均准确率提升8.4个百分点，同时保证推理轨迹的可验证性。

Conclusion: SAFE框架通过严格的实体接地验证，有效解决了多跳问答中虚假正确性的问题，为大型语言模型的推理评估提供了更可靠的方法。
...
TL;DR: 扩散模型在离散数据生成中面临挑战，研究发现DDPM采样器在离散分布的多模态临界区间内会进入低密度区域导致采样质量下降，提出结合自条件和q-sampling的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但在离散数据上的应用仍然具有挑战性。本文旨在探究高斯扩散模型与DDPM求解器在处理离散分布时遇到的困难，特别是当离散分布表示为连续空间中的混合δ分布时。

Method: 使用随机层次模型作为玩具模型进行分析，识别出噪声化数据密度变为多模态的临界采样区间。研究现有启发式方法（包括自条件和q-sampling求解器）如何缓解这一问题。提出将自条件与在临界区间内从DDPM切换到q-sampling相结合的方法。

Result: 在临界区间内，DDPM偶尔会进入模态之间的低密度区域，产生模型分布之外的输入，从而降低采样质量。自条件和q-sampling有助于缓解这一问题。结合自条件并在临界区间内切换到q-sampling的方法在文本、编程代码和蛋白质等多个领域的条件和非条件任务中都能提高生成质量。

Conclusion: 离散数据上的扩散模型面临特定的采样挑战，特别是在多模态临界区间。通过结合自条件和在适当时间切换到q-sampling的混合策略，可以有效改善离散分布的生成质量，这一发现在多个实际应用领域得到了验证。
...
TL;DR: 研究自监督语音模型训练过程中不同层次语言结构的编码何时出现，分析Wav2Vec2和HuBERT模型在荷兰语语音数据上的层间分布和学习轨迹


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能学习有效的语音表征，反映语言结构，但需要探究这些结构在训练过程中何时出现，以及不同层次语言结构的编码模式差异

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语语音数据上的中间检查点，研究多种语言结构在不同层间的编码模式和学习轨迹，考察预训练目标定义层次的影响

Result: 不同层次的语言结构在层间分布和学习轨迹上存在显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释；预训练目标的定义层次强烈影响语言结构的层间组织和学习轨迹，高阶预测任务（迭代优化的伪标签）能诱导更大的并行性

Conclusion: 自监督语音模型中语言结构的编码具有层次特异性，预训练目标的设计对语言表征的组织和学习过程有重要影响，为理解模型如何学习语言结构提供了新见解
...
TL;DR: 提出BidirLM方法，将因果生成语言模型转化为双向编码器，通过系统消融实验确定关键适应因素，采用权重合并和多领域数据混合策略避免灾难性遗忘，并通过与专业模型合并增强多模态能力。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转化为双向编码器的方法存在三个主要问题：缺乏训练目标共识、大规模时出现灾难性遗忘、难以灵活集成专业生成模型。需要开发一种通用方法来解决这些限制。

Method: 1) 在Gemma3和Qwen3模型上进行系统消融实验，确定成功适应的关键因素，特别是常被忽略的先验掩码阶段；2) 采用线性权重合并与轻量级多领域数据混合的双重策略，避免灾难性遗忘；3) 通过与专业因果模型合并，增强编码器的模态和领域特定能力。

Result: 开发出BidirLM系列五个编码器，在文本、视觉和音频表示基准测试中优于现有替代方案。该方法开源且适用于任何因果解码器LLM。

Conclusion: 提出了一种通用方法，成功将因果生成语言模型转化为高性能双向编码器，解决了现有方法的局限性，实现了多模态能力的灵活集成，为LLM的双向编码器转换提供了有效解决方案。
...
TL;DR: GOOSE提出了一种训练无关的推测解码框架，通过构建自适应脊柱树结构，利用高质量上下文匹配令牌形成深度链，低质量统计预测令牌形成宽分支，显著提升LLM推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在构建候选令牌树时未区分不同来源令牌的质量差异。研究发现上下文n-gram匹配令牌与统计预测令牌的接受率存在显著差异（中位数6倍差距），但现有方法仍使用对称的平衡树结构，限制了性能提升。

Method: GOOSE采用自适应脊柱树结构：将高接受率的上下文匹配令牌组织成深度链（脊柱），在每个节点上为低接受率的统计预测令牌创建宽分支。这种各向异性结构突破了平衡树的深度限制，在固定验证预算下最大化接受令牌数量。

Result: 在5个LLM（7B-33B）和5个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线提升12-33%。理论证明GOOSE每步接受的令牌数至少与单独使用任一令牌源时相同。

Conclusion: 通过识别并利用不同令牌源的质量差异，构建各向异性的自适应脊柱树结构，GOOSE显著提升了推测解码的效率，为训练无关的LLM加速提供了有效解决方案。
...
Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的基线模型。分析显示框架具有良好泛化性、与查询扩展模块正交集成、对噪声监督具有鲁棒性

Conclusion: RRPO通过直接对齐重排序与LLM生成质量，有效解决了传统重排序模型与下游生成过程脱节的问题，为检索增强生成提供了更有效的重排序解决方案
...
Method: 1. 提出prosodic ABX框架，扩展传统ABX判别任务来评估韵律对比；2. 构建并发布英语和日语最小对立对数据集；3. 结合普通话数据集，评估英语重音、日语音高重音和普通话声调的对比敏感性；4. 分析模型和层级排名在不同实验条件下的稳定性

Result: 1. 成功开发了prosodic ABX框架，仅需少量示例即可评估韵律对比；2. 模型和层级排名在多种实验条件下保持稳定，使其适用于低资源场景；3. 验证了自监督语音模型对英语重音、日语音高重音和普通话声调等韵律特征的编码能力

Conclusion: prosodic ABX框架为评估自监督语音模型的韵律敏感性提供了有效工具，无需大量标注数据。模型层级排名的稳定性表明该方法在低资源场景中具有实用价值，有助于理解模型如何编码韵律信息
...
TL;DR: 提出稳定性过滤方法改进推理行为转向向量构建，通过筛选行为稳定边界并移除问题特定噪声，显著提升数学推理性能


<details>
  <summary>Details</summary>
Motivation: 现有基于关键词匹配检测推理行为边界的方法存在严重缺陷：93.3%的检测边界行为不稳定，无法在相同前缀下重现目标行为，导致转向信号被稀释

Method: 1) 建立概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件；2) 提出稳定性过滤，仅保留模型能一致重现目标行为的边界；3) 结合内容子空间投影移除残留的问题特定噪声

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0%），构建的转向向量可在同架构模型家族中迁移，无需重新提取：Nemotron-Research-Reasoning-1.5B提升5.0%，DeepScaleR-1.5B-Preview提升6.0%

Conclusion: 稳定性过滤能有效识别真正的行为信号，构建更纯净的转向向量，显著提升推理性能并实现跨模型迁移，为训练自由控制语言模型推理行为提供了更可靠的方法
...
TL;DR: GaelEval：首个苏格兰盖尔语多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答，评估19个LLM发现前沿模型在语法任务上超越人类基线，专有模型优于开源模型，盖尔语提示有小幅优势。
...
Method: 开发GaelEval基准测试，包含三个部分：1)专家编写的形态句法多项选择题任务；2)文化背景翻译基准；3)大规模文化知识问答任务。评估19个LLM，并与30名流利说话者的人类基线对比，同时测试盖尔语提示的效果。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超越人类基线(78.1%)。专有模型持续优于开源模型，盖尔语提示带来小幅稳定优势(+2.4%)。在文化任务上，领先模型超过90%准确率，但多数系统在盖尔语提示下表现更差，绝对分数相对于手动基准存在膨胀。

Conclusion: GaelEval揭示前沿模型在盖尔语语法的多个维度上达到超人类性能，证明了盖尔语提示的效果，并显示专有模型相对于开源模型存在一致性能差距。该基准为少数民族语言评估提供了新标准。
...
TL;DR: 该论文在SOMD 2026软件提及共指消解任务中，比较了两种免微调方法：模糊匹配和上下文感知表示，两者均取得优异性能，CAR方法在噪声鲁棒性和可扩展性方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 研究跨文档软件提及共指消解任务，比较两种免微调方法的性能差异，探索不同噪声条件下方法的鲁棒性，为实际应用中的系统选择提供依据。

Method: 采用两种免微调方法：1) 模糊匹配(FM)：基于词汇字符串相似度的方法；2) 上下文感知表示(CAR)：结合提及级别和文档级别嵌入的方法。通过噪声注入实验分析方法的鲁棒性，并进行推理时间分析评估可扩展性。

Result: 在SOMD 2026任务中排名第二，两种方法均取得0.94-0.96的CoNLL F1分数，CAR比FM高1个百分点。噪声实验显示：边界噪声下CAR更鲁棒（F1下降0.07 vs 0.20），提及替换下FM更稳健（F1下降0.52 vs 0.63）。CAR的推理时间随语料规模近似线性增长，而FM呈超线性增长。

Conclusion: 软件名称的高表面规律性降低了复杂语义推理的需求。系统选择应考虑上游提及检测器的噪声特征和目标语料库的规模，CAR在大规模应用中更具效率和鲁棒性优势。
...
TL;DR: 提出Neuro-RIT框架，通过神经元引导的鲁棒指令调优解决检索增强语言模型对噪声上下文的脆弱性问题，从密集适应转向精确神经元对齐


<details>
  <summary>Details</summary>
Motivation: 检索增强语言模型在知识密集型任务中表现良好，但对不相关或噪声检索上下文仍然脆弱。现有方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性

Method: 提出Neuro-RIT框架：1) 使用基于归因的神经元挖掘显式解耦处理相关与不相关上下文的神经元；2) 引入两阶段指令调优策略：通过功能失活仅处理不相关上下文的神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏

Result: 在多样化QA基准测试上的广泛实验表明，Neuro-RIT始终优于强基线方法和鲁棒性增强方法

Conclusion: Neuro-RIT通过从密集适应转向精确神经元对齐，有效解决了检索增强语言模型对噪声上下文的脆弱性问题，实现了更好的噪声鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 传统人才推荐系统采用点对点范式，需要大语言模型重复处理文本，无法捕捉候选人列表中的关系，导致令牌消耗高且推荐效果不佳。同时，大语言模型在处理多项选择题和长文档时存在位置偏差和"迷失在中间"问题。

Method: 提出L3TR框架：1) 采用隐式策略利用大语言模型的潜在输出进行推荐；2) 设计块注意力机制和局部位置编码方法，增强文档间处理并缓解位置偏差和并发令牌偏差；3) 引入ID采样方法解决训练和推理阶段候选集大小不一致问题；4) 设计位置偏差和令牌偏差检测方法及无需训练的去偏方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法显示出持续改进。

Conclusion: L3TR框架通过列表式处理范式、创新的注意力机制和位置编码方法，有效解决了大语言模型在人才推荐中的位置偏差、令牌消耗和关系捕捉问题，为人才推荐系统提供了更高效准确的解决方案。
...
Method: 创建CV-18 NER数据集，基于Arabic Common Voice 18语料库进行人工NER标注，采用细粒度Wojood模式（21种实体类型）。对比级联系统（ASR+文本NER）和基于Whisper与AraBEST-RQ的端到端模型。

Result: 端到端系统在测试集上显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。分析表明阿拉伯语特定自监督预训练在ASR上表现良好，而多语言弱监督在联合语音到实体学习上更有效。

Conclusion: 端到端方法在阿拉伯语语音NER任务上具有优势，特别是在低资源环境下。阿拉伯语特定预训练和多语言弱监督各有优势，较大模型在低资源环境下可能更难适应。该研究提供了首个阿拉伯语语音NER的公开基准。
...
TL;DR: 该论文研究如何从大型语言模型中生成全面多样的有效回答，提出了多样性覆盖率指标来评估回答集的全面性，并开发了一个路由器来为每个查询选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 当面对允许大量有效答案的提示时，全面生成这些答案是满足广泛用户需求的第一步。现有研究缺乏评估模型生成全面多样回答能力的方法，且没有单一模型在所有开放提示上都能生成最全面的回答集。

Method: 1. 引入多样性覆盖率指标，衡量预测答案集中每个独特答案相对于相同数量最佳可能答案集的总质量分数；2. 评估18个LLM在生成多样化回答方面的表现；3. 基于发现每个提示都有特定模型表现最佳，开发了一个路由器来预测每个查询的最佳模型；4. 在NB-Wildchat数据集上训练路由器，并测试其在NB-Curated数据集和不同答案生成提示策略上的泛化能力。

Result: 1. 评估发现没有单一模型在所有开放提示上都能生成最全面的回答集；2. 但每个提示都存在一个显著优于其他模型的特定模型；3. 训练的路由器在NB-Wildchat上优于单一最佳模型基线（26.3% vs 23.8%）；4. 路由器在域外数据集NB-Curated和不同答案生成提示策略上表现出良好的泛化能力。

Conclusion: 该研究为在拥有多个模型的情况下生成全面答案奠定了基础，证明了通过路由器选择最佳模型可以显著提高回答集的多样性覆盖率，为构建更全面的AI回答系统提供了方法论。
...
TL;DR: 论文分析了语言模型扩展新词汇时的均值初始化问题，提出了基于语言学基础的初始化方法GTI，在生成推荐任务中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型扩展新词汇时采用均值初始化策略，但研究发现这种初始化会导致新token坍缩到退化子空间，抹除token间差异，使得后续微调难以完全恢复，表明token初始化是词汇扩展的关键瓶颈。

Method: 提出Grounded Token Initialization (GTI)方法：在微调前，通过配对的语言学监督，将新token映射到预训练嵌入空间中的不同、语义有意义的位置。这是一种轻量级的"基础化"阶段，利用预训练模型的一般知识来支持新token领域。

Result: GTI在大多数评估设置中优于均值初始化和现有的辅助任务适应方法，覆盖了工业规模和公共数据集的多个人工智能推荐基准。分析显示，基础化嵌入产生更丰富的token间结构，这些结构在微调过程中得以保持。

Conclusion: 初始化质量是词汇扩展的关键瓶颈，基于语言学基础的新token初始化(GTI)能更好地利用预训练模型的一般知识，为新token领域提供更有效的表示学习。
...
### [157] [Non-Rigid 3D Shape Correspondences: From Foundations to Open Challenges and Opportunities](https://arxiv.org/abs/2604.01274)
...
TL;DR: 这篇报告综述了三维形状对应估计领域的最新进展，将方法分为三大范式：基于函数映射的谱方法、施加离散约束的组合方法、以及直接恢复全局对齐的形变方法，并讨论了新兴挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 变形形状实例之间的对应估计是计算机图形学中长期存在的问题，从纹理传输到统计建模等众多应用都依赖于恢复准确的对应映射。由于下游应用需求不同，出现了多种方法，需要系统梳理和分类，以帮助研究人员、从业者和学生理解该领域的最新趋势和进展。

Method: 报告采用分类综述的方法，将现有技术分为三大范式：1）基于函数映射的谱方法；2）施加离散约束的组合方法；3）直接恢复全局对齐的形变方法。对每种范式的优缺点进行系统分析，并强调各领域的最新进展。

Result: 报告系统梳理了形状对应估计领域的技术发展脉络，明确了三种主要方法范式的特点和适用场景。同时识别了该领域的新兴挑战和机遇，包括使用视觉基础模型进行零样本对应，以及部分形状匹配等特别具有挑战性的任务。

Conclusion: 形状对应估计是一个活跃且不断发展的研究领域，三种主要范式各有优势和局限性。未来研究方向包括利用视觉基础模型实现零样本对应、解决部分形状匹配等挑战性问题，以及探索不同范式的融合方法以提升对应估计的准确性和鲁棒性。
...
### [158] [ColorGradedGaussians: Palette-Based Color Grading for 3D Gaussian Splatting via View-Space Sparse Decomposition](https://arxiv.org/abs/2604.01551)
*Cheng-Kang Ted Chao,Yotam Gingold*

Main category: cs.GR

TL;DR: 提出实时交互式3D高斯泼溅颜色编辑框架，通过视图空间调色板分解解决原始空间方法中像素级分解稀疏性不足的问题，实现专业色彩分级工作流


<details>
  <summary>Details</summary>
Motivation: 专业色彩编辑需要精确控制色彩（色调和饱和度）和亮度，现有基于调色板的3DGS方法在原始级别分解颜色，但稀疏的原始级别权重无法保证alpha混合后的像素级分解稀疏性，导致编辑影响非目标区域

Method: 采用视图空间调色板分解方法，泼溅权重而非颜色以优化场景可观察外观；引入基于逆重心坐标的几何损失来强制一致的稀疏模式，确保相似颜色具有相似分解

Result: 相比原始空间方法获得更优的编辑质量，支持基于调色板的重新着色、每调色板色调曲线进行色彩感知亮度调整，以及精确像素级约束，实现3DGS场景的实时交互

Conclusion: 提出的视图空间调色板分解框架解决了现有方法的局限性，为3D高斯泼溅场景提供了专业色彩分级工作流，实现了实时交互式颜色编辑
...
### [159] [Topology-First B-Rep Meshing](https://arxiv.org/abs/2604.02141)
*YunFan Zhou,Daniel Zint,Nafiseh Izadyar,Michael Tao,Daniele Panozzo,Teseo Schneider*

Main category: cs.GR

TL;DR: 提出一种保证B-Rep拓扑正确性的网格生成方法，通过将B-Rep拓扑作为网格化过程的不变量，在用户定义的容差范围内生成拓扑正确的网格，无需后处理修复。


<details>
  <summary>Details</summary>
Motivation: 现有B-Rep到网格的转换方法存在拓扑不一致问题，需要启发式特征合并和修复策略，这些方法缺乏拓扑保证且在处理复杂模型时经常失败。

Method: 将B-Rep拓扑作为网格化过程的不变量，算法在保证精确B-Rep拓扑的同时，允许用户定义单个容差来控制网格与底层参数曲面的偏差。这种解耦消除了后处理修复的需求。

Result: 在数千个来自ABC和Fusion 360存储库的真实CAD模型上评估，包括标准网格工具失败的实例。结果表明算法层面的拓扑保证能够实现适用于下游应用的可靠网格生成。

Conclusion: 通过将B-Rep拓扑作为网格化过程的不变量，该方法能够在任何可接受容差下生成拓扑正确的网格，几何保真度随容差增加而降低，但拓扑始终正确，从而实现了稳健的网格生成。
